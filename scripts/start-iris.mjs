#!/usr/bin/env node
/**
 * start-iris.mjs — Spawns one gateway-bridge daemon per agent in iris.json
 *
 * Usage:
 *   node scripts/start-iris.mjs          # start all agents (skips if already running)
 *   node scripts/start-iris.mjs --force  # kill all + restart (safe restart)
 *   node scripts/start-iris.mjs --stop   # kill all bridge daemons
 *   node scripts/start-iris.mjs --status # show running bridges
 */
...
const IRIS_DIR = path.resolve(process.env.IRIS_DIR || process.env.OPENCLAW_DIR || process.cwd());
const OPENCLAW_CFG = path.join(os.homedir(), ".openclaw", "openclaw.json");
const BRIDGE = path.join(IRIS_DIR, "gateway-bridge.mjs");
const RESOLVE_NODE_BIN = path.join(IRIS_DIR, "scripts", "resolve-node-bin.sh");
...
const ALLOW_PGREP_FALLBACK = process.env.IRIS_DISABLE_PGREP_FALLBACK !== "1";
...
function loadConfig() {
  ...
  let rtToken = sys?.rt?.authToken || sys?.env?.IRIS_RT_AUTH_TOKEN
    || raw?.rt?.authToken || raw?.env?.IRIS_RT_AUTH_TOKEN || "";

  // Legacy fallback
  if (!rtToken) {
    try {
      const legacy = JSON.parse(fs.readFileSync(OPENCLAW_CFG, "utf8"));
      rtToken = legacy?.rt?.authToken || legacy?.env?.IRIS_RT_AUTH_TOKEN || "";
    } catch {}
  }
...
if (args.includes("--stop") || forceRestart) {
  ...
  // Also stop the iris-owned opencode serve (port 4097) if running.
...
if (!validation.valid) {
  console.error(`\n❌ FATAL: Required agents missing from ~/.iris/iris.json:`);
  for (const missing of validation.missing) {
    console.error(`  - ${missing}`);
  }
  console.error(`\nThese agents are essential for iris to function.`);
  console.error(`Fix: Run the installer to populate the config:\n`);
  console.error(`  bash install.sh\n`);
  console.error(`Or add the missing agents manually to ~/.iris/iris.json\n`);
  process.exit(1);
}
...
if (actualRunning > already.size && actualRunning > allRtIds.size) {
  ...
  console.error(`\n   This usually means you ran start-iris.mjs multiple times without cleanup.`);
  console.error(`   Run this to fix:\n`);
  console.error(`   node scripts/start-iris.mjs --stop && node scripts/start-iris.mjs\n`);
...
if (singleAgentId) {
  ...
  if (!allRtIds.has(rtId)) {
    console.error(`Agent "${singleAgentId}" (rt: ${rtId}) not found in iris.json`);
    process.exit(1);
  }
  if (already.has(rtId)) {
...
const MAX_BRIDGES = parseInt(process.env.IRIS_MAX_BRIDGES || "30", 10);
...
if (totalAfterStart > MAX_BRIDGES) {
  ...
  console.warn(`⚠️  Bridge cap: ${already.size} running + ${toStart.length} requested exceeds max ${MAX_BRIDGES}. Launching ${cap}. Set IRIS_MAX_BRIDGES to raise limit.`);
  toStart = toStart.slice(0, cap);
  if (toStart.length === 0) {
    console.warn(`⚠️  No new bridges started — cap reached. Stop some agents first or raise IRIS_MAX_BRIDGES.`);
    process.exit(0);
  }
}

console.log(`Starting iris bridges…`);
console.log(`  Already running : ${already.size} (${[...already].join(", ") || "none"})`);
console.log(`  Launching new   : ${toStart.length} (${toStart.join(", ")})`);
console.log();

// iris-lead runs its own script, not gateway-bridge
const IRIS_LEAD_SCRIPT = path.join(IRIS_DIR, "iris-lead.mjs");
const crewLeadRunning = (() => {
  if (!ALLOW_PGREP_FALLBACK) return false;
  try {
    execSync("pgrep -f 'iris-lead.mjs'", { encoding: "utf8", timeout: 2000, stdio: "pipe" });
    return true;
  } catch { return false; }
})();

// Skip iris-lead if SKIP_CREW_LEAD env var is set (dashboard manages iris-lead separately)
const shouldSkipCrewLead = process.env.SKIP_CREW_LEAD === "1";

if (toStart.includes("iris-lead") && !crewLeadRunning && !shouldSkipCrewLead && fs.existsSync(IRIS_LEAD_SCRIPT)) {
  const logFile = path.join(os.tmpdir(), "bridge-iris-lead.log");
  const logFd = fs.openSync(logFile, "a");
  const proc = spawn(NODE_BIN, [IRIS_LEAD_SCRIPT], {
    cwd: IRIS_DIR,
    stdio: ["ignore", logFd, logFd],
    detached: true,
    env: { ...process.env, ...(raw.env || {}), IRIS_RT_AUTH_TOKEN: rtToken },
  });
  proc.unref();
  console.log(`  ✓ Spawned iris-lead (pid ${proc.pid})`);
}

for (const rtId of toStart.filter(id => id !== "iris-lead")) {
  const env = {
    ...process.env,
    ...(raw.env || {}),  // ← ADD env vars from config
    IRIS_RT_AGENT: rtId,
    IRIS_RT_AUTH_TOKEN: rtToken,
    IRIS_RT_CHANNELS: process.env.IRIS_RT_CHANNELS || "command,assign,handoff,reassign,events",
    IRIS_OPENCODE_MODEL: process.env.IRIS_OPENCODE_MODEL || "groq/moonshotai/kimi-k2-instruct-0905",
    IRIS_OPENCODE_PROJECT: process.env.IRIS_OPENCODE_PROJECT || IRIS_DIR,
    IRIS_DIR,
  };
...
// iris-scribe — memory maintenance daemon (watches done.jsonl, writes brain.md + session-log.md)
const SCRIBE_SCRIPT = path.join(IRIS_DIR, "scripts", "iris-scribe.mjs");
const scribeRunning = (() => {
  try { execSync("pgrep -f 'iris-scribe.mjs'", { encoding: "utf8", timeout: 2000, stdio: "pipe" }); return true; } catch { return false; }
})();
if (!scribeRunning && fs.existsSync(SCRIBE_SCRIPT)) {
  const logFd = fs.openSync(path.join(os.tmpdir(), "iris-scribe.log"), "a");
  const proc = spawn(NODE_BIN, [SCRIBE_SCRIPT], {
    cwd: IRIS_DIR, stdio: ["ignore", logFd, logFd], detached: true,
    env: { ...process.env, IRIS_RT_AUTH_TOKEN: rtToken },
  });
  proc.unref();
  console.log(`  ✓ Spawned iris-scribe (pid ${proc.pid})`);
}

// mcp-server — MCP endpoint for Cursor / Claude Code / OpenCode
const MCP_SCRIPT = path.join(IRIS_DIR, "scripts", "mcp-server.mjs");
const mcpRunning = (() => {
...
if (!mcpRunning && fs.existsSync(MCP_SCRIPT)) {
  const logFd = fs.openSync(path.join(os.tmpdir(), "iris-mcp.log"), "a");
...
console.log(`\n✓ Iris started — ${allRtIds.size} agents online`);
console.log(`  Run 'node scripts/start-iris.mjs --status' to verify`);
