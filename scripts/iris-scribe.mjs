#!/usr/bin/env node
/**
 * iris-scribe — Memory maintenance daemon
 *
 ...
 *
 * Usage:
 *   node scripts/iris-scribe.mjs
 *   node scripts/iris-scribe.mjs --once   (process backlog and exit)
 */
...
const RT_BASE       = process.env.SHARED_MEMORY_DIR
  ? path.join(process.env.SHARED_MEMORY_DIR, "claw-swarm", "openiris-rt")
  : path.join(os.homedir(), ".openclaw", "workspace", "shared-memory", "claw-swarm", "openiris-rt"); // runtime socket path set by RT daemon
const DONE_LOG      = path.join(RT_BASE, "channels", "done.jsonl");
const STATE_FILE    = path.join(os.homedir(), ".iris", "scribe-state.json");
const POLL_MS       = 4000;
...
function loadLLMConfig() {
  try {
    const csPath = path.join(os.homedir(), ".iris", "iris.json");
...
function extractBrainEntries(agentId, reply) {
  ...
  for (const m of matches) {
    ...
    if (isBrainDuplicate(fact)) {
      console.log(`[iris-scribe] Skipping duplicate brain entry: ${fact.slice(0, 60)}`);
      continue;
    }
...
function extractLessonEntries(agentId, reply) {
  ...
  for (const m of matches) {
    ...
    if (isLessonDuplicate(lesson)) {
      console.log(`[iris-scribe] Skipping duplicate lesson: ${lesson.slice(0, 60)}`);
      continue;
    }
...
const _SKIP_AGENTS_STATIC = new Set(["iris-lead", "orchestrator"]);
...
async function processNewEntries(state) {
  ...
  if (sessionEntries.length > 0) {
    ...
    console.log(`[iris-scribe] Wrote ${sessionEntries.length} session-log entries`);
  }

  if (brainEntries.length > 0) {
    appendToFile(BRAIN_MD, brainEntries.join("\n") + "\n");
    console.log(`[iris-scribe] Wrote ${brainEntries.length} brain.md entries (deduped)`);
  }

  if (lessonEntries.length > 0) {
    // Bootstrap lessons.md header on first write
    if (!fs.existsSync(LESSONS_MD)) {
      appendToFile(LESSONS_MD, "# Iris Lessons\n\nMistake patterns captured automatically. iris-fixer reads this before every fix.\n");
    }
    appendToFile(LESSONS_MD, lessonEntries.join("\n") + "\n");
    console.log(`[iris-scribe] Wrote ${lessonEntries.length} lessons.md entries (deduped)`);
  }
...
console.log("[iris-scribe] Starting — watching", DONE_LOG);
console.log("[iris-scribe] Memory dir:", MEMORY_DIR);
...
if (ONCE_MODE) {
  ...
  saveState(state);
  console.log("[iris-scribe] --once done");
  process.exit(0);
}
...
async function tick() {
  try {
    ...
    console.error("[iris-scribe] Error:", err.message);
  }
