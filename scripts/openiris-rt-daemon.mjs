 ...
 * Source: ~/swarm/.opencode/plugin-test/openiris-rt.ts (server + protocol only).
 */
...
const MAX_CLIENTS = Number(process.env.IRIS_RT_MAX_CLIENTS || "200");
...
const LEGACY_COMPAT_ALLOWED_AGENTS = new Set([
  ...
  "opencode-coder", "opencode-coder-2", "security", "iris-coder-2", "iris-lead",
]);
...
function isAgentAllowed(agentId) {
  ...
  const cfgPath = join(process.env.HOME || "", ".iris", "iris.json");
  try {
...
const ENV_OVERRIDE_AGENTS = process.env.OPENCLAW_ALLOWED_AGENTS
  ...
  : null;
const API_KEY = (process.env.IRIS_API_KEY || process.env.OPENCLAW_API_KEY || "").trim();
const REQUIRE_API_KEY = (process.env.OPENCLAW_REQUIRE_API_KEY || "1") !== "0";

const MEMORY_BASE_DIR = process.env.SHARED_MEMORY_DIR || join(process.env.HOME || process.env.USERPROFILE || "", ".iris", "workspace", "shared-memory");
const MEMORY_NAMESPACE = process.env.SHARED_MEMORY_NAMESPACE || "claw-swarm";
const PROTOCOL_DIR = join(MEMORY_BASE_DIR, MEMORY_NAMESPACE, "openiris-rt");
...
const MAX_MESSAGE_BYTES = Number(process.env.IRIS_RT_MAX_MESSAGE_BYTES || "65536");
const RATE_LIMIT_PER_MIN = Number(process.env.IRIS_RT_RATE_LIMIT_PER_MIN || "300");
const REQUIRE_AGENT_TOKEN = (process.env.IRIS_RT_REQUIRE_AGENT_TOKEN || "0") === "1";
const ALLOWED_ORIGINS = (process.env.IRIS_RT_ALLOWED_ORIGINS || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const AGENT_TOKENS = new Map(
  (process.env.IRIS_RT_AGENT_TOKENS || "")
    .split(";")
    .map((s) => s.trim())
    .filter(Boolean)
    .map((pair) => {
...
function failUnauthorized(reason) {
  throw new Error(`[openiris-rt] Unauthorized: ${reason}`);
}
...
function checkPermissions(context, providedKey) {
  ...
  if (REQUIRE_API_KEY && !API_KEY) {
    failUnauthorized("API key is required (set OPENCLAW_API_KEY or IRIS_API_KEY)");
  }
  if (REQUIRE_API_KEY) {
...
function sanitizeChannel(channel) {
  ...
  if (!/^[a-z0-9._-]{1,40}$/.test(value)) {
    throw new Error("[openiris-rt] Invalid channel. Use 1-40 chars: a-z 0-9 . _ -");
  }
  return value;
}
...
function sanitizeType(type) {
  ...
  if (!/^[a-z0-9._-]{1,64}$/.test(value)) {
    throw new Error("[openiris-rt] Invalid message type. Use 1-64 chars: a-z 0-9 . _ -");
  }
  return value;
}
...
function sanitizeTarget(to) {
  ...
  if (!/^[a-zA-Z0-9._:-]{1,80}$/.test(value)) {
    throw new Error("[openiris-rt] Invalid target. Use 1-80 chars: letters, numbers, . _ : -");
  }
  return value;
}
...
function normalizePriority(priority) {
  ...
  throw new Error("[openiris-rt] Invalid priority. Use: low, medium, high, critical");
}
...
function validateEnvelopeSemantics(envelope) {
  if (envelope.channel === "command" && !COMMAND_TYPES.has(envelope.type)) {
    throw new Error(`[openiris-rt] Invalid command type: ${envelope.type}`);
  }
}
...
function setupConnectionHandlers() {
  ...
  state.wss.on("connection", (socket, req) => {
    ...
    if (state.clients.size >= MAX_CLIENTS) {
      ...
      socket.close();
      console.warn(`[openiris-rt-daemon] MAX_CLIENTS (${MAX_CLIENTS}) reached — rejected new connection`);
      return;
    }
          ...
          if (claimedAgent !== "anonymous") {
            for (const [otherSocket, otherMeta] of state.clients) {
              if (otherSocket !== socket && otherMeta.agent === claimedAgent) {
                ...
                console.log(`[openiris-rt-daemon] Evicted stale ${claimedAgent} connection (replaced)`);
              }
            }
          }
...
export async function startServer(config) {
  ...
  const lockResult = acquireStartupLock("openiris-rt-daemon", { port: config.port, killStale: true });
  if (!lockResult.ok) {
    console.error(`[openiris-rt] ${lockResult.message}`);
    process.exit(1);
  }

  if (state.server || state.wss) {
    throw new Error("[openiris-rt] Server already running");
  }
  await ensureProtocolDirs();
  ...
  if (config.secure) {
    if (!config.tlsKeyPath || !config.tlsCertPath) {
      throw new Error("[openiris-rt] TLS enabled but key/cert paths are missing");
    }
...
const isMain = process.argv[1] && (process.argv[1] === scriptPath || process.argv[1].endsWith("openiris-rt-daemon.mjs"));

if (isMain) {
  const host = process.env.IRIS_RT_HOST || "127.0.0.1";
  const port = Number(process.env.IRIS_RT_PORT || "18889");
  const requireTokenEnv = process.env.IRIS_RT_REQUIRE_TOKEN;
  // Token: env first, then config JSON (dashboard saves to ~/.iris or ~/.openclaw)
  let token = process.env.IRIS_RT_AUTH_TOKEN || "";
  if (!token) {
    const home = process.env.HOME || process.env.USERPROFILE || "";
    for (const p of [join(home, ".iris", "iris.json"), join(home, ".openclaw", "openclaw.json")]) {
      try {
  ...
  // No token anywhere: auth is optional (local use). Set IRIS_RT_REQUIRE_TOKEN=1 to require a token.
  const requireToken = requireTokenEnv === "1" || (requireTokenEnv !== "0" && token !== "");

  if (requireToken && !token) {
    console.error("IRIS_RT_AUTH_TOKEN is required when IRIS_RT_REQUIRE_TOKEN=1. Set it in ~/.iris/iris.json (rt.authToken) or ~/.openclaw/openclaw.json (env.IRIS_RT_AUTH_TOKEN).");
    process.exit(1);
  }
