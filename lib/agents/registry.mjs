...
export function buildAgentMapsFromConfig() {
  ...
  if (process.env.IRIS_RT_SWARM_AGENTS) {
    const list = process.env.IRIS_RT_SWARM_AGENTS.split(",").map(s => s.trim()).filter(Boolean);
    const map = {};
    for (const a of list) map[a] = BUILT_IN_MAP[a] || a.replace(/^iris-/, "");
    return { list, map };
  }
...
const { list: IRIS_RT_SWARM_AGENTS, map: RT_TO_GATEWAY_AGENT_MAP } = buildAgentMapsFromConfig();
export { IRIS_RT_SWARM_AGENTS, RT_TO_GATEWAY_AGENT_MAP };
