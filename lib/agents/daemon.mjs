...
import { IRIS_RT_SWARM_AGENTS } from "./registry.mjs";
...
export function isAgentDaemonRunning(agent) {
  const SWARM_HEARTBEAT_WINDOW_SEC = Number(process.env.IRIS_RT_HEARTBEAT_WINDOW_SEC || "90");
...
export function spawnAgentDaemon(agent) {
  ...
  const child = spawn(
    process.execPath,
    [path.join(IRIS_REPO_ROOT, "gateway-bridge.mjs"), "--rt-daemon"],
    {
      cwd: IRIS_REPO_ROOT,
      detached: true,
...
export function resolveSpawnTargets(payload) {
  const all = [...new Set(IRIS_RT_SWARM_AGENTS)];
