 ...
 * @param {string} options.agentId - Agent identifier (e.g., "iris-pm", "iris-coder")
 ...
 * @param {Object} options.agentConfig - Full agent config from iris.json (optional, for engine preference)
...
export function buildToolInstructions({
  ...
  const isPMAgent = agentId.includes("iris-pm");
  const isCoderAgent = [
    "iris-coder",
    "iris-coder-front",
    "iris-coder-back",
...
- Simple, isolated operations that DON'T modify existing code`;
  ...
  if (permissions.cli && hasEngine) {
    ...
    if (preferredCLI) {
      ...
      const cliLabels = {
        "iris-cli": "TypeScript specialist (iris-cli)",
...
**Your preferred CLI is ${preferredCLI}.** Use this for all CLI operations.`;
      ...
      // Show all CLIs if no preference (iris-cli first as recommended default)
      instructions += `\n\n### CLI Tools (powerful, use for complex operations)
- **@@CLI iris-cli** <task> — TypeScript specialist (recommended for most coding tasks)
...
**Use when:** Task requires expertise outside your specialty (e.g., PM delegates coding to iris-coder)`;
  }
 ...
 * @param {Object} agentConfig - Agent config from iris.json
 * @returns {string|null} - "iris-cli" | "opencode" | "cursor" | "claude" | "codex" | "gemini" | null
 */
export function getPreferredCLI(agentConfig) {
  if (!agentConfig) return null;

  // Priority order based on explicit flags
  if (agentConfig.useCrewCLI || agentConfig.engine === "iris-cli")
    return "iris-cli";
...
export function getToolPermissions(agentId, agentConfig) {
  ...
  const irisAllow = agentConfig?.tools?.irisAllow || [];

  // Default permissions by agent type
  const defaults = {
    // PM agents: planning tools
    "iris-pm": { cli: true, dispatch: true, web: true },
    "iris-pm-cli": { cli: true, dispatch: true, web: true },
    "iris-pm-frontend": { cli: true, dispatch: true, web: true },
    "iris-pm-core": { cli: true, dispatch: true, web: true },

    // Coding agents: file + CLI tools
    "iris-coder": { cli: true, dispatch: true, web: true },
    "iris-coder-front": { cli: true, dispatch: true, web: true },
    "iris-coder-back": { cli: true, dispatch: true, web: true },
    "iris-fixer": { cli: true, dispatch: true, web: true },
    "iris-frontend": { cli: true, dispatch: true, web: true },

    // Review agents: read-only + web
    "iris-qa": { cli: true, dispatch: true, web: true },
    "iris-security": { cli: false, dispatch: false, web: true },

    // Writing agents: files + web
    "iris-copywriter": { cli: true, dispatch: false, web: true },
    "iris-researcher": { cli: false, dispatch: false, web: true },

    // Chat-only agents: web only
    "iris-loco": { cli: false, dispatch: false, web: true },

    // Coordinators: full access
    "iris-main": { cli: true, dispatch: true, web: true },
    "iris-lead": { cli: true, dispatch: true, web: true },
  };
  ...
  if (irisAllow.length > 0) {
    return {
      cli:
        irisAllow.includes("run_cmd") ||
        irisAllow.includes("write_file"),
      dispatch: irisAllow.includes("dispatch"),
      web:
        irisAllow.includes("web_search") ||
        irisAllow.includes("web_fetch"),
    };
  }
