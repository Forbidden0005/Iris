...
export function isDoneRecordFresh(record) {
  ...
  return (Date.now() - doneAtMs) <= IRIS_RT_TASK_STATE_TTL_MS;
}
...
export function acquireTaskLease({ key, source, incomingType, from, leaseMs }) {
  ...
  const now = Date.now();
  const claimId = `${IRIS_RT_AGENT}-${process.pid}-${crypto.randomUUID()}`;
  const leaseRecord = {
    key,
    claimId,
    agent: IRIS_RT_AGENT,
...
export function renewTaskLease({ key, claimId, leaseMs }) {
  ...
  if (!current || current.claimId !== claimId || current.agent !== IRIS_RT_AGENT) return false;
  const now = Date.now();
...
export function shouldUseDispatchGuard(incomingType) {
  if (!IRIS_RT_DISPATCH_ENABLED) return false;
