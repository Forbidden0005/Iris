/**
 * Pipeline wave dispatcher — extracted from iris-lead.mjs
 * Handles: pending dispatches, pipelines, timeouts, quality gates, dispatchTask
 * Supports hermetic testing via IRIS_TEST_MODE env var.
 */
...
import { linkIrisPlanTaskToDispatch } from "../iris/plans.mjs";
...
const getQueueLimit = () =>
  _deps.dispatchQueueLimit ?? parseInt(process.env.IRIS_DISPATCH_QUEUE_LIMIT || "50", 10);
...
const _PLANNING_AGENTS_STATIC = new Set(["iris-pm", "iris-copywriter"]);
const _BUILD_AGENTS_STATIC = new Set(["iris-coder", "iris-coder-front", "iris-coder-back", "iris-frontend"]);
...
function advancePipelineOnQualityGateFailure() {
  return /^1|true|yes$/i.test(String(process.env.IRIS_PIPELINE_ADVANCE_ON_QUALITY_FAIL || ""));
}
...
export function failPipelineOnQualityGate(pipelineId, issues = []) {
  ...
  console.error(`[iris-lead] Pipeline ${pipelineId} HALTED (quality gate): ${summary}`);
  _deps.broadcastSSE?.({
  ...
  _deps.appendHistory?.(
    sessionId,
    "system",
    `Pipeline stopped — quality gate still failing after retries: ${summary}. Fix the underlying issue or set IRIS_PIPELINE_ADVANCE_ON_QUALITY_FAIL=1 to advance anyway (legacy, not recommended).`,
  );
...
export function checkDispatchTimeouts() {
  ...
  for (const [taskId, d] of pendingDispatches.entries()) {
      ...
      if (!d._autoExtended) {
        ...
        _deps.isAgentOnRtBus?.(agent).then(online => {
          if (online && !d.done) {
            ...
            console.log(`[iris-lead] ⚡ Auto-extending timeout for ${agent} (online on RT bus but slow to claim) — ${dispatchClaimedTimeoutMs / 1000}s`);
    ...
    const msg = d.claimed
      ? `[iris-lead] Task to ${agent} timed out after ${Math.round(limit / 1000)}s (agent claimed it but never finished). Consider @@SERVICE restart ${agent}.`
      : `[iris-lead] Task to ${agent} never claimed (no response within ${Math.round(limit / 1000)}s). Agent may be offline — try @@SERVICE restart ${agent} or re-dispatch to another agent.`;
    ...
    console.log(`[iris-lead] ${kind} taskId=${taskId} agent=${agent} elapsed=${Math.round(elapsed / 1000)}s`);
...
export function markDispatchClaimed(taskId, agent) {
  ...
  if (!d.claimed) {
    ...
    d.claimedAt = Date.now();
    const dispatchClaimedTimeoutMs = _deps.dispatchClaimedTimeoutMs ?? 900_000;
    console.log(`[iris-lead] ⚡ ${agent || d.agent} claimed task ${taskId} — extending timeout to ${dispatchClaimedTimeoutMs / 1000}s`);
...
export function cancelAllPipelines(sessionId) {
  ...
  for (const [pid, pipeline] of pendingPipelines) {
    ...
    console.log(`[iris-lead] Cancelling pipeline ${pid} (${waveInfo}, ${pipeline.pendingTaskIds.size} pending tasks)`);
...
export function dispatchPipelineWave(pipelineId) {
  ...
  if (pipeline.projectDir) {
    ...
    if (n && n !== pipeline.projectDir) {
      console.warn(
        `[iris-lead] normalized pipeline.projectDir: ${pipeline.projectDir} → ${n}`,
      );
  ...
  if (currentWave >= waves.length) {
    ...
    console.log(`[iris-lead] Pipeline ${pipelineId} complete`);
  ...
  const projectRootBanner = pipeline.projectDir
    ? `\n\n[Pipeline project root — REQUIRED]\nAll new artifacts from this step MUST be written under:\n  ${pipeline.projectDir}\nUse the exact output paths from your task (or under this directory). Do NOT write to the iris repo root unless the task explicitly names a path there.\n`
    : "";
  ...
  console.log(`[iris-lead] Pipeline ${pipelineId} wave ${currentWave + 1}/${waves.length} — dispatching ${waveSteps.length} agent(s) in parallel: ${resolvedAgentNames.join(", ")}`);
  ...
  // iris-orchestrator Cursor subagent which fans all tasks out in parallel.
  ...
  const orchestratorHasCliEngine = (() => {
    try {
      ...
      const orch = agents.find(a => a.id === "iris-orchestrator");
  ...
  if (_deps._cursorWavesEnabled && orchestratorHasCliEngine && waveSteps.length > 1 && !pipeline.projectDir) {
    const waveManifest = {
      ...
      tasks: waveSteps.map(step => {
        ...
        const isQa = step.agent === "iris-qa" || (step.agent && step.agent.includes("qa"));
    ...
    const orchestratorTask = [
      `[iris-orchestrator] Execute this wave — dispatch ALL tasks to subagents in parallel:`,
      "```json",
    ...
    console.log(`[iris-lead] WAVE_DISPATCH: routing wave ${currentWave + 1} through iris-orchestrator (${waveSteps.length} parallel tasks)`);
    const taskId = dispatchTask("iris-orchestrator", { task: orchestratorTask }, sessionId, {
      pipelineId,
  ...
  // When IRIS_WORKTREE_ISOLATION is not "false" (default: enabled for
  ...
  const worktreeEnabled = (() => {
    const envVal = process.env.IRIS_WORKTREE_ISOLATION;
...
function isPlanningAgent(agentId) {
  ...
  const swarm = _deps.tryRead?.(getConfigPath("iris.json"));
...
export function checkWaveQualityGate(pipeline, pipelineId) {
  ...
  // When _cursorWavesEnabled routed the wave through iris-orchestrator, we get
  ...
  if (isCursorWaveResult) {
    // Parse per-agent sections from the combined report: "[iris-X]: ..."
  ...
  for (let i = 0; i < effectiveResults.length; i++) {
    ...
    if (agent === "iris-pm" && nextWaveHasBuilders) {
      const hasPDD = /PDD\.md|product.design.document/i.test(result);
      const hasRoadmap = /ROADMAP\.md/i.test(result);
      if (!hasPDD && !hasRoadmap) {
        issues.push(`iris-pm did not produce PDD.md or ROADMAP.md — build agents need these before starting`);
      }
    }
  ...
  for (let i = 0; i < effectiveResults.length; i++) {
    ...
    const step = effectiveSteps[i] || {};
    const agent = step.agent || "unknown";
    const isQaAgent = agent === "iris-qa" || agent.includes("qa");
    ...
    if ((hasFailVerdict || criticalCount >= 2) && pipeline[qaFixRetryKey] < MAX_QA_FIX_LOOPS) {
      ...
      console.log(`[iris-lead] Pipeline ${pipelineId} QA FAIL detected — auto-dispatching iris-fixer (loop ${pipeline[qaFixRetryKey]}/${MAX_QA_FIX_LOOPS})`);

      const fixerTask = `Fix all CRITICAL and HIGH issues found by iris-qa. QA report:\n\n${result.slice(0, 4000)}\n\nRead each failing file, patch in place (same path, no _fixed variants), run python3 -m py_compile on each fixed file to confirm PASS.`;
      ...
      waves.splice(currentWave + 1, 0,
        [{ agent: "iris-fixer", task: fixerTask }],
        qaWaveClone
      );
      ...
      _deps.appendHistory?.(sessionId, "system", `QA FAIL detected — auto-dispatching iris-fixer (loop ${pipeline[qaFixRetryKey]}/${MAX_QA_FIX_LOOPS}), then re-running QA.`);
  ...
  if (issues.length === 0) {
    console.log(`[iris-lead] Pipeline ${pipelineId} wave ${currentWave + 1} quality gate: PASS`);
    return { pass: true };
  }

  console.log(`[iris-lead] Pipeline ${pipelineId} wave ${currentWave + 1} quality gate: ${issues.length} issue(s)`);
  ...
  _deps.appendHistory?.(
    ...
    `Pipeline wave ${currentWave + 1} quality gate flagged ${issues.length} issue(s): ${issues.join("; ")}. ${
      willRetry ? "Retrying wave." : advanceAnyway ? "Advancing anyway (IRIS_PIPELINE_ADVANCE_ON_QUALITY_FAIL)." : "Stopping pipeline after failed retries."
    }`,
  );
  ...
  if (pipeline[retryKey] < MAX_WAVE_RETRIES) {
    ...
    console.log(`[iris-lead] Retrying wave ${currentWave + 1} with quality feedback (jitter=${jitterMs}ms)`);
  ...
  if (advancePipelineOnQualityGateFailure()) {
    console.warn(`[iris-lead] Max retries reached for wave ${currentWave + 1} — advancing with issues (IRIS_PIPELINE_ADVANCE_ON_QUALITY_FAIL)`);
    return { pass: true };
  }
...
export function dispatchTask(agent, task, sessionId = "owner", pipelineMeta = null) {
  ...
  if (activePending >= queueLimit) {
    console.warn(`[iris-lead] Dispatch queue full (${activePending}/${queueLimit}) — rejecting task for ${agent}. Raise IRIS_DISPATCH_QUEUE_LIMIT to increase.`);
  ...
  const isBriefAgent = agent === "iris-qa" || agent === "iris-fixer" || agent.includes("qa");
  ...
  if (rp) {
    try {
      ...
      // ✅ FIX: Enrich payload with agent's engine configuration from iris.json
      ...
      if (taskId) {
        ...
        // Iris plan bridge — additive, best-effort only, no-op unless a
        // caller already put irisPlanId/irisTaskId on pipelineMeta. Set by
        // the plans-tab "Dispatch task" action (http-server.mjs's
        // /api/dispatch route) and carried forward on ws-router.mjs's
        // auto-retry redispatches; every other dispatch path is
        // unaffected.
        if (pipelineMeta?.irisPlanId && pipelineMeta?.irisTaskId) {
          try {
            linkIrisPlanTaskToDispatch(pipelineMeta.irisPlanId, pipelineMeta.irisTaskId, taskId);
          } catch (e) {
            console.warn(`[wave-dispatcher] Iris plan-task link failed: ${e.message}`);
          }
        }
      }
      console.log(`[iris-lead] dispatched via RT to ${agent} (taskId=${taskId} correlationId=${correlationId}): ${task.slice(0, 60)}`);
      ...
      console.error(`[iris-lead] RT dispatch failed: ${e.message}`);
    }
  }
  // Fallback: openswitchctl (no reply routing back to iris-lead)
  console.log("[iris-lead] RT not connected — using openswitchctl send (replies won't appear in chat; check RT Messages tab)");
  try {
    const safeTask = task.replace(/"/g, '\\"').replace(/\n/g, " ");
    _deps.execSync?.(`"${_deps.CTL_PATH}" send "${agent}" "${safeTask}"`, { encoding: "utf8", timeout: 10000 });
    console.log(`[iris-lead] dispatched via ctl to ${agent}: ${task.slice(0, 60)}`);
    return true;
  } catch (e) {
    console.error(`[iris-lead] dispatch failed: ${e.message}`);
    return false;
  }
}
