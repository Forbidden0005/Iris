/**
 * Brain, global rules, and search helpers — extracted from iris-lead.mjs
 */
...
export function appendToBrain(agentId, entry, projectDir = null) {
  ...
  const block = `\n## [${date}] ${agentId}: ${entry}\n`;
  if (projectDir) {
    const projectBrainDir = path.join(projectDir, ".iris");
...
export async function searchWithBrave(query) {
  ...
  try {
    ...
    console.log(`[iris-lead] Brave search query="${query.slice(0, 80)}" → ${results.length} results`);
    return text;
  } catch { return null; }
}

export function getWorkspaceRoot() {
  return process.env.IRIS_LEAD_WORKSPACE || process.cwd();
}
