/**
 * iris-lead direct tools executor — extracted from iris-lead.mjs
...
export function initTools({ historyDir, irisCfgFile }) {
  if (historyDir !== undefined) _historyDir = historyDir;
  if (irisCfgFile !== undefined) _irisCfgFile = irisCfgFile;
}

export const CREWLEAD_BLOCKED_CMDS = /rm\s+-rf\s+\/(?!\S)|mkfs|dd\s+if=|:(){ :|:& };:|shutdown|reboot|halt|pkill\s+-9\s+iris-lead/i;
...
export async function execCrewLeadTools(reply) {
  ...
  while ((m = readRe.exec(reply)) !== null) {
    ...
    try {
      ...
      console.log(`[iris-lead:read_file] ${filePath}`);
    } catch (e) {
      const alt =
        path.basename(filePath) === "iris-lead.mjs" &&
        e?.code === "ENOENT" &&
        path.resolve(filePath) !== path.resolve(path.join(IRIS_REPO_ROOT, "iris-lead.mjs"))
          ? path.join(IRIS_REPO_ROOT, "iris-lead.mjs")
          : null;
      if (alt) {
        try {
          ...
          console.log(`[iris-lead:read_file] ${alt} (fallback from ${filePath})`);
        } catch (e2) {
  ...
  while ((m = writeRe.exec(reply)) !== null) {
    ...
    try {
      ...
      console.log(`[iris-lead:write_file] ${filePath}`);
  ...
  while ((m = mkdirRe.exec(reply)) !== null) {
    ...
    try {
      ...
      console.log(`[iris-lead:mkdir] ${dirPath}`);
  ...
  while ((m = cmdRe.exec(reply)) !== null) {
    ...
    try {
      ...
      console.log(`[iris-lead:run_cmd] ${cmd}`);
    } catch (e) {
  ...
  while ((m = searchRe.exec(reply)) !== null) {
    ...
    try {
      ...
      let braveKey = (() => {
        try {
          const cfg = JSON.parse(fs.readFileSync(_irisCfgFile, "utf8"));
      ...
      if (!braveKey) {
        try {
          const cfgDir = _irisCfgFile ? path.dirname(_irisCfgFile) : path.join(os.homedir(), ".iris");
      ...
      console.log(`[iris-lead:web_search] "${query}" → ${results.length} results`);
  ...
  while ((m = fetchRe.exec(reply)) !== null) {
    const url = m[1].trim();
    try {
      const res = await fetch(url, { headers: { "User-Agent": "iris/1.0" }, signal: AbortSignal.timeout(15000) });
      ...
      console.log(`[iris-lead:web_fetch] ${url}`);
  ...
  while ((m = searchHistRe.exec(reply)) !== null) {
    ...
    try {
      ...
      console.log(`[iris-lead:search_history] query="${query}" hits=${hits.length}`);
  ...
  while ((m = telegramRe.exec(reply)) !== null) {
    ...
    try {
      const tgBridge = (() => {
        try { return JSON.parse(fs.readFileSync(path.join(os.homedir(), ".iris", "telegram-bridge.json"), "utf8")); }
        catch { return {}; }
      })();
      ...
      console.log(`[iris-lead:telegram] sent to ${chatId}`);
  ...
  while ((m = whatsappRe.exec(reply)) !== null) {
    ...
    try {
      const waBridge = (() => {
        try { return JSON.parse(fs.readFileSync(path.join(os.homedir(), ".iris", "whatsapp-bridge.json"), "utf8")); }
        catch { return {}; }
      })();
      ...
      console.log(`[iris-lead:whatsapp] sent to ${phone}`);
