/**
 * LLM caller for iris-lead — extracted from iris-lead.mjs
...
const TOKEN_USAGE_FILE = path.join(os.homedir(), ".iris", "token-usage.json");
...
/**
 * Call Gemini CLI for iris-lead chat — zero-key path (uses Google OAuth).
...
async function callGeminiCliForChat(messages) {
  ...
  return new Promise((resolve, reject) => {
    ...
    function handleLine(line) {
      ...
      try {
        ...
        if (ev.type === "result" && !resultReceived) {
          ...
          console.log(`[iris-lead] Gemini CLI reply — ${out.length} chars`);
          resolve(out);
        }
        if (ev.type === "error") {
          console.error(`[iris-lead] Gemini CLI error:`, ev.message || JSON.stringify(ev));
        }
      } catch {}
    }
    ...
    child.stderr.on("data", (chunk) => {
      console.error(`[iris-lead] Gemini CLI stderr:`, chunk.toString().slice(0, 200));
    });
...
export async function _callLLMOnce(baseUrl, apiKey, modelId, providerKey, messages, options = {}) {
  ...
  if (isAnthropic) {
    ...
    body = {
      ...
      temperature: 0.7,
      stream: enableStreaming,
      // Disable Gemini 2.5 thinking — iris-lead is a conversational router, not a reasoner.
  ...
  if (!res.ok) {
    ...
    const url = endpoint;
    console.error(`[iris-lead] LLM ${res.status} @ ${url} model=${modelId}: ${err.slice(0, 400)}`);
  ...
  if (enableStreaming) {
    ...
    console.log(
      `[iris-lead] stream complete ${providerKey}/${modelId} — ${fullReply.length} chars in assembled reply`,
    );
...
function _recordCrewLeadTokens(modelId, providerKey, usage) {
  ...
  if (cached > 0) {
    ...
    console.log(`[iris-lead] cache hit: ${cached}/${p} tokens cached (${pct}%) — ${providerKey}/${modelId}`);
  }
  ...
  if (cached > 0 && cached / p > 0.5) {
    ...
    console.log(`[iris-lead] cache saved $${savings.toFixed(4)} (would cost $${fullCost.toFixed(4)} without cache)`);
  }
  
  // Record to spending.json for dashboard
  const SPENDING_FILE = path.join(os.homedir(), ".iris", "spending.json");
  try {
    const spendingData = (() => {
    ...
    if (!spendingData.agents["iris-lead"]) spendingData.agents["iris-lead"] = { tokens: 0, costUSD: 0 };
    spendingData.agents["iris-lead"].tokens  += total;
    spendingData.agents["iris-lead"].costUSD += costUSD;
...
/** Inject active-model note when iris-lead is running on fallback. PREPEND so model sees it first.
...
export function patchMessagesWithActiveModel(messages, activeModel, primaryModel, reason) {
  ...
  const note = `CRITICAL — you are on FALLBACK: ${activeModel} (primary ${primaryModel} failed: ${reason}). When asked what model you use, say ${activeModel}. Never say codex or openai-local — that's iris-main.\n\n`;
  ...
  for (let i = 0; i < out.length; i++) {
    if (out[i].role === "user" && out[i].content.includes("iris-lead:")) {
      out[i] = {
        ...out[i],
        content: out[i].content.replace(/(iris-lead:)\s*[\w-]+\/[\w.-]+(\s*\|)/g, `$1 ${activeModel} (fallback — primary failed)$2`),
      };
    }
  }
  return out;
...
export async function callLLM(messages, cfg, options = {}) {
  ...
  if ((useGeminiCli || noApiKey) && isGeminiCliAvailable()) {
    try {
      ...
      if (noApiKey) {
        console.warn(`[iris-lead] Gemini CLI failed (${err.message}) — no API key configured`);
        throw new Error(
  ...
  try {
    ...
    console.log(`[iris-lead] LLM reply from primary ${primaryLabel} @ ${url}`);
    ...
    console.log(`[iris-lead] Primary ${providerKey}/${modelId} failed (${reason}) — falling back to ${fbLabel}`);
    const fallbackMessages = isContextErr ? trimMessagesForFallback(messages) : messages;
    // So iris-lead knows which model is actually running when on fallback
    const primaryLabel = `${providerKey}/${modelId}`;
    ...
    try {
      ...
      console.log(`[iris-lead] LLM reply from fallback ${fbLabel} @ ${fbUrl}`);
      ...
      if (fbContextErr) {
        console.log(`[iris-lead] Fallback also hit context limit — retrying with aggressively trimmed messages`);
        ...
        const fbUrl = `${cfg.fallbackProvider.baseUrl.replace(/\/$/, "")}/chat/completions`;
        console.log(`[iris-lead] LLM reply from fallback ${fbLabel} (trimmed) @ ${fbUrl}`);
