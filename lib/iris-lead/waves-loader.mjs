 ...
 * Used by iris-lead to construct @@PIPELINE for "build me X" requests.
 */
...
/**
 * Format planning pipeline for iris-lead's prompt
...
export function formatWavesForPrompt() {
  ...
  const lines = [];
  lines.push("PLANNING PHASE — for 'build me X' or 'dispatch the iris' requests:");
  lines.push("- PM cannot receive replies from other agents (one-shot task). So YOU (iris-lead) orchestrate planning via a 3-wave pipeline.");
  ...
  lines.push(`- The pipeline STOPS after wave ${config.waves.length} (PM delivers PDD + TECH-SPEC + ROADMAP). User reviews via iris-lead, then you launch a separate build pipeline.`);
...
/**
 * Generate @@PIPELINE JSON for iris-lead to emit
 * @param {Object} projectContext - {projectName, projectPath, userBrief, userRequest}
