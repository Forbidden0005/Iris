 ...
 * Format: /tmp/iris-wt-{pipelineId.slice(0,8)}-{agentId}
 */
export function worktreePath(pipelineId, agentId) {
  return `/tmp/iris-wt-${pipelineId.slice(0, 8)}-${agentId}`;
}

/**
 * Return the deterministic branch name for an agent in a pipeline wave.
 * Format: iris/wave-{pipelineId.slice(0,8)}-{agentId}
 */
export function worktreeBranch(pipelineId, agentId) {
  return `iris/wave-${pipelineId.slice(0, 8)}-${agentId}`;
}
...
export function mergeWorktree(projectDir, pipelineId, waveIndex, agentId) {
  ...
  try {
    ...
    execSync(`git merge --no-ff -m "iris: merge wave ${waveIndex + 1} ${agentId}" "${branch}"`, {
      cwd: projectDir,
      encoding: "utf8",
      stdio: ["pipe", "pipe", "pipe"],
      timeout: 30000,
...
export function cleanupPipelineWorktrees(projectDir, pipelineId) {
  const prefix = `/tmp/iris-wt-${pipelineId.slice(0, 8)}-`;
  const branchPrefix = `iris/wave-${pipelineId.slice(0, 8)}-`;

  // Find all matching worktree paths under /tmp.
  let wtDirs = [];
  try {
    wtDirs = fs.readdirSync("/tmp")
      .filter(name => name.startsWith(`iris-wt-${pipelineId.slice(0, 8)}-`))
