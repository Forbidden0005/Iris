/**
 * Dynamic agent creation and management — extracted from iris-lead.mjs
 * Manages agents in ~/.iris/iris.json.
 */
...
const MAX_DYNAMIC_AGENTS = Number(process.env.IRIS_MAX_DYNAMIC_AGENTS || "5");

export function createAgent({ id, role, displayName, prompt, description, model }) {
  if (!id) throw new Error("Agent id is required");
  if (!id.startsWith("iris-")) id = `iris-${id}`;
  ...
  const agentModel = model || swarm.agents.find(a => a.id === "iris-main")?.model || "groq/llama-3.3-70b-versatile";
...
export function removeDynamicAgent(id) {
  ...
  const swarmPath = path.join(os.homedir(), ".iris", "iris.json");
  const swarm = tryRead(swarmPath) || {};
  if (!Array.isArray(swarm.agents)) throw new Error(`${id} is not a dynamic agent (or doesn't exist)`);
