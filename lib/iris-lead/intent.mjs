/**
 * Intent parsing and task brief — extracted from iris-lead.mjs
 */
...
export function writeTaskBrief(agent, task, projectDir) {
  ...
  try {
    const briefName = `_iris-${agent.replace("iris-","")}-brief-${Date.now()}.md`;
...
export function parseServiceIntent(msg) {
  ...
  const agentMatch = t.match(/restart\s+(iris-[a-z0-9-]+)/);
  if (agentMatch) return { action: "restart", id: agentMatch[1] };

  if (/restart\s+(them|it|everything|the\s+iris|all|bridges?)|bring\s+(them|the\s+iris|everyone)\s+back/.test(t))
    return { action: "restart", id: "agents" };
...
export function messageNeedsSearch(msg) {
  ...
  const delegationPattern = /(?:ask|tell|have|send|forward|give|pass)\s+(?:the\s+)?(?:writer|copywriter|pm|planner|coder|fixer|qa|security|github|frontend|main|iris-[a-z0-9-]+|planx|frank|blazer|antoine|copycopy|copycat|testy|stinki)/i;
...
export const DISPATCH_INTENT_REQUIRED = [
  ...
  /\bhave\s+(iris-\S+|\w+)\s+(do|fix|build|write|create|audit|test|run|check|handle|implement)/i,
  /\btell\s+(iris-\S+|\w+)\s+to\b/i,
  /\bask\s+(iris-\S+|\w+)\s+to\b/i,
  /\b(dispatch|send)\s+(to\s+)?(iris-\S+|\w+)\b/i,
  /\b(kick\s*off|rally|launch|start)\s+(the\s+)?(iris|pipeline|build|task|project)\b/i,
...
export function isDispatchIntended(userMessage) {
  ...
  if (DISPATCH_NEVER_PATTERNS.some(re => re.test(msg))) {
    console.log(`[iris-lead] 🚫 Dispatch blocked — message matches non-directive pattern: "${msg.slice(0, 60)}"`);
    return false;
  }
  ...
  if (wordCount <= 8) {
    console.log(`[iris-lead] 🚫 Dispatch blocked — short message with no dispatch intent: "${msg.slice(0, 60)}"`);
    return false;
  }
