/**
 * Background loop and rate-limit fallback — extracted from iris-lead.mjs
 */
...
function getBgConsciousnessLLM() {
  const cfg = _tryRead(path.join(os.homedir(), ".iris", "iris.json")) || {};
...
async function runBackgroundConsciousnessDirect() {
  ...
  const system = "You are iris-main managing the process for the user. Reply in under 100 words. Output: 1) One sentence on system/iris state or suggested next step. 2) If something needs follow-up, emit exactly one line: @@BRAIN iris-main: <fact> OR @@DISPATCH {\"agent\":\"...\",\"task\":\"...\"}. Otherwise reply NO_ACTION.";
  ...
  if (brainMatch) {
    ...
    const BRAIN_JUNK = /^(iris-lead \(auto\)|Services:|System Status|All (systems|agents)|✅|Hello|Hey|Hi[,!.\s]|Yo[,!\s]|I'm |You |What |Now |Look|Always|Hell |Cute|First|RT bus)/i;
    ...
    if (looksLikeChat) {
      ...
      try {
        _appendToBrain("iris-main", entry);
        console.log("[iris-lead] @@BRAIN (bg):", entry.slice(0, 60));
      } catch (e) {
  ...
  for (const d of dispatches) {
    try {
      ...
      console.log("[iris-lead] @@DISPATCH (bg):", d.agent, d.task?.slice(0, 50));
    } catch (e) {
  ...
  if (!isNoAction) {
    _appendHistory("default", "owner", "system", `[iris-main — background]: ${short}`);
    _broadcastSSE({ type: "agent_reply", from: "iris-main", content: short, sessionId: "owner", _bg: true, ts: Date.now() });
  }
  try {
    ...
    if (_pendingPipelines.size === 0) {
      ...
      if (bgEnabled && Date.now() - _lastBgConsciousnessAt >= _bgConsciousnessIntervalMs) {
        ...
        if (useDirect) {
          ...
          const consciousnessTask = `BACKGROUND CYCLE — you are managing the process for the user. Your reply is shown in their chat and written to ~/.iris/process-status.md.
@@READ_FILE ${_brainPath}
Consider: what should the user know? (stalled work, next steps, blockers, health.) Reply in under 100 words.
Reply with: 1) One sentence on system/iris state or suggested next step. 2) If something needs follow-up, emit exactly one @@BRAIN: or @@DISPATCH line (e.g. dispatch to fix a stuck pipeline). Otherwise reply NO_ACTION.`;
          try {
            _dispatchTask("iris-main", consciousnessTask, "bg-consciousness", null);
            console.log("[bg-loop] Dispatched background consciousness cycle to iris-main (no cheap model configured)");
          } catch (e) {
...
export function startBackgroundLoop() {
  ...
  console.log("[iris-lead] Background loop started (5m interval)");
  const bgEnabled = _getBgConsciousnessEnabled ? _getBgConsciousnessEnabled() : _bgConsciousnessEnabled;
  if (bgEnabled) {
    console.log("[iris-lead] Background consciousness ON — reflect every " + (_bgConsciousnessIntervalMs / 60000) + "m when idle");
  } else {
    console.log("[iris-lead] Background consciousness OFF — toggle in Dashboard → Settings");
  }
}
...
const _ROLE_FALLBACK = {
  ...
  researcher: "iris-main",
  auditor: "iris-qa",
  ops: "iris-main",
  generalist: "iris-main",
};
...
export function getRateLimitFallback(agentId) {
  ...
  const swarm = _tryRead(path.join(os.homedir(), ".iris", "iris.json"));
