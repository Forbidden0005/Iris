/**
 * HTTP server for iris-lead — extracted from iris-lead.mjs
...
import { IRIS_REPO_ROOT, loadSwarmConfig } from "../runtime/config.mjs";
...
function shouldDropPassthroughStderrLine(engine, line) {
  ...
  if (
    ...
    return true;
  // Codex rmcp client: configured HTTP MCP is down (common: iris-cli on :4097 not running)
  if (
    engine === "codex" &&
    /\/mcp/i.test(l) &&
...
const DIRECT_FANOUT_TIMEOUT_MS = Number(
  process.env.IRIS_DIRECT_FANOUT_TIMEOUT_MS || 20_000,
);
const BROADCAST_ALL_TIMEOUT_MS = Number(
  process.env.IRIS_BROADCAST_ALL_TIMEOUT_MS || 15_000,
);
...
function buildBroadcastReplyInstructions() {
  return [
    ...
    "- You were invoked by @iris-all broadcast.",
...
function filterDirectChatHistory(history = [], directChatMetadata = null) {
  ...
  // Shared-room direct mentions should not inherit iris-lead assistant turns.
...
function buildDirectChatContext(agent, directChatMetadata = null) {
  ...
  const bareId = agent.replace(/^iris-/, "");
  ...
  const csSwarm =
    tryRead(path.join(os.homedir(), ".iris", "iris.json")) || {};
...
function buildDirectCliPrompt({
  ...
  const context = history
    ...
    .join("\n");
  return [
    `You are @${participant.id} participating in a shared iris chat.`,
    "",
    "## Direct Chat Mode",
    "- Reply directly to the latest message in plain text.",
...
async function maybeRouteDirectReplyMentions({
  ...
  try {
    ...
    console.warn(
      `[iris-lead] Direct mention routing failed for ${sender}: ${error.message}`,
    );
  }
}
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === "/health" && req.method === "GET") {
        json(res, 200, { ok: true, agent: "iris-lead", port: PORT });
        return;
      }
      ...
      if (url.pathname === "/chat" && req.method === "POST") {
        ...
        const mentionRoute =
          !targetAgent && channelMode
            ...
            : null;
        console.log(
          `[iris-lead] /chat session=${sessionId || "default"} channelMode=${channelMode ? "1" : "0"} targetAgent=${targetAgent || "-"} mentionMode=${mentionRoute?.mode || "-"} message="${String(message || "").replace(/\s+/g, " ").slice(0, 120)}"`,
        );
        ...
        if (
          ...
          try {
            ...
            console.error(
              "[iris-lead] /chat multi direct route failed:",
              e.message,
              e.stack,
            );
            json(res, 500, { ok: false, error: e.message });
        ...
        if (!targetAgent && (mentionTargetParticipant?.kind === "cli" || pinnedParticipant)) {
          try {
            ...
            console.error(
              `[iris-lead] /chat direct CLI route to ${mentionTargetParticipant.id} failed:`,
              e.message,
              e.stack,
            );
            json(res, 500, { ok: false, error: e.message });
        ...
        // If targetAgent is specified and it's not iris-lead, use direct chat semantics.
        if (targetAgent && targetAgent !== "iris-lead") {
          console.log(
            `[iris-lead] /chat routing to ${targetAgent} via direct chat (session=${sessionId})`,
          );
          try {
            const enriched = await enrichTwitterLinks(message, {
              source: "iris-lead:direct-agent",
            });
            ...
            console.error(
              `[iris-lead] /chat direct route to ${targetAgent} failed:`,
              e.message,
              e.stack,
            );
            json(res, 500, { ok: false, error: e.message });
        ...
        console.log(
          `[iris-lead] /chat session=${sessionId} project=${projectId || "none"} msg=${message.slice(0, 60)}`,
        );
        try {
          const enriched = await enrichTwitterLinks(message, {
            source: "iris-lead:chat",
          });
          ...
          if (result.reply && sessionId) {
            ...
            if (rtPublish) {
              ...
              console.log(
                `[iris-lead] Published reply to RT bus for session=${sessionId}`,
              );
            }
          }
          ...
          console.error("[iris-lead] /chat handler error:", e);
      ...
      if (url.pathname === "/chat/stream" && req.method === "POST") {
        ...
        console.log(
          `[iris-lead] /chat/stream session=${sessionId} project=${projectId || "none"} agent=${agentId || "iris-lead"} msg=${message.slice(0, 60)}`,
        );

        try {
          const enriched = await enrichTwitterLinks(message, {
            source: "iris-lead:chat-stream",
          });

          // Direct agent mode: use runDirectAgentChat if agentId is provided
          if (agentId && agentId !== "iris-lead") {
            const result = await runDirectAgentChat({
              agent: agentId,
              message: enriched.text,
              sessionId,
            ...
            // Iris-lead mode: use handleChat with streaming callback
            const result = await handleChat({
              message: enriched.text,
              sessionId,
              userId,
          ...
          console.error("[iris-lead] /chat/stream handler error:", e);
      ...
      // POST /api/chat-agent — fast direct LLM call for non-iris-lead agents (bypasses dispatch)
      // Body: { agent: "iris-loco", message: "...", sessionId: "whatsapp-xxx", firstName: "Name" }
      ...
      if (url.pathname === "/api/chat-agent" && req.method === "POST") {
        ...
        console.log(
          `[iris-lead] /api/chat-agent agent=${agent} session=${sessionId} msg=${message.slice(0, 60)}`,
        );

        try {
          const enriched = await enrichTwitterLinks(message, {
            source: "iris-lead:api-chat-agent",
          });
          const result = await runDirectAgentChat({
            agent,
            message: enriched.text,
          ...
          console.log(
            `[iris-lead] /api/chat-agent ${agent} replied: ${result.reply.slice(0, 80)}...`,
          );
          json(res, 200, {
            ok: true,
            reply: result.reply,
          ...
          console.error(
            `[iris-lead] /api/chat-agent ${agent} failed:`,
            e.message,
            e.stack,
          );
          json(res, 500, { ok: false, error: e.message });
      ...
      // GET /api/iris-lead/history?sessionId=xyz&projectId=abc — load chat history for session (optionally project-scoped)
      ...
      // GET /api/iris-lead/project-messages?projectId=xyz — load unified project messages (all sources)
      if (
        ...
        try {
          ...
          console.error("[iris-lead] Failed to load project messages:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }

      // GET /api/iris-lead/search-project-messages?projectId=xyz&q=auth — search project messages
      if (
        url.pathname === "/api/iris-lead/search-project-messages" &&
        req.method === "GET"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error("[iris-lead] Failed to search project messages:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }

      // GET /api/iris-lead/export-project-messages?projectId=xyz&format=markdown — export project messages
      if (
        url.pathname === "/api/iris-lead/export-project-messages" &&
        req.method === "GET"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error("[iris-lead] Failed to export project messages:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }

      // GET /api/iris-lead/message-threads?projectId=xyz — get message threads
      if (
        url.pathname === "/api/iris-lead/message-threads" &&
        req.method === "GET"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error("[iris-lead] Failed to load message threads:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }

      // GET /api/iris-lead/search-messages-semantic?projectId=xyz&q=authentication — semantic search
      if (
        url.pathname === "/api/iris-lead/search-messages-semantic" &&
        req.method === "GET"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error(
            "[iris-lead] Failed to search messages semantically:",
            e,
          );
          json(res, 500, { ok: false, error: e.message });
        }
      ...
      // POST /api/iris-lead/index-project-messages — index project messages into RAG
      if (
        url.pathname === "/api/iris-lead/index-project-messages" &&
        req.method === "POST"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error("[iris-lead] Failed to index project messages:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }

      // GET /api/iris-lead/message-index-stats — get RAG index statistics
      if (
        url.pathname === "/api/iris-lead/message-index-stats" &&
        req.method === "GET"
      ) {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
        ...
        try {
          ...
          console.error("[iris-lead] Failed to get index stats:", e);
          json(res, 500, { ok: false, error: e.message });
        }
        return;
      }
      ...
      if (url.pathname === "/confirm-project" && req.method === "POST") {
        ...
        console.log(`[iris-lead] /confirm-project draftId=${draftId}`);
        try {
      ...
      if (url.pathname === "/approve-cmd" && req.method === "POST") {
        ...
        if (rtPublish) {
          ...
          console.log(`[iris-lead] ✅ cmd approved: ${approvalId}`);
        }
        json(res, 200, { ok: true });
        return;
      }
      ...
      if (url.pathname === "/reject-cmd" && req.method === "POST") {
        ...
        if (rtPublish) {
          ...
          console.log(`[iris-lead] ⛔ cmd rejected: ${approvalId}`);
        }
        json(res, 200, { ok: true });
        return;
      }
      ...
      if (url.pathname === "/allowlist-cmd" && req.method === "POST") {
        ...
        if (!list.includes(pattern)) {
          ...
          console.log(`[iris-lead] ✅ Added to cmd allowlist: ${pattern}`);
        }
      ...
      if (url.pathname === "/allowlist-cmd" && req.method === "DELETE") {
        ...
        fs.writeFileSync(file, JSON.stringify(list, null, 2));
        console.log(`[iris-lead] 🗑 Removed from cmd allowlist: ${pattern}`);
      ...
      // Any external tool (another iris, OpenClaw plugin, scripts) can dispatch tasks
      // and poll status without sharing LLM credentials.
      // Auth: Authorization: Bearer <RT_TOKEN from ~/.iris/iris.json rt.authToken>

      // POST /api/dispatch  { agent, task, verify?, done?, sessionId? }
      // Returns { ok, taskId, agent }
      // ── OpenCode plugin push — receives events from the iris-feed plugin
      ...
      if (url.pathname === "/api/dispatch" && req.method === "POST") {
        ...
        // Iris plan bridge — explicit opt-in only. Both ids must be given
        // together; a caller not passing either gets exactly today's
        // behavior. No validation that they resolve to a real plan/task
        // here — the wave-dispatcher link hook and ws-router evidence
        // hooks that consume these already fail safe (caught, logged,
        // no-op) if they don't.
        if (
          typeof irisPlanId === "string" && irisPlanId.trim() &&
          typeof irisTaskId === "string" && irisTaskId.trim()
        ) {
          routeFlags.irisPlanId = irisPlanId.trim();
          routeFlags.irisTaskId = irisTaskId.trim();
        }
        ...
        console.log(`[iris-lead] /api/dispatch → ${agent} taskId=${taskId}`);
        json(res, 200, {
          ok: true,
      ...
      if (url.pathname === "/api/pipeline" && req.method === "POST") {
        ...
        try {
          ...
          console.log(`[iris-lead] /api/pipeline → pipelineId=${pipelineId} (${waves.length} waves)`);
      ...
      if (url.pathname === "/api/agents" && req.method === "GET") {
        ...
        // iris-lead is not in agentRoster — expose configured primary model here (bridge agents only in `agents`)
        json(res, 200, {
          ok: true,
          crewLead: {
            id: "iris-lead",
      ...
      if (toolsPatchMatch && req.method === "PATCH") {
        ...
        console.log(
          `[iris-lead] tools updated for ${agentId}: ${saved.join(", ")}`,
        );
      ...
      if (webhookMatch && req.method === "POST") {
        ...
        if (rtPublish) {
          ...
          console.log(
            `[iris-lead] webhook → ${channel}: ${JSON.stringify(body).slice(0, 80)}`,
          );
        }
        broadcastSSE({
          type: "webhook_event",
      ...
      if (url.pathname === "/api/skills/approve" && req.method === "POST") {
        ...
        console.log(
          `[iris-lead] skill approved: ${entry.skillName} (${approvalId}) — notifying bridge agents`,
        );
        const rtPublish = getRtPublish();
        if (rtPublish) {
          rtPublish({
      ...
      if (url.pathname === "/api/settings/bg-consciousness") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            console.warn(
              "[iris-lead] Could not persist bgConsciousness:",
              e.message,
            );
          }
          console.log(
            `[iris-lead] Background consciousness ${enable ? "ENABLED" : "DISABLED"} model=${bgConsciousnessRef.model} via dashboard`,
          );
      ...
      // GET /api/iris-cli-sessions — list iris-cli session history from .iris/sessions
      // ?limit=10
      if (url.pathname === "/api/iris-cli-sessions" && req.method === "GET") {
        if (!checkBearer(req)) {
          json(res, 401, { ok: false, error: "Unauthorized" });
          return;
        }
        try {
          ...
          const sessionsBase = path.join(IRIS_REPO_ROOT, ".iris", "sessions");
          const sessions = [];
          if (fs.existsSync(sessionsBase)) {
      ...
      if (url.pathname === "/api/engine-passthrough/clear-session" && req.method === "POST") {
        ...
        try {
          ...
          const storeFile = path.join(os.homedir(), ".iris", "passthrough-sessions.json");
          let store = {};
      ...
      if (url.pathname === "/api/engine-passthrough" && req.method === "POST") {
        ...
        const enriched = await enrichTwitterLinks(message, {
          source: `iris-lead:engine-${engine}`,
        });
        ...
        if (engine === "cursor") {
          // Match gateway `runCursorCliTask` / iris-cli: use Cursor **agent** CLI (`agent`),
          ...
          bin = resolveCliBinary(process.env.IRIS_OPENCODE_BIN || "opencode", [
          ...
          const geminiModel =
            reqModel || process.env.IRIS_GEMINI_CLI_MODEL || null;
        ...
        } else if (engine === "iris-cli") {
          bin = resolveCliBinary(process.env.IRIS_CLI_BIN || "iris", [
            path.join(IRIS_REPO_ROOT, "iris-cli", "bin", "iris.js"),
          ]);
          const crewCliModel =
            reqModel || process.env.IRIS_CREW_CLI_MODEL || null;
          if (bin.endsWith(".js")) {
          ...
          bin = process.env.IRIS_OPENCODE_BIN || "opencode";
          const agModel =
            process.env.IRIS_ANTIGRAVITY_MODEL ||
          ...
          bin = "docker";
          const sandboxName =
            process.env.IRIS_DOCKER_SANDBOX_NAME || "iris";
          const innerEngine = (
            process.env.IRIS_DOCKER_SANDBOX_INNER_ENGINE || "claude"
          ).toLowerCase();
          let innerArgs;
          ...
          args.push("--strict-mcp-config", "--mcp-config", path.join(os.homedir(), ".iris", "config", "empty-mcp.json"));
        ...
        // Merge iris.json env vars into spawn env (iris-cli L2, model overrides, etc.)
        ...
        if (engine === "claude") {
          ...
          delete spawnEnv.IRIS_CLAUDE_CODE;
        }
        if (engine === "codex") {
        ...
        // Force L2 planner for iris-cli when requested (enhance-prompt planner path)
        if (reqForceL2) spawnEnv.IRIS_FORCE_L2 = "true";
        ...
        const passthroughTimeoutMs = Number(
          process.env.IRIS_PASSTHROUGH_TIMEOUT_MS || "300000",
        );
        ...
        const handleChunk = (chunk) => {
          ...
          for (const line of lines) {
            ...
            // iris-cli JSON handling - collect entire output, don't stream it
            if (engine === "iris-cli") {
        ...
        proc.on("close", async (code) => {
          ...
          // iris-cli special handling: extract JSON from output (skipping logs) and send only response field
          if (engine === "iris-cli" && fullOutput.trim()) {
            let extracted = false;
            try {
              ...
              if (jsonMatch) {
                ...
                if (parsed.response) {
                  ...
                  console.log(`[iris-cli] ✅ Extracted response: ${responseText.slice(0, 80)}...`);
            ...
            if (!extracted) {
              ...
              try {
                ...
                if (respMatch) {
                  ...
                  console.log(`[iris-cli] ✅ Extracted response (regex): ${responseText.slice(0, 80)}...`);
                  send({ type: "chunk", text: responseText });
                  fullOutput = responseText;
                  extracted = true;
                }
            ...
            if (!extracted) {
              console.error(`[iris-cli] ❌ Failed to extract response from iris-cli output`);
              console.error(`[iris-cli] First 200 chars: ${fullOutput.slice(0, 200)}`);
          ...
          // Set in ~/.iris/iris.json env block or as env var
          ...
          if (notifyTG)
            try {
              const tgBridge = JSON.parse(
                fs.readFileSync(
                  path.join(os.homedir(), ".iris", "telegram-bridge.json"),
                  "utf8",
                ),
              );
              const botToken =
      ...
      if (url.pathname === "/api/settings/claude-code") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            console.warn(
              "[iris-lead] Could not persist claudeCode:",
              e.message,
            );
          }
          console.log(
            `[iris-lead] Claude Code executor ${enable ? "ENABLED" : "DISABLED"} via dashboard`,
          );
      ...
      if (url.pathname === "/api/settings/cursor-waves") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            console.warn(
              "[iris-lead] Could not persist cursorWaves:",
              e.message,
            );
          }
          console.log(
            `[iris-lead] Cursor Waves ${enable ? "ENABLED" : "DISABLED"} via dashboard`,
          );
          json(res, 200, { ok: true, enabled: cursorWavesRef.enabled });
          return;
        }
      ...
      if (url.pathname === "/api/settings/tmux-bridge") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            console.warn(
              "[iris-lead] Could not persist tmuxBridge:",
              e.message,
            );
          }
          console.log(
            `[iris-lead] tmux-bridge ${enable ? "ENABLED" : "DISABLED"} via dashboard`,
          );
      ...
      if (url.pathname === "/api/settings/global-fallback") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            console.log(
              `[iris-lead] Global fallback model set to: ${model || "(cleared)"}`,
            );
      ...
      // GET/POST /api/settings/opencode-project — set/get OpenCode project dir, fallback model, and iris-lead model
      if (url.pathname === "/api/settings/opencode-project") {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            fs.writeFileSync(cfgPath, JSON.stringify(cfg, null, 2));
            console.log(
              `[iris-lead] Settings saved: opencodeProject=${cfg.settings.opencodeProject || "(cleared)"} fallback=${cfg.opencodeFallbackModel || "(cleared)"} crewLeadModel=${cfg.crewLeadModel || "(cleared)"}`,
            );
            json(res, 200, {
              ok: true,
      ...
      // Helper: read/write iris.json for simple boolean/string settings
      ...
      if (url.pathname === "/api/settings/autonomous-mentions") {
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Autonomous mentions ${cfg.settings.autonomousMentionsEnabled ? "ENABLED" : "DISABLED"} via dashboard`);
      ...
      if (url.pathname === "/api/settings/codex") {
        ...
        if (req.method === "GET") {
          const cfg = readCfg();
          const enabled = cfg.codexEnabled === true || /^1|true|yes$/i.test(String(process.env.IRIS_CODEX_ENABLED || ""));
          json(res, 200, { ok: true, enabled });
          return;
        }
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Codex executor ${enable ? "ENABLED" : "DISABLED"} via dashboard`);
          json(res, 200, { ok: true, enabled: enable });
          return;
        }
      }
      ...
      if (url.pathname === "/api/settings/gemini-cli") {
        ...
        if (req.method === "GET") {
          const cfg = readCfg();
          const enabled = cfg.geminiCliEnabled === true || /^1|true|yes$/i.test(String(process.env.IRIS_GEMINI_CLI_ENABLED || ""));
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Gemini CLI ${enable ? "ENABLED" : "DISABLED"} via dashboard`);
          json(res, 200, { ok: true, enabled: enable, installed: whichBin("gemini") });
          return;
        }
      }

      // GET/POST /api/settings/iris-cli
      if (url.pathname === "/api/settings/iris-cli") {
        if (!checkBearer(req)) { json(res, 401, { ok: false, error: "Unauthorized" }); return; }
        if (req.method === "GET") {
          const cfg = readCfg();
          const enabled = cfg.crewCliEnabled === true || /^1|true|yes$/i.test(String(process.env.IRIS_CREW_CLI_ENABLED || ""));
          json(res, 200, { ok: true, enabled });
          return;
        }
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Iris CLI ${enable ? "ENABLED" : "DISABLED"} via dashboard`);
          json(res, 200, { ok: true, enabled: enable });
          return;
        }
      }
      ...
      if (url.pathname === "/api/settings/opencode") {
        ...
        if (req.method === "GET") {
          const cfg = readCfg();
          const enabled = cfg.opencodeEnabled === true || /^1|true|yes$/i.test(String(process.env.IRIS_OPENCODE_ENABLED || ""));
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] OpenCode ${enable ? "ENABLED" : "DISABLED"} via dashboard`);
      ...
      if (url.pathname === "/api/settings/global-oc-loop") {
        ...
        if (req.method === "GET") {
          const cfg = readCfg();
          const enabled = cfg.engineLoop === true || /^1|true|yes$/i.test(String(process.env.IRIS_ENGINE_LOOP || ""));
          const maxRounds = cfg.engineLoopMaxRounds ?? parseInt(process.env.IRIS_ENGINE_LOOP_MAX_ROUNDS || "10", 10);
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Engine loop: enabled=${cfg.engineLoop ?? false}, maxRounds=${cfg.engineLoopMaxRounds ?? 10}`);
      ...
      if (url.pathname === "/api/settings/passthrough-notify") {
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Passthrough notify → ${cfg.passthroughNotify}`);
      ...
      if (url.pathname === "/api/settings/loop-brain") {
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log(`[iris-lead] Loop brain → ${cfg.loopBrain || "(cleared)"}`);
          json(res, 200, { ok: true, loopBrain: cfg.loopBrain });
          return;
        }
      }
      ...
      if (url.pathname === "/api/settings/rt-token") {
        ...
        if (req.method === "GET") {
          const cfg = readCfg();
          const token = cfg.rtToken || process.env.IRIS_RT_AUTH_TOKEN || "";
        ...
        if (req.method === "POST") {
          ...
          writeCfg(cfg);
          console.log("[iris-lead] RT token saved via dashboard");
      ...
      if (url.pathname === "/api/config/lock-status" && req.method === "GET") {
        if (!checkBearer(req)) { json(res, 401, { ok: false, error: "Unauthorized" }); return; }
        const lockFile = path.join(os.homedir(), ".iris", ".config.lock");
      ...
      if (url.pathname === "/api/config/lock" && req.method === "POST") {
        ...
        try {
          ...
          console.log("[iris-lead] Config LOCKED via dashboard");
      ...
      if (url.pathname === "/api/config/unlock" && req.method === "POST") {
        ...
        try {
          ...
          console.log("[iris-lead] Config UNLOCKED via dashboard");
        } catch {}
      ...
      if (url.pathname === "/api/health" && req.method === "GET") {
        ...
        if (!checkBearer(req)) {
          json(res, 200, { ok: true, ts: new Date().toISOString(), agent: "iris-lead", port: PORT });
          return;
        }
        ...
        const opencodeProject =
          ...
          process.env.IRIS_OPENCODE_PROJECT ||
          "";
      ...
      if (restartMatch && req.method === "POST") {
        ...
        try {
          ...
          // Respawn via start-iris --agent (waits for spawn, then returns)
          setTimeout(async () => {
            try {
              exec2(
                `node ${path.join(IRIS_REPO_ROOT, "scripts", "start-iris.mjs")} --agent ${agentId}`,
                {
                  shell: true,
                  timeout: 10000,
                  env: { ...process.env, IRIS_RT_AUTH_TOKEN: RT_TOKEN },
                },
              );
              console.log(`[iris-lead] bridge respawned for ${agentId}`);
            } catch (e2) {
              console.error(
                `[iris-lead] failed to respawn bridge for ${agentId}:`,
                e2.message,
              );
            }
          }, 500);
          console.log(`[iris-lead] restart requested for ${agentId}`);
      ...
      console.error("[iris-lead] error:", err.message);
      json(res, 500, { ok: false, error: err.message });
    }
  });

  const BIND_HOST = process.env.IRIS_BIND_HOST || "127.0.0.1";
  server.listen(PORT, BIND_HOST, () => {
    const cfg = loadConfig();
    console.log(`[iris-lead] HTTP server on http://${BIND_HOST}:${PORT}`);
    console.log(`[iris-lead] Model: ${cfg.providerKey}/${cfg.modelId}`);
    console.log(`[iris-lead] History: ${historyDir}`);
    console.log(`[iris-lead] Agents: ${cfg.knownAgents.join(", ")}`);
    console.log(`[iris-lead] Dispatch timeout: ${dispatchTimeoutMs / 1000}s`);
  ...
  server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      if (_portRetries < 6) {
        ...
        console.error(
          `[iris-lead] Port ${PORT} in use — retry ${_portRetries}/6 in ${delay}ms (waiting for old process to exit)`,
        );
        setTimeout(() => {
          server.close();
          server.listen(PORT, BIND_HOST);
        ...
        console.error(
          `[iris-lead] Port ${PORT} still in use after 6 retries — run: lsof -ti :${PORT} | xargs kill -9`,
        );
        process.exit(1);
      }
    } else {
      console.error("[iris-lead] server error:", err.message);
      process.exit(1);
    }
  });
