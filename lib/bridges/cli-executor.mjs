 ...
 * 
 * Executes OpenCode/Cursor/Iris-CLI tasks with:
 ...
 * 
 * @param {string} cliName - "opencode" | "cursor" | "iris-cli" | "codex" | "gemini" | "claude"
 * @param {string} task - Task description
 * @param {string} agentId - Agent ID (e.g. "iris-coder")
...
export async function executeCLI(cliName, task, agentId = null, context = {}, onOutput = null) {
  ...
  if (agentId) {
    try {
      const csSwarm = JSON.parse(readFileSync(IRIS_CFG_PATH, "utf8"));
  ...
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn(bin, args, {
      ...
      env: {
        ...process.env,
        IRIS_SESSION_ID: sessionId || chatId,
        IRIS_AGENT: agentId
      },
  ...
  switch (cliName) {
    ...
    case "iris-cli":
      model =
        ...
        null;
      return {
        bin: process.env.IRIS_CLI_BIN || "iris",
        args: [
          "chat",
          task,
          "--json",
    ...
    case "claude":
      ...
      model =
        ...
        model ||
        process.env.IRIS_CLAUDE_CODE_MODEL ||
        null;
      return {
...
export function getAvailableCLIs() {
  ...
  if (which("iris") || existsSync(join(process.cwd(), "iris-cli"))) clis.push("iris-cli");
...
function getDefaultModelForCLI(cliName) {
  switch (cliName) {
    case "opencode":
      return process.env.IRIS_OPENCODE_MODEL || "openai/gpt-5.4";
    case "cursor":
      return process.env.CURSOR_DEFAULT_MODEL || "gemini-3-flash";
    case "claude":
      return process.env.IRIS_CLAUDE_CODE_MODEL || null;
    case "gemini":
      return process.env.IRIS_GEMINI_CLI_MODEL || null;
    case "iris-cli":
      return process.env.IRIS_CREW_CLI_MODEL || "groq/llama-3.3-70b-versatile";
    case "codex":
      return process.env.IRIS_CODEX_MODEL || null;
    default:
      return null;
  }
}
