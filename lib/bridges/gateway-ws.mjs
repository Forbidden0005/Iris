export function initGatewayWs(deps) {
    ...
    function createRealtimeClient({ onEnvelope, agentName = IRIS_RT_AGENT, token = IRIS_RT_TOKEN, channels = IRIS_RT_CHANNELS }) {
        return new Promise((resolveConnect, rejectConnect) => {
            const ws = new WebSocket(IRIS_RT_URL, IRIS_RT_URL.startsWith("wss://") && IRIS_RT_TLS_INSECURE
            ...
            ws.on("open", () => {
                telemetry("realtime_open", { url: IRIS_RT_URL, agent: agentName });
            });
            ...
            ws.on("close", () => {
                ready = false;
                telemetry("realtime_closed", { url: IRIS_RT_URL });
                if (!settled) {
                    settled = true;
        ...
        return new Promise((resolveConnect, rejectConnect) => {
            ...
            function doConnect(nonce) {
                ...
                send("connect", {
                    ...
                    resolveConnect({
                        send, ws,
                        chat: (msg, sessionKey = IRIS_RT_AGENT || "main", options = {}) => {
    ...
    async function runRealtimeDaemon(bridge) {
        syncOpenCodePermissions();
        progress(`Starting OpenCrew realtime daemon via ${IRIS_RT_URL}...`);
        ...
        const shutdown = () => {
            ...
            try {
                currentClient?.publish({ channel: "events", type: "agent.offline", payload: { agent: IRIS_RT_AGENT } });
            } catch (e) {
        ...
        while (!stopRequested) {
            try {
                ...
                console.log(`OpenCrew daemon online: ${IRIS_RT_AGENT}`);
                console.log(`- gateway: ${GATEWAY_URL}`);
                console.log(`- realtime: ${IRIS_RT_URL}`);
                console.log(`- subscribed: ${IRIS_RT_CHANNELS.join(", ")}`);
                ...
                heartbeat = setInterval(() => {
                    try {
                        rt.publish({
                            ...
                            to: "broadcast",
                            payload: { agent: IRIS_RT_AGENT, ts: new Date().toISOString() },
                        });
                    } catch (e) {
                ...
                currentClient = null;
                if (!stopRequested) {
                    progress(`Realtime disconnected. Reconnecting in ${IRIS_RT_RECONNECT_MS}ms...`);
                    await sleep(IRIS_RT_RECONNECT_MS);
                }
            } catch (err) {
                telemetry("realtime_daemon_error", { message: err?.message ?? String(err) });
                progress(`Realtime daemon error: ${err?.message ?? String(err)}`);
                if (!stopRequested) await sleep(IRIS_RT_RECONNECT_MS);
            }
        }
    }
