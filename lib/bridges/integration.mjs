...
const PLATFORM_CONFIG = {
  telegram: {
    ...
    excludeAgents: ['iris-loco'], // Chat-only agents
    icon: '📱'
  },
  whatsapp: {
    enabled: true,
    sourcePrefix: 'whatsapp',
    excludeAgents: ['iris-loco'],
    icon: '💬'
  },
  slack: {
    enabled: true,
 ...
 * @param {string|null} agent - Agent ID if from an agent (e.g., 'iris-pm')
...
export function saveBridgeMessage(platform, projectId, chatId, role, content, agent = null, metadata = {}) {
  ...
  // Skip if agent is excluded (e.g., iris-loco chat-only mode)
...
export function detectProjectFromMessage(content, allProjects) {
  ...
  // "dispatch iris-coder to website project: implement feature"
