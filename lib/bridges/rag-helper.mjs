/**
 * RAG Helper — fetch codebase context from iris-cli RAG API
 * Shared by: iris-lead, Telegram, WhatsApp, dashboard
 */

/**
 * Fetch codebase context from iris-cli RAG API
...
export async function fetchCrewCliRagContext(query, projectDir = process.cwd()) {
  try {
    ...
    const ragPort = process.env.IRIS_CLI_RAG_PORT || process.env.IRIS_API_PORT || '4317';
    ...
    if (error.name === 'TimeoutError' || error.code === 'ECONNREFUSED') {
      // Expected when iris serve is not running
      return '';
    }
