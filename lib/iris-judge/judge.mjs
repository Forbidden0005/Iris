/**
 * Iris Judge — Autonomous cycle decision maker
 * 
...
function buildJudgePrompt(data) {
  ...
  return `You are iris-judge. Evaluate the PM loop cycle and decide: CONTINUE, SHIP, or RESET.
...
function getJudgeModel() {
  ...
  const judgeModel = process.env.IRIS_JUDGE_MODEL 
...
async function callJudgeModel(model, prompt) {
  // Import LLM caller from iris-lead
  const { callLLM } = await import('../iris-lead/llm-caller.mjs');
  ...
  try {
    ...
    const messages = [
      {
        role: 'system',
        content: 'You are iris-judge. Return ONLY valid JSON: {"decision": "CONTINUE"|"SHIP"|"RESET", "reasoning": "...", "confidence": 0.0-1.0}'
      },
      {
        role: 'user',
        content: prompt
    ...
    if (!jsonMatch) {
      console.warn('[iris-judge] No JSON in response, defaulting to CONTINUE');
      return {
        decision: 'CONTINUE',
    ...
    if (!['CONTINUE', 'SHIP', 'RESET'].includes(parsed.decision)) {
      console.warn('[iris-judge] Invalid decision, defaulting to CONTINUE');
      return {
        decision: 'CONTINUE',
    ...
    console.error('[iris-judge] Error calling model:', error.message);
