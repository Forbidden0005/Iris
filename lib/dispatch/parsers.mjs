/**
 * Dispatch, pipeline, and project directive parsers — extracted from iris-lead.mjs
...
export function parseDispatch(text, userMessage = "") {
  ...
  // Direct imperative fallback: "dispatch iris-coder ...", "send it to iris-coder", etc.
  ...
  if (imperativeMatch) {
    ...
    const task = taskSource
      .replace(
        /^(?:please\s+)?(?:go\s+)?(?:dispatch|send|route|forward|tell|ask|use|sic(?:k)?)(?:\s+(?:it|this|him|her|them))?(?:\s+to)?\s+(?:fucking\s+|fuckin\s+|damn\s+|goddamn\s+)?(?:iris-[a-z0-9-]+)\b[:\s-]*/i,
        "",
      )
      .trim();
    if (agent && task) {
      console.warn(`[DEPRECATED] Imperative dispatch format detected. Use @@DISPATCH JSON marker instead.`);
      console.log(`[iris-lead] Imperative dispatch fallback: agent=${agent} task="${task.slice(0, 60)}"`);
      return { agent, task };
    }
  }
  ...
  const nlMatch = normalizedText.match(
    /(?:dispatching now|I(?:'ll| will| am) dispatch(?:ing)?|sending(?: this)? to|routing to|forwarding to|siccing)\b[^.]*?\b(iris-[a-z0-9-]+)/i
  );
  if (nlMatch) {
    let agent = nlMatch[1].trim();
    // Try to resolve display names to iris-X IDs
    if (!agent.startsWith("iris-")) {
      try {
    ...
    const task = userMessage
      ? userMessage.replace(/^(?:go\s+(?:write\s+)?(?:have\s+)?|have\s+|ask\s+|tell\s+)(?:iris-[a-z0-9-]+|[a-z]+)\s+(?:to\s+)?/i, "").trim() || userMessage
      : cleanText.replace(/\n/g, " ").slice(0, 200).trim();
    if (agent && task) {
      console.warn(`[DEPRECATED] Natural language dispatch detected. Use @@DISPATCH JSON marker instead.`);
      console.log(`[iris-lead] NL dispatch fallback: agent=${agent} task="${task.slice(0, 60)}"`);
      return { agent, task };
    }
  }
...
/**
 * When a dashboard project is selected, iris-lead knows `projectDir` but models often emit
...
// ── Pipeline DSL ──────────────────────────────────────────────────────────────
// Format: @@PIPELINE [{"wave":1,"agent":"iris-coder","task":"..."},{"wave":1,"agent":"iris-coder-front","task":"..."},{"wave":2,"agent":"iris-qa","task":"..."}]
...
export function parsePipeline(text) {
  ...
  try {
    ...
    // Auto-append iris-pm ROADMAP update as final wave if any coding agents are present
    // and iris-pm isn't already in the pipeline
    const codingAgents = new Set(['iris-coder','iris-coder-front','iris-coder-back','iris-frontend','iris-fixer','iris-ml']);
    const hasCodingAgent = steps.some(s => codingAgents.has(s.agent) || codingAgents.has((s.agent||'').toLowerCase()));
    const hasPm = steps.some(s => s.agent === 'iris-pm' || s.agent === 'pm');
    const hasQa = steps.some(s => s.agent === 'iris-qa' || s.agent === 'qa');
    const hasFixer = steps.some(s => s.agent === 'iris-fixer' || s.agent === 'fixer');

    // If pipeline has fixer but only one QA pass, insert a re-QA wave after fixer
    // so the pattern is always: ... → fixer → QA (re-check) → pm
    if (hasFixer && hasQa) {
      const fixerWaves = steps.filter(s => s.agent === 'iris-fixer' || s.agent === 'fixer').map(s => Number(s.wave));
      const maxFixerWave = Math.max(...fixerWaves);
      const qaAfterFixer = steps.some(s => (s.agent === 'iris-qa' || s.agent === 'qa') && Number(s.wave) > maxFixerWave);
      if (!qaAfterFixer) {
        ...
        steps.push({
          wave: maxFixerWave + 1,
          agent: 'iris-qa',
          task: 'Re-audit the previously flagged files after iris-fixer ran. Read the existing qa-report.md in the project directory (same folder as ROADMAP.md) to know what was fixed. Run py_compile on all .py files. Confirm CRITICAL and HIGH issues are resolved. Write your updated report to qa-report.md in that same project directory (no other filename).',
        });
      }
    }
