...
import { IRIS_REPO_ROOT } from "./config.mjs";
import {
...
export const SHARED_MEMORY_DIR = path.join(IRIS_REPO_ROOT, "memory");
...
export const SHARED_MEMORY_FILES = [
  "law.md", // Iris laws — no harm, no unauthorized access, don't break machine, create value
  "current-state.md", // System overview — what iris is, CRITICAL task guidance
...
export const _AGENT_EXTRA_MEMORY_STATIC = {
  "iris-fixer": ["lessons.md"],
  "iris-coder": ["lessons.md"],
  "iris-coder-front": ["lessons.md"],
  "iris-coder-back": ["lessons.md"],
};
...
export function getAgentExtraMemory(agentId) {
  const bareId = agentId.startsWith("iris-")
    ? `iris-${agentId.slice(5)}`
    : agentId;
...
export function loadAgentPrompts() {
  const candidates = [
    path.join(os.homedir(), ".iris", "agent-prompts.json"),
...
function buildExecutionGuardrails(agentId = "") {
  if (!agentId || agentId === "iris-lead") return "";

  const isPM = agentId.startsWith("iris-pm");
  const isQA = agentId === "iris-qa";
  const isCoder =
    agentId === "iris-coder" ||
...
export function buildTaskPrompt(taskText, sourceLabel, agentId, options = {}) {
  ...
  const bareId = agentId ? agentId.replace(/^iris-/, "") : null;
  const agentSystemPrompt =
    (agentId && agentPrompts[agentId]) ||
    (bareId && agentPrompts[bareId]) ||
    (agentId ? getSharedChatPromptOverlay(agentId) : null);

  const agentAllowed = _loadAgentToolPermissions(agentId || "iris-main");
  ...
  const projectRoot =
    taskProjectDir ||
    (agentId === "iris-fixer" ? _getOpencodeProjectDir() : null);
  ...
  if (agentId === "iris-fixer") {
    if (projectRoot) {
      ...
      projectDiscoveryRule = `## Project discovery (external projects)\n- External projects (e.g. polymarket-ai-strat) are NOT inside the iris repo. Their root is typically ${desktopProjectsHint}. If a path contains "iris/<project-name>/", replace that with "${path.join(os.homedir(), "Desktop")}/<project-name>/". Example: polymarket-ai-strat main.py is at ${path.join(os.homedir(), "Desktop", "polymarket-ai-strat", "src/api/main.py")} (not under iris, and not src/api/routers/main.py). Use @@RUN_CMD find to locate files if unsure.`;
    }
  }

  // Load per-project memory from <projectDir>/.iris/context.md and brain.md
  ...
  if (taskProjectDir) {
    const projectMemoryDir = path.join(taskProjectDir, ".iris");
