...
const SWARM_RUNTIME_DIR       = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "runtime");
...
const SWARM_DLQ_DIR           = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "dlq");

const IRIS_RT_DISPATCH_LEASE_MS      = Number(process.env.IRIS_RT_DISPATCH_LEASE_MS      || "45000");
const IRIS_RT_DISPATCH_HEARTBEAT_MS  = Number(process.env.IRIS_RT_DISPATCH_HEARTBEAT_MS  || "10000");
const IRIS_RT_TASK_STATE_TTL_MS      = Number(process.env.IRIS_RT_TASK_STATE_TTL_MS      || "21600000");
const IRIS_RT_AGENT                  = process.env.IRIS_RT_AGENT                         || process.env.OPENCREW_RT_AGENT || "iris-coder";
...
export async function withTaskLock(taskKey, fn) {
  ...
  const deadline = Date.now() + Math.max(200, IRIS_RT_DISPATCH_HEARTBEAT_MS);
  let lastErr;
...
export function clearStaleTaskState() {
  try {
    ...
    for (const fileName of fs.readdirSync(SWARM_TASK_STATE_DIR)) {
      ...
      if (now - ts > IRIS_RT_TASK_STATE_TTL_MS) {
...
export async function claimTaskLease({ taskKey, identity, incomingType, envelope, payload }) {
  ...
  return withTaskLock(taskKey, async () => {
    ...
    if (leaseActive && existingLease.owner && existingLease.owner !== IRIS_RT_AGENT) {
      return {
    ...
    const attempt = leaseActive && existingLease.owner === IRIS_RT_AGENT
    ...
    const leaseRecord = {
      ...
      claimedAt: nowIso,
      heartbeatAt: nowIso,
      leaseExpiresAt: new Date(nowMs + IRIS_RT_DISPATCH_LEASE_MS).toISOString(),
    };
...
export function startTaskLeaseHeartbeat(taskKey) {
  return setInterval(async () => {
    try {
      await withTaskLock(taskKey, async () => {
        const leaseFile = leasePath(taskKey);
        const existingLease = parseTaskState(leaseFile);
        if (!existingLease || existingLease.owner !== IRIS_RT_AGENT) return;
        const nowMs = Date.now();
        existingLease.heartbeatAt = new Date(nowMs).toISOString();
        existingLease.leaseExpiresAt = new Date(nowMs + IRIS_RT_DISPATCH_LEASE_MS).toISOString();
  ...
  }, Math.max(1000, IRIS_RT_DISPATCH_HEARTBEAT_MS));
}
...
export async function releaseRuntimeTaskLease(taskKey) {
  await withTaskLock(taskKey, async () => {
    ...
    if (!lease || lease.owner !== IRIS_RT_AGENT) return;
