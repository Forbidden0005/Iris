...
// ── Path constants ─────────────────────────────────────────────────────────
/** Repo root of this checkout (`lib/runtime` → `../..`). Not `process.cwd()` — iris-lead may start from any cwd. */
const _RUNTIME_DIR = path.dirname(fileURLToPath(import.meta.url));
export const IRIS_REPO_ROOT =
  (process.env.IRIS_REPO_ROOT && String(process.env.IRIS_REPO_ROOT).trim())
    ? path.resolve(process.env.IRIS_REPO_ROOT)
    : path.resolve(_RUNTIME_DIR, "..", "..");

export const LEGACY_STATE_DIR = path.join(os.homedir(), ".openclaw");
export const IRIS_DIR = path.join(os.homedir(), ".iris");
export const IRIS_CONFIG_PATH = path.join(IRIS_DIR, "config.json");
export const IRIS_SWARM_PATH = path.join(IRIS_DIR, "iris.json");
export const TELEGRAM_BRIDGE_CONFIG_PATH = path.join(IRIS_DIR, "telegram-bridge.json");
...
/**
 * Read ~/.iris/iris.json (system/dashboard config)
 * Contains: rt.authToken, claudeCode, providers, opencodeFallbackModel, etc.
 * @returns {SystemConfig}
 */
export function loadSystemConfig() {
  return _tryReadJSON(IRIS_CONFIG_PATH) || {};
}

/**
 * Read ~/.iris/iris.json (agent definitions, providers, env)
 * Contains: agents[], providers{}, env{}, globalFallbackModel, etc.
 * @returns {SwarmConfig}
 */
export function loadSwarmConfig() {
  return _tryReadJSON(IRIS_SWARM_PATH) || {};
}

/**
 * Load agent list from iris.json (with legacy fallback)
...
export const IRIS_RT_URL = process.env.IRIS_RT_URL || "ws://127.0.0.1:18889";
export const IRIS_RT_AGENT = process.env.IRIS_RT_AGENT || "iris-main";

export function getRTToken() {
  let token = process.env.IRIS_RT_AUTH_TOKEN || "";
  if (!token) {
    const cfg = loadSystemConfig();
    token = cfg?.rt?.authToken || cfg?.env?.IRIS_RT_AUTH_TOKEN || "";
  }
  return typeof token === "string" ? token.trim() : "";
}

export const IRIS_RT_TOKEN = getRTToken();
export const IRIS_RT_CHANNELS = (process.env.IRIS_RT_CHANNELS || "command,assign,handoff,reassign,events")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
export const IRIS_RT_TLS_INSECURE = process.env.IRIS_RT_TLS_INSECURE === "1";
export const IRIS_RT_RECONNECT_MS = Number(process.env.IRIS_RT_RECONNECT_MS || "1500");
export const IRIS_RT_DISPATCH_ENABLED = (process.env.IRIS_RT_DISPATCH_ENABLED || "1") !== "0";
export const IRIS_RT_DISPATCH_LEASE_MS = Number(process.env.IRIS_RT_DISPATCH_LEASE_MS || "45000");
export const IRIS_RT_DISPATCH_HEARTBEAT_MS = Number(process.env.IRIS_RT_DISPATCH_HEARTBEAT_MS || "10000");
export const IRIS_RT_DISPATCH_MAX_RETRIES = Number(process.env.IRIS_RT_DISPATCH_MAX_RETRIES || "2");
export const IRIS_RT_DISPATCH_MAX_RETRIES_CODING = Number(process.env.IRIS_RT_DISPATCH_MAX_RETRIES_CODING || "3");
export const IRIS_RT_DISPATCH_RETRY_BACKOFF_MS = Number(process.env.IRIS_RT_DISPATCH_RETRY_BACKOFF_MS || "2000");
export const IRIS_OPENCODE_ENABLED = (process.env.IRIS_OPENCODE_ENABLED || "1") !== "0";
export const IRIS_CURSOR_WAVES = process.env.IRIS_CURSOR_WAVES === "1";
export const IRIS_CLAUDE_CODE = (() => {
  if (process.env.IRIS_CLAUDE_CODE) return /^1|true|yes$/i.test(String(process.env.IRIS_CLAUDE_CODE));
  const cfg = loadSystemConfig();
  if (typeof cfg.claudeCode === "boolean") return cfg.claudeCode;
  return false;
})();
export const IRIS_OPENCODE_FORCE = process.env.IRIS_OPENCODE_FORCE === "1";
export const IRIS_OPENCODE_BIN = process.env.IRIS_OPENCODE_BIN || path.join(os.homedir(), ".opencode", "bin", "opencode");
export const IRIS_OPENCODE_AGENT = process.env.IRIS_OPENCODE_AGENT || "admin";
export const IRIS_OPENCODE_MODEL = process.env.IRIS_OPENCODE_MODEL || "openai/gpt-5.4";
export const IRIS_OPENCODE_FALLBACK_DEFAULT = "groq/llama-3.3-70b-versatile";
export const IRIS_OPENCODE_TIMEOUT_MS = Number(process.env.IRIS_OPENCODE_TIMEOUT_MS || "300000");
...
export const ENGINES_USER_DIR = path.join(os.homedir(), ".iris", "engines");
...
// ── Shared memory paths ─────────────────────────────────────────────────────
export const SHARED_MEMORY_BASE = process.env.SHARED_MEMORY_DIR || path.join(os.homedir(), ".iris", "workspace", "shared-memory");
export const SHARED_MEMORY_NAMESPACE = process.env.SHARED_MEMORY_NAMESPACE || "claw-swarm";
export const SWARM_STATUS_LOG = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "channels", "status.jsonl");
export const SWARM_DISPATCH_DIR = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "dispatch");
export const SWARM_DLQ_DIR = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "dlq");
export const SWARM_HEARTBEAT_WINDOW_SEC = Number(process.env.IRIS_RT_HEARTBEAT_WINDOW_SEC || "90");
export const IRIS_RT_TASK_LEASE_MS = Number(process.env.IRIS_RT_TASK_LEASE_MS || "120000");
export const IRIS_RT_TASK_HEARTBEAT_MS = Number(process.env.IRIS_RT_TASK_HEARTBEAT_MS || "15000");
export const IRIS_RT_TASK_RETRY_MAX = Number(process.env.IRIS_RT_TASK_RETRY_MAX || "2");
export const IRIS_RT_TASK_STATE_TTL_MS = Number(process.env.IRIS_RT_TASK_STATE_TTL_MS || "21600000");
export const SWARM_RUNTIME_DIR = path.join(SHARED_MEMORY_BASE, SHARED_MEMORY_NAMESPACE, "openiris-rt", "runtime");
...
// ── Misc constants ─────────────────────────────────────────────────────────
export const IRIS_RT_COMMAND_TYPES = new Set([
  "command.spawn_agent",
  "command.run_task",
  "command.cancel_task",
...
// ── iris-lead.mjs constants ───────────────────────────────────────────────
export const IRIS_LEAD_PORT = Number(process.env.IRIS_LEAD_PORT || 5010);
export const IRIS_LEAD_PID_PATH = path.join(os.homedir(), ".iris", "logs", "iris-lead.pid");
export const IRIS_LEAD_HISTORY_DIR = path.join(os.homedir(), ".iris", "chat-history");
export const PROJECTS_REGISTRY = path.join(path.dirname(new URL(import.meta.url).pathname), "..", "iris-lead", "orchestrator-logs", "projects.json");
...
export const DASH_PORT = Number(process.env.IRIS_DASH_PORT || 4319);
export const DASH_HOST = process.env.IRIS_RT_HOST || "127.0.0.1";
export const DASHBOARD = `http://${DASH_HOST}:${DASH_PORT}`;
export const DISPATCH_TIMEOUT_MS = Number(process.env.IRIS_DISPATCH_TIMEOUT_MS) || 300_000;
export const DISPATCH_CLAIMED_TIMEOUT_MS = Number(process.env.IRIS_DISPATCH_CLAIMED_TIMEOUT_MS) || 900_000;

export function loadCursorWavesEnabled() {
  if (process.env.IRIS_CURSOR_WAVES) return /^1|true|yes$/i.test(String(process.env.IRIS_CURSOR_WAVES));
  const cfg = loadSystemConfig();
...
export function loadTmuxBridgeEnabled() {
  if (process.env.IRIS_TMUX_BRIDGE) return /^1|true|yes$/i.test(String(process.env.IRIS_TMUX_BRIDGE));
  const cfg = loadSystemConfig();
...
export function resolveConfig() {
  const paths = [IRIS_CONFIG_PATH, path.join(LEGACY_STATE_DIR, "openclaw.json")];
...
export function loadAgentLLMConfig(ocAgentId) {
  try {
    ...
    const crewId = ocAgentId.startsWith("iris-") ? ocAgentId : `iris-${ocAgentId}`;
    const bareId = ocAgentId.startsWith("iris-") ? ocAgentId.slice(5) : ocAgentId;
    ...
    if (!provider?.baseUrl || !provider?.apiKey) {
      console.warn(`[bridge] No provider config for "${providerKey}" (agent ${ocAgentId}) — check ~/.iris/iris.json providers`);
      return null;
    }
...
export function loadLoopBrainConfig() {
  try {
    const cfg = JSON.parse(fs.readFileSync(IRIS_CONFIG_PATH, "utf8"));
    const loopBrain = cfg.loopBrain || process.env.IRIS_LOOP_BRAIN || null;
