...
const DEFAULT_TIMEOUT_MS = Number(
  process.env.IRIS_TWITTER_FETCH_TIMEOUT_MS || 20_000,
);
const DEFAULT_MAX_LINKS = Number(
  process.env.IRIS_TWITTER_FETCH_MAX_LINKS || 2,
);
const DEFAULT_CACHE_TTL_MS = Number(
  process.env.IRIS_TWITTER_FETCH_CACHE_MS || 10 * 60 * 1000,
);
...
function resolveTwitterCommands() {
  ...
  const configuredBin = process.env.IRIS_TWITTER_CLI_BIN || "";
  const configuredArgs = splitArgs(process.env.IRIS_TWITTER_CLI_ARGS || "");
  const candidates = [];
...
async function fetchTweetPayload(link, timeoutMs) {
  ...
  throw new Error(
    "twitter CLI not found; set IRIS_TWITTER_CLI_BIN or install twitter-cli",
  );
}
