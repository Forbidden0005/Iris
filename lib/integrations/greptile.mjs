 ...
 * Semantic codebase search and RAG for iris agents
 */
...
export async function indexRepository(repository, branch = "main", remote = "github") {
  ...
  if (!apiKey) {
    throw new Error("Greptile API key not found. Set GREPTILE_API_KEY or add to ~/.iris/iris.json");
  }
...
/**
 * Helper: Index iris repository
 */
export async function indexiris() {
  return await indexRepository("iris/iris", "main");
}

/**
 * Helper: Search iris codebase
 */
export async function searchiris(query) {
  return await queryRepositories(query, [
    { remote: "github", repository: "iris/iris", branch: "main" }
  ]);
}
