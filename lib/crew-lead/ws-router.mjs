        ...
        ws.on("message", (raw) => {
            ...
            if (p.type === "message" && p.envelope) {
                ...
                if (env.channel === "issues" && (msgType === "task.failed" || env.type === "task.failed")) {
                    ...
                    // dispatch time). Nothing outside the explicit
                    // dashboard dispatch path sets those yet, so this has
                    // no effect on existing failure handling today.
                    // Symmetric with the task.done hook below, and fires
                    // once here regardless of whatever retry/fallback
                    // decision the rest of this block makes.
                ...
                if (isDone && content && from && from !== "crew-lead") {
                    ...
                    if (dispatch) {
                        ...
                        // no-op unless dispatch already carries
                        // irisPlanId/irisTaskId (set via pipelineMeta at
                        // dispatch time). Nothing sets those yet, so this
                        // has no effect on existing dispatch today.
                        if (dispatch.irisPlanId && dispatch.irisTaskId) {
                            try {
                                attachEvidenceFromDispatchCompletion(dispatch.irisPlanId, dispatch.irisTaskId, {
                                    taskKey: taskId,
