...
const _AUTO_APPROVE_STATIC = new Set(["iris-fixer", "iris-github", "iris-pm"]);
...
export const AGENT_TOOL_ROLE_DEFAULTS = {
  'iris-qa':          new Set(['read_file','skill']),
  'iris-coder':       new Set(['write_file','read_file','mkdir','run_cmd','skill','define_skill','browser']),
  'iris-coder-front': new Set(['write_file','read_file','mkdir','run_cmd','skill','browser']),
  'iris-coder-back':  new Set(['write_file','read_file','mkdir','run_cmd','skill','browser']),
  'iris-frontend':    new Set(['write_file','read_file','mkdir','run_cmd','skill']),
  'iris-fixer':       new Set(['write_file','read_file','mkdir','run_cmd','skill','browser']),
  'iris-github':      new Set(['read_file','run_cmd','git','skill']),
  'iris-pm':          new Set(['read_file','write_file','mkdir','dispatch','skill']),
  'iris-main':        new Set(['read_file','write_file','run_cmd','dispatch','skill','define_skill']),
  'iris-security':    new Set(['read_file','run_cmd']),
  'iris-copywriter':  new Set(['write_file','read_file','skill']),
  'iris-telegram':    new Set(['telegram','read_file']),
  'iris-lead':        new Set(['read_file','write_file','mkdir','run_cmd','web_search','web_fetch','skill','define_skill','dispatch','telegram','whatsapp']),
};

// iris @@TOOL permission names — distinct from legacy gateway tool names
const IRIS_TOOL_NAMES = new Set(['write_file','append_file','read_file','mkdir','run_cmd','git','dispatch','skill','define_skill','telegram','web_search','web_fetch','browser']);
...
export function loadAgentToolPermissions(agentId) {
  // Check config files for explicit iris-style tool permissions.
  // tools.alsoAllow in iris.json may contain legacy gateway tool names (exec, web_search, etc.)
  // — only use it if it contains at least one iris @@TOOL name.
  try {
    const cfgPaths = [
    ...
    for (const p of cfgPaths) {
      ...
      const crewId = agentId.startsWith("iris-") ? agentId : `iris-${agentId}`;
      const bareId = agentId.startsWith("iris-") ? agentId.slice(5) : agentId;
      const agent = agents.find(a => a.id === agentId || a.id === crewId || a.id === bareId);
      // Only accept if the list contains iris-style tool names, not just legacy gateway names
      const allow = agent?.tools?.irisAllow || agent?.tools?.alsoAllow || [];
      const irisTools = allow.filter(t => IRIS_TOOL_NAMES.has(t));
      if (irisTools.length > 0) {
        return new Set([...(getRoleDefaultPermissions(agentId) || []), ...irisTools]);
      }
    }
  } catch {}
  // Fall back to role defaults (covers iris-coder, iris-qa, iris-fixer, etc.)
  const roleDefaults = getRoleDefaultPermissions(agentId);
  if (roleDefaults) return roleDefaults;
  // Dynamic agents: derive tools from _role in iris.json
  try {
    const agents = _loadAgentList();
    const cfg = agents.find(a => a.id === agentId);
    if (cfg?._role) {
      ...
      // Custom role: check iris.json top-level roleToolDefaults map
      // e.g. "roleToolDefaults": { "analyst": ["read_file","web_search","skill"] }
      try {
        const rootCfg = JSON.parse(fs.readFileSync(path.join(os.homedir(), ".iris", "iris.json"), "utf8"));
...
keyFrom format: "providers.PROVIDER.apiKey" (reads from iris.json) or "env.ENV_VAR_NAME".`);
    }
  }
  ...
  const externalProjectHint =
    projectDir === process.cwd()
      ? `- If the task refers to an external project by name (e.g. polymarket-ai-strat), its root is typically ${path.join(os.homedir(), "Desktop", "<project-name>")}, not under PROJECT DIRECTORY. Do not use paths like .../iris/<project-name>/...; use .../Desktop/<project-name>/... instead.`
      : "";
...
// Allowlist — patterns stored in ~/.iris/cmd-allowlist.json
const CMD_ALLOWLIST_FILE = path.join(os.homedir(), ".iris", "cmd-allowlist.json");
...
export function sanitizeToolPath(raw) {
  ...
  // that output bare filenames don't accidentally write to the iris root.
...
export async function executeToolCalls(reply, agentId, { suppressWriteIfSearchPending = false, taskId = null, projectId = null } = {}) {
  ...
  while ((m = webSearchRe.exec(reply)) !== null) {
    ...
    try {
      ...
      const braveKey = (() => {
        // Check iris.json first, then search-tools.json, then env var
        try {
        ...
        const stPaths = [
          path.join(IRIS_DIR, "search-tools.json"),
      ...
      results.push(`[tool:web_search] ❌ No search provider available (no Brave or Perplexity key in iris.json or search-tools.json)`);
    } catch (err) {
  ...
  while ((m = webFetchRe.exec(reply)) !== null) {
    ...
    try {
      const res = await fetch(url, {
        headers: { "User-Agent": "iris/1.0 (agent fetch)" },
  ...
  while ((m = telegramRe.exec(reply)) !== null) {
    ...
    try {
      ...
      chatId = chatId.trim();
      if (!botToken || !chatId) {
        results.push(`[tool:telegram] ❌ TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID must be set in env, ~/.iris/iris.json, or ~/.iris/telegram-bridge.json (token + allowedChatIds or defaultChatId)`);
        continue;
      }
