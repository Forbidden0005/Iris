/**
 * Pipeline / project manager — extracted from iris-lead.mjs
...
function getPMLLMProviders() {
  const csSwarm = tryRead(path.join(os.homedir(), ".iris", "iris.json")) || {};
...
> Generated by iris on ${today}. Update this file as the project evolves.
...
- \`qa-report.md\` — QA findings (written by iris-qa)
`;
}
...
\`\`\`
[System diagram will be provided by iris-architect]
...
> Managed by iris PM Loop. Add \`- [ ] items\` here at any time.
...
async function generateRoadmarkWithAI(name, description, outputDir) {
  ...
  if (!providers.length) {
    console.log("[iris-lead] No PM LLM providers configured — using template roadmap");
...
Generate the ROADMAP.md:`;
  ...
  for (const pmCfg of providers) {
    ...
    console.log(`[iris-lead] Generating roadmap via ${pmCfg.name || pmCfg.model}...`);
    try {
      ...
      if (!resp.ok) {
        ...
        console.warn(`[iris-lead] ${pmCfg.name} returned ${resp.status} — trying next provider`);
        continue;
      }

      const data = await resp.json();
      const roadmap = data?.choices?.[0]?.message?.content?.trim();
      if (!roadmap) { console.warn(`[iris-lead] ${pmCfg.name} returned empty — trying next`); continue; }

      console.log(`[iris-lead] Roadmap generated via ${pmCfg.name} (${roadmap.length} chars)`);
      return roadmap.startsWith("#") ? roadmap : `# ${name} — Living Roadmap\n\n${roadmap}`;
    } catch (e) {
      console.warn(`[iris-lead] ${pmCfg.name} failed: ${e.message} — trying next provider`);
    }
  }

  console.warn("[iris-lead] All PM LLM providers failed — using template roadmap");
  return templateRoadmap(name, description, outputDir);
}
...
export async function draftProject({ name, description, outputDir }, sessionId) {
  ...
  console.log(`[iris-lead] Roadmap draft ready: ${name} (draftId=${draftId})`);
...
export async function confirmProject({ draftId, roadmapMd: overrideMd }) {
  ...
  // Write PDD.md if it doesn't already exist (iris-pm may write a richer version during planning)
  ...
  // Write TECH-SPEC.md template (iris-pm + iris-architect will fill it out during planning)
  ...
  console.log(`[iris-lead] Project confirmed: ${name} (${projectId}) — roadmap + PDD + TECH-SPEC written`);
  ...
  try {
    ...
    console.log(`[iris-lead] PM loop started for ${name}:`, startData.pid || startData.message);
  } catch (e) {
    console.error(`[iris-lead] PM loop start failed: ${e.message}`);
  }
...
Plan and execute this phase as a @@PIPELINE. Use the correct agents for each task. End with iris-qa → iris-fixer → iris-qa → iris-pm (ROADMAP update). All file paths must be absolute.`;
