/**
 * Shared Memory Adapter — bridges CLI's AgentKeeper/AgentMemory with main iris
 * 
 ...
 * 
 * Storage: IRIS_MEMORY_DIR env var or ~/.iris/shared-memory/
 */
...
const CLI_MEMORY_PATH = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../iris-cli/dist/memory.mjs');
...
try {
  ...
  console.warn('[shared-adapter] CLI memory bundle not found — shared memory disabled. Run: cd iris-cli && npm run build');
  AgentKeeper = null;
  AgentMemory = null;
  MemoryBroker = null;
  Collections = null;
...
export const IRIS_MEMORY_DIR = process.env.IRIS_MEMORY_DIR || path.join(os.homedir(), '.iris', 'shared-memory');
 ...
 */
export function getAgentMemory(agentId = 'iris-lead') {
...
export function getMemoryBroker(projectDir, options = {}) {
  ...
  return new MemoryBroker(projectDir || process.cwd(), {
    storageDir: IRIS_MEMORY_DIR,
    crewId: options.crewId || 'iris-lead'
  });
}
...
export async function recallMemoryContext(projectDir, query, options = {}) {
  const broker = getMemoryBroker(projectDir, { crewId: options.crewId || 'iris-lead' });
 ...
 */
export function getMemoryStats(agentId = 'iris-lead') {
  const memory = getAgentMemory(agentId);
  if (!memory) return null;
  return memory.stats();
}
...
export function initSharedMemory() {
  try {
    fs.mkdirSync(IRIS_MEMORY_DIR, { recursive: true });
    fs.mkdirSync(path.join(IRIS_MEMORY_DIR, '.iris'), { recursive: true });
    fs.mkdirSync(path.join(IRIS_MEMORY_DIR, '.iris', 'agent-memory'), { recursive: true });
    
    return { ok: true, path: IRIS_MEMORY_DIR };
  } catch (err) {
    return { ok: false, path: IRIS_MEMORY_DIR, error: err.message };
  }
}
 ...
 */
export async function migrateBrainToMemory(brainPath, agentId = 'iris-lead') {
  const memory = getAgentMemory(agentId);
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const agentMatch = factContent.match(/\b(iris-\w+)\b/);
