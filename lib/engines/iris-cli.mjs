...
/**
 * Lazy-load the iris-cli engine API (dist/engine.mjs).
 * This gives us direct access to runAgenticWorker + Sandbox
 * without spawning a subprocess.
 */
async function getEngine() {
  if (_engineModule) return _engineModule;
  const enginePath = path.join(IRIS_REPO_ROOT, "iris-cli", "dist", "engine.mjs");
  if (!fs.existsSync(enginePath)) {
    throw new Error(
      `iris-cli engine not found at ${enginePath}. Run: cd iris-cli && npm run build`
    );
  }
...
/**
 * Load providers from iris config (for API keys)
 */
...
/**
 * Run a task through iris-cli's agentic executor — DIRECT function call.
...
export async function runCrewCLITask(prompt, payload = {}) {
  ...
  const agentId = String(
    payload?.agentId || payload?.agent || IRIS_RT_AGENT || ""
  );
  ...
  let model = String(
    payload?.crewCliModel ||
      ...
      process.env.IRIS_CREW_CLI_MODEL ||
      payload?.model ||
      agentConfig.model ||
      ""
  );
  ...
  console.log(`[iris-cli engine] Task for ${agentId} | model: ${model || "auto"} | dir: ${projectDir}`);
  ...
  const result = await engine.runAgenticWorker(prompt, sandbox, {
    model: model || process.env.IRIS_EXECUTION_MODEL || "gemini-2.5-flash",
    maxTurns: 25,
    projectDir,
    verbose: Boolean(process.env.IRIS_VERBOSE || process.env.IRIS_DEBUG),
  });
  ...
  if (sandbox.hasChanges()) {
    ...
    await sandbox.apply();
    console.log(
      `[iris-cli engine] Applied ${pendingPaths.length} file(s): ${pendingPaths.join(", ")}`
    );
  }
  ...
  console.log(
    `[iris-cli engine] ✅ Done in ${turns} turns ($${cost}) — tools: ${tools}`
  );
...
/**
 * Check if iris-cli engine is available (dist/engine.mjs exists)
 */
