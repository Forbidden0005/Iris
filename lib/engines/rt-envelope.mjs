...
export async function handleRealtimeEnvelope(envelope, client, bridge) {
  ...
  if (to !== "broadcast" && to !== IRIS_RT_AGENT) {
    client.ack({
      messageId: envelope.id,
      status: "skipped",
      note: `not for us (to=${to}, we=${IRIS_RT_AGENT})`,
    });
    return;
  }

  if (!IRIS_RT_COMMAND_TYPES.has(incomingType)) {
    client.ack({
      messageId: envelope.id,
      status: "skipped",
  ...
  // ── cmd approval resolution (from iris-lead via RT bus) ───────────────────
  if (incomingType === "cmd.approved" || incomingType === "cmd.rejected") {
    ...
    if (approvalId && pendingCmdApprovals.has(approvalId)) {
      ...
      console.log(
        `[${IRIS_RT_AGENT}] cmd ${incomingType === "cmd.approved" ? "✅ approved" : "⛔ rejected"}: ${approvalId}`,
      );
    }
    try {
      client.ack({
  ...
  if (dispatchGuardEnabled) {
    try {
      dispatchClaim = acquireTaskLease({
        ...
        incomingType,
        from,
        leaseMs: IRIS_RT_DISPATCH_LEASE_MS,
      });
    } catch (err) {
    ...
    client?.publish({
      ...
      type: "agent_working",
      to: "broadcast",
      payload: { agent: IRIS_RT_AGENT, ts: Date.now() },
    });
  }
  ...
  client.ack({
    messageId: envelope.id,
    status: "received",
    note: `iris accepted ${incomingType}`,
  });
  client.publish({
    channel: "status",
  ...
  try {
    ...
    // CRITICAL: Engine selection must use IRIS_RT_AGENT (the agent this gateway represents),
    ...
    const enrichedPayload = {
      ...payload,
      agent: IRIS_RT_AGENT,
      agentId: IRIS_RT_AGENT,
    };
    console.error(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    console.error(`[${IRIS_RT_AGENT}] 🔍 ENGINE SELECTION START`);
    console.error(`[${IRIS_RT_AGENT}] TaskID: ${taskId}`);
    console.error(`[${IRIS_RT_AGENT}] IncomingType: ${incomingType}`);
    console.error(`[${IRIS_RT_AGENT}] Agent: ${IRIS_RT_AGENT}`);
    console.error(
      `[${IRIS_RT_AGENT}] Payload flags:`,
      JSON.stringify(
        {
    ...
    if (selectedEngine) {
      console.error(
        `[${IRIS_RT_AGENT}] 🚀 ROUTING TO: ${selectedEngine.label || selectedEngine.id}`,
      );
      progress(
      ...
      console.error(
        `[${IRIS_RT_AGENT}] ⚠️  NO ENGINE MATCHED — FALLBACK TO OPENCODE OR DIRECT LLM`,
      );
    ...
    if (selectedEngine && selectedEngine.run) {
      ...
      if (!fs.existsSync(projectDir)) {
        console.error(
          `[${IRIS_RT_AGENT}] engine cwd: path missing (${projectDir}), using process.cwd()`,
        );
      ...
      const agentCfg = getAgentOpenCodeConfig(IRIS_RT_AGENT);
      modelUsed = payload?.model || agentCfg.model || "unknown";

      // RT `command.run_task` often omits per-agent CLI fields; merge from iris.json
      ...
      try {
        ...
        if (
          ...
          console.error(
            `[${IRIS_RT_AGENT}] 📞 CALLING ENGINE: ${selectedEngine.id}.run()`,
          );
          console.error(
            `[${IRIS_RT_AGENT}] 📦 Model being sent: ${modelUsed}`,
          );
          reply = await selectedEngine.run(enginePrompt, enginePayload);
          console.error(
            `[${IRIS_RT_AGENT}] ✅ ENGINE RETURNED: ${reply?.length || 0} chars`,
          );
        }
      } catch (e) {
        ...
        if (isUsageLimit) {
          ...
          const needsEngineExecution =
            agentMustNotUseEngineLlmFallback(IRIS_RT_AGENT);
          const globalDisable = /^1|true|yes$/i.test(
            String(process.env.IRIS_DISABLE_ENGINE_FALLBACK || ""),
          );
          const disableFallback = globalDisable || needsEngineExecution;

          console.error(
            `[${IRIS_RT_AGENT}] ❌ ENGINE ${selectedEngine.id} FAILED: ${msg.slice(0, 200)}`,
          );
          console.error(
            `[${IRIS_RT_AGENT}] Agent type: ${needsEngineExecution ? "TOOL/ENGINE (fail-fast)" : "CONVERSATIONAL (can fallback)"}`,
          );
          console.error(
            `[${IRIS_RT_AGENT}] Fallback disabled: ${disableFallback}`,
          );
          ...
          if (disableFallback) {
            ...
            reply = `❌ **Engine failed**: ${selectedEngine.label} error:\n\n${msg}\n\n${needsEngineExecution ? "(This agent needs the engine for real file/CLI work — fix Codex/Cursor/OpenCode, or switch engine in Settings. No LLM-only fallback.)" : "(Fallback disabled via IRIS_DISABLE_ENGINE_FALLBACK=1)"}`;
          } else {
            // Standard fallback with warning (only for non-coding agents)
            console.error(
              `[${IRIS_RT_AGENT}] 🔄 FALLING BACK TO DIRECT LLM`,
            );
            progress(
      ...
      // engine: "opencode", or IRIS_OPENCODE_FORCE=1). Otherwise use direct LLM
      // so agents without a coding engine still get responses via their configured model.
      const ocAgentCfg = getAgentOpenCodeConfig(IRIS_RT_AGENT);
      const forceOpenCode = /^1|true|yes$/i.test(
        String(process.env.IRIS_OPENCODE_FORCE || ""),
      );
      ...
      try {
        ...
        const agentCfg = agents.find((a) => a.id === IRIS_RT_AGENT);
        if (agentCfg?.useOpenCode === false) {
          console.error(
            `[${IRIS_RT_AGENT}] ✋ Config explicitly disables OpenCode (useOpenCode: false) — using direct LLM`,
          );
      ...
      console.error(
        `[${IRIS_RT_AGENT}] ⚠️  FALLBACK PATH: No engine selected. agentWantsOpenCode=${agentWantsOpenCode}, forceOpenCode=${forceOpenCode}, ocEnabled=${ocAgentCfg.enabled}`,
      );
      ...
      if (agentWantsOpenCode || forceOpenCode) {
        ...
        modelUsed =
          payload?.model || ocAgentCfg.model || IRIS_OPENCODE_MODEL;
        let opencodeErr;
        ...
        if (ocAgentCfg.loop) {
          ...
          try {
            ...
            console.error(`[rt-envelope] OpenCode error:`, {
              ...
              taskId,
              agentId: IRIS_RT_AGENT,
              model: payload?.model || IRIS_OPENCODE_MODEL,
            });
            ...
            if (disableFallback && (isRateLimit || isTimeout)) {
              ...
              reply = `❌ **OpenCode failed**: ${msg}\n\n(Fallback disabled via IRIS_DISABLE_ENGINE_FALLBACK=1)`;
            } else if (isRateLimit || isTimeout) {
              const primaryModel = String(
                payload?.model || IRIS_OPENCODE_MODEL,
              );
              const configFallback =
                getAgentOpenCodeConfig(IRIS_RT_AGENT).fallbackModel;
            ...
            if (!reply) {
              ...
              const ocAgentId =
                RT_TO_GATEWAY_AGENT_MAP[IRIS_RT_AGENT] || "main";
        ...
        // When a gateway process handles a task for iris-researcher, it needs
        // iris-researcher's model config (perplexity/sonar), not its own.
        ...
        const ocAgentId = (dispatchedAgent && dispatchedAgent !== "broadcast")
          ? dispatchedAgent
          : IRIS_RT_AGENT;
        const agentSysPrompt = loadAgentPrompts()[ocAgentId] || loadAgentPrompts()[IRIS_RT_AGENT] || null;
        const agentCfg = getAgentOpenCodeConfig(ocAgentId);
        modelUsed = agentCfg?.model || payload?.model || "unknown";

        console.error(
          `[${IRIS_RT_AGENT}] 🧠 Direct LLM route (no coding engine): agent=${ocAgentId}, model=${modelUsed}`,
        );
        ...
        try {
          ...
          console.error(`[${IRIS_RT_AGENT}] Direct LLM failed: ${msg}`);
          ...
          if (bridge?.kind === "gateway") {
            ...
            reply = `❌ **No engine available**: No coding engine is assigned to ${IRIS_RT_AGENT} and direct LLM call failed.\n\nError: ${msg}\n\nTo fix: set \`"engine": "iris-cli"\` (or opencode/cursor/gemini-cli/claude-code) in ~/.iris/iris.json for this agent, or set IRIS_OPENCODE_ENABLED=1 to enable OpenCode globally.`;
          }
        }
        ...
        if (!reply) {
          ...
          if (!reply) {
            reply = `⚠️ **No engine or LLM configured** for ${IRIS_RT_AGENT}.\n\nTo fix, add this agent to ~/.iris/iris.json with either:\n- \`"engine": "iris-cli"\` (or opencode, cursor, gemini-cli, claude-code)\n- \`"model": "provider/model-id"\` for direct LLM routing\n\nOr set an environment variable: IRIS_CREW_CLI_ENABLED=1, IRIS_OPENCODE_ENABLED=1, etc.`;
          }
        }
      }
    }
    ...
    console.log(`[${IRIS_RT_AGENT}] 🔧 Post-LLM: reply=${reply?.length || 0} chars, starting tool execution...`);

    // Execute gateway-side @@TOOL parsing only for direct-LLM style replies.
    // Native code engines (iris-cli/cursor/claude/codex/gemini-cli/opencode) own
    ...
    const toolResults = engineOwnsTools
      ? []
      : await executeToolCalls(reply, IRIS_RT_AGENT, {
        suppressWriteIfSearchPending: true,
        taskId,
        projectId,
      });
    console.log(`[${IRIS_RT_AGENT}] 🔧 Tool execution done: ${toolResults.length} results`);
    if (toolResults.length > 0) {
      ...
      if (hasSearchResults && (!didWriteFile || writeSuppressed)) {
        console.log(`[${IRIS_RT_AGENT}] 🔄 Follow-up synthesis: hasSearch=${hasSearchResults}, didWrite=${didWriteFile}, suppressed=${writeSuppressed}`);
        try {
          const pathGuard = taskProjectDir
            ? `\n\n**Output path rule:** The original task is tied to project directory:\n  ${taskProjectDir}\nYour @@WRITE_FILE path(s) MUST match the file path(s) named in the original task (typically under that directory). Do NOT write to the iris installation repo root or process.cwd() unless the task explicitly requires it.\n`
            : "";
    ...
    if (!validation.valid) {
      ...
      client.publish({
        ...
        to: IRIS_RT_AGENT, // Send feedback to self for learning
        taskId,
        correlationId,
        priority: "high",
        payload: {
    ...
    if (lessonMatches.length > 0) {
      ...
      for (const m of lessonMatches) {
        ...
        try {
          if (projectDir) {
            const projectMemDir = path.join(projectDir, ".iris");
          ...
          console.log(
            `[bridge:${IRIS_RT_AGENT}] @@LESSON → ${projectDir ? path.basename(projectDir) + "/.iris/brain.md" : "lessons.md"}: ${entry.slice(0, 80)}`,
          );
        } catch (e) {
          console.warn(
            `[bridge:${IRIS_RT_AGENT}] @@LESSON write failed: ${e.message}`,
          );
        }
      }
    }
    ...
    // Canonical format: @@DISPATCH {"agent":"iris-coder","task":"..."}
    ...
    const rawDispatches = COORDINATOR_AGENTS.has(IRIS_RT_AGENT)
      ? (() => {
          const results = [];
    ...
    if (rawDispatches.length > 0) {
      for (const { targetAgent, taskText } of rawDispatches) {
        ...
        if (!targetAgent || !taskText || targetAgent === IRIS_RT_AGENT)
          continue;
        try {
          ...
          console.error(
            `[bridge] IRIS_DISPATCH to ${targetAgent} failed:`,
            dispErr?.message,
          );
        }
      }
    ...
    console.log(
      `[${IRIS_RT_AGENT}] 🔍 SAVE CHECKPOINT - projectId:`,
      saveProjectId,
      "typeof saveProjectMessage:",
      typeof saveProjectMessage,
    );
    ...
    if (saveProjectId) {
      try {
        ...
        console.log(
          `[${IRIS_RT_AGENT}] 💾 Calling saveProjectMessage for project:`,
          saveProjectId,
        );
        const savedMessageId = saveProjectMessage(saveProjectId, {
          ...
          metadata: {
            agentName: formatAgentDisplayName(IRIS_RT_AGENT),
            agentEmoji: "🤖",
            taskId,
            engineUsed,
            model: modelUsed,
        ...
        console.log(
          `[${IRIS_RT_AGENT}] ✅ Sub-agent message saved successfully`,
        );
      } catch (e) {
        console.error(
          `[${IRIS_RT_AGENT}] ❌ Failed to save project message:`,
          e.message,
          e.stack,
        );
      }
      ...
      console.log(
        `[${IRIS_RT_AGENT}] ⏭️  Skipping save - no projectId (payload.projectId:`,
        payload?.projectId,
        ")",
      );
    }

    console.log(`[${IRIS_RT_AGENT}] ✅ Publishing task.done for ${taskId} (reply: ${reply?.length || 0} chars)`);
    client.publish({
      channel: "done",
      type: "task.done",
      to: from,
    ...
    if (isSharedMemoryAvailable()) {
      ...
      recordTaskMemory(projectDir, {
        ...
        console.warn(
          `[${IRIS_RT_AGENT}] Failed to record task memory: ${err.message}`,
        );
      });
    }
    ...
    if (shouldRetry) {
      ...
      const retryAfterMs =
        IRIS_RT_DISPATCH_RETRY_BACKOFF_MS * 2 ** dispatchAttempt;
      ...
      setTimeout(() => {
        try {
          client.publish({
            channel: "command",
            type: incomingType,
            to: IRIS_RT_AGENT, // Retry to SELF, not broadcast (prevents 7x amplification)
            taskId,
            priority: "high",
            payload: {
              ...payload,
    ...
    // ── Auto-escalate to iris-fixer when coding agents exhaust retries ─────
    ...
    if (dispatchAttempt >= maxRetries) {
      ...
      const isSelf = IRIS_RT_AGENT === "iris-fixer"; // prevent fixer→fixer loop
      if (ESCALATABLE_AGENTS.has(IRIS_RT_AGENT) && !isSelf) {
        const fixerTaskId = `fixer-escalation-${Date.now()}`;
        const fixerPrompt =
          `⚠️ Auto-escalation from ${IRIS_RT_AGENT} (failed after ${dispatchAttempt + 1} attempts).\n\n` +
        ...
        try {
          client.publish({
            ...
            payload: {
              action: "run_task",
              prompt: fixerPrompt,
              escalatedFrom: IRIS_RT_AGENT,
              parentTaskId: taskId,
            },
          });
          telemetry("task_escalated_to_fixer", {
            fromAgent: IRIS_RT_AGENT,
            taskId,
            fixerTaskId,
          });
          console.log(
            `[${IRIS_RT_AGENT}] ⬆️ Escalated failed task to iris-fixer (${fixerTaskId})`,
          );
        } catch (escErr) {
          console.error(
            `[${IRIS_RT_AGENT}] Escalation to iris-fixer failed:`,
            escErr?.message,
          );
        }
      }
