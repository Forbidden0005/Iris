 ...
 *
 * Drop-in engines: place a JSON file in engines/ or ~/.iris/engines/ with
...
import { runCrewCLITask } from "./iris-cli.mjs";
...
function findAgentConfig(agentId) {
  const canonical = String(agentId || IRIS_RT_AGENT || "").toLowerCase();
  try {
    const agents = _loadAgentList();
    return (
      agents.find((a) => a.id === canonical || a.id === `iris-${canonical}`) || null
    );
  } catch {
    return null;
  }
...
const IRIS_RT_AGENT = process.env.IRIS_RT_AGENT || "iris-main";
const IRIS_OPENCODE_ENABLED = process.env.IRIS_OPENCODE_ENABLED === "1";  // Opt-IN (was opt-OUT)
const IRIS_CURSOR_WAVES = process.env.IRIS_CURSOR_WAVES === "1";
...
const IRIS_OPENCODE_FORCE = process.env.IRIS_OPENCODE_FORCE === "1";
// Legacy fixed timeout — kept for OpenCode. Cursor/Claude use activity-based watchdog instead.
const IRIS_OPENCODE_TIMEOUT_MS = Number(process.env.IRIS_OPENCODE_TIMEOUT_MS || "300000");
// Idle watchdog: kill engine if no stdout/stderr activity for this long (default 5 min).
const IRIS_ENGINE_IDLE_TIMEOUT_MS = Number(process.env.IRIS_ENGINE_IDLE_TIMEOUT_MS || "300000");
// Absolute ceiling for any single engine task — safety net (default 45 min).
const IRIS_ENGINE_MAX_TOTAL_MS = Number(process.env.IRIS_ENGINE_MAX_TOTAL_MS || "2700000");
...
function _isCodexEnabled() {
  if (process.env.IRIS_CODEX === "1") return true;
...
/**
 * `gemini -m` expects a Google model id (e.g. gemini-2.5-flash), not iris roster strings
...
function resolveGeminiCliModelFlag(payload) {
  ...
  const envM = normalizeGeminiCliModelId(process.env.IRIS_GEMINI_CLI_MODEL);
  if (envM) return envM;
...
function resolveCodexCliModel(payload, agentCfg = null) {
  ...
  const envModel = String(process.env.IRIS_CODEX_MODEL || "").trim();
...
export function shouldUseClaudeCode(payload, incomingType) {
  ...
  if (_isClaudeCodeEnabled()) {
    const ocCfg = _getAgentOpenCodeConfig(agentId);
    // Skip if agent explicitly uses iris-cli, codex, gemini-cli, docker-sandbox, or opencode
    ...
    if (
      eng &&
      ["codex", "codex-cli", "cursor", "cursor-cli", "opencode", "gpt5", "gpt-5", "gemini", "gemini-cli", "iris-cli", "crewcli", "docker-sandbox"].includes(eng)
    ) {
      return false;
    }
...
export function shouldUseOpenCode(payload, prompt, incomingType) {
  if (!IRIS_OPENCODE_ENABLED) return false;
  if (IRIS_OPENCODE_FORCE) return true;
  if (shouldUseCursorCli(payload, incomingType)) return false;
...
export function shouldUseGeminiCli(payload, incomingType) {
  ...
  return process.env.IRIS_GEMINI_CLI_ENABLED === "1";
}
...
export function shouldUseCrewCLI(payload, incomingType) {
  ...
  const runtime = String(payload?.runtime || payload?.executor || payload?.engine || "").toLowerCase();
  if (runtime === "iris-cli" || runtime === "crewcli") return true;
  if (payload?.useCrewCLI === true) return true;
  const agentId = String(payload?.agentId || payload?.agent || IRIS_RT_AGENT || "").toLowerCase();
  try {
    const agents = _loadAgentList();
    const cfg = agents.find(a => a.id === agentId || a.id === `iris-${agentId}`);
    if (cfg?.useCrewCLI === true) return true;
  } catch {}
  return process.env.IRIS_CREW_CLI_ENABLED === "1";
}

export async function runGeminiCliTask(prompt, payload = {}) {
  return new Promise((resolve, reject) => {
    const agentId = String(payload?.agentId || payload?.agent || IRIS_RT_AGENT || "");
    ...
    if (model) args.push("-m", model);
      ...
      console.error(
        `[GeminiCli:${agentId}] Omitting -m: roster model "${payload.model}" is not a Gemini CLI id. Set per-agent geminiCliModel (e.g. gemini-2.5-flash) or IRIS_GEMINI_CLI_MODEL — using CLI default.`,
      );
    }
...
// To add a new engine: drop a JSON in engines/ or ~/.iris/engines/ — no code changes needed.
...
export async function runCursorCliTask(prompt, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Kills only when idle for IRIS_ENGINE_IDLE_TIMEOUT_MS or absolute ceiling hit.
...
export async function runCodexTask(prompt, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Match iris-lead engine passthrough (http-server codex path): allow cwd outside a trusted git root.
    const agentCfg = _getAgentOpenCodeConfig(agentId);
...
export function shouldUseDockerSandbox(payload, incomingType) {
  ...
  return process.env.IRIS_DOCKER_SANDBOX === "1";
}
...
export async function runDockerSandboxTask(prompt, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    const sandboxName = process.env.IRIS_DOCKER_SANDBOX_NAME || "iris";
    const innerEngine = (process.env.IRIS_DOCKER_SANDBOX_INNER_ENGINE || "claude").toLowerCase();
    ...
    if (innerEngine === "opencode") {
      innerArgs = ["opencode", "run", titledPrompt, "--model", process.env.IRIS_OPENCODE_MODEL || "openai/gpt-5.4"];
    ...
    const TIMEOUT_MS = parseInt(process.env.IRIS_DOCKER_SANDBOX_TIMEOUT_MS || "300000", 10);
...
export async function runClaudeCodeTask(prompt, payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    const emptyMcpCfg = path.join(os.homedir(), ".iris", "config", "empty-mcp.json");
    const args = [
      "-p",
    ...
    const agentCfg = _getAgentOpenCodeConfig(agentId);
    const model = payload?.claudeCodeModel || agentCfg.claudeCodeModel || process.env.IRIS_CLAUDE_CODE_MODEL || null;
