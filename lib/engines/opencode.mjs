...
export function runOpenCodeTask(prompt, payload = {}) {
  ...
  return new Promise((resolve, reject) => {
    const bin = fs.existsSync(IRIS_OPENCODE_BIN) ? IRIS_OPENCODE_BIN : "opencode";
    ...
    const model = String(payload?.model || agentOcCfg.model || IRIS_OPENCODE_MODEL);
    const OC_AGENT_MAP = {
      "iris-coder":         "coder",
    ...
    const ocAgentName = OC_AGENT_MAP[agentId] || agentId.replace(/^iris-/, "") || payload?.agent || IRIS_OPENCODE_AGENT || "admin";
    ...
    // pick up sentence-ending periods (e.g. "…/iris.") producing an invalid cwd.
    if (!projectDir) {
    ...
    let skipSessionResume = /^1|true|yes$/i.test(
      String(process.env.IRIS_OPENCODE_NO_SESSION_RESUME || ""),
    );
    ...
    const agentLabel = agentId || IRIS_RT_AGENT || "opencode";
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`OpenCode timeout after ${IRIS_OPENCODE_TIMEOUT_MS}ms`));
    ...
    child.on("close", (code) => {
      ...
      // Filter by agentPrefix (e.g. "[iris-coder]") so parallel agents don't steal
