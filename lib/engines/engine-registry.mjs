...
const ENGINES_USER_DIR = path.join(os.homedir(), ".iris", "engines");
...
/**
 * Single preferred engine for this task from iris.json (use* flags + `engine` string).
...
function resolveAgentPreferredEngineId(payload) {
  ...
  try {
    ...
    if (cfg.useCrewCLI === true) return "iris-cli";
...
function evaluateShouldUse(engineDef, payload, incomingType) {
  ...
  if (shouldUse.specialLogic === "orchestrator-waves") {
    ...
    if (agentId === "iris-orchestrator" || agentId === "orchestrator") {
      if (process.env.IRIS_CURSOR_WAVES === "1") return true;
    }
  }
...
/**
 * Select engine: payload flags → payload runtime → per-agent iris.json → fallback rules
 */
export function selectEngine(payload, incomingType) {
  ...
  // PRIORITY 2.3: Per-agent engine from iris.json (`engine` + use* flags) — always respected.
  ...
  if (agentId && _loadAgentList) {
    try {
      const agents = _loadAgentList() || [];
      const agentCfg = agents.find(a => a.id === agentId || a.id === `iris-${agentId}`);
