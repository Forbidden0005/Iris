...
iris-cli includes three benchmark suites that measure execution quality across models and providers.
...
IRIS_EXECUTION_MODEL=ollama:glm-5.1:cloud node scripts/benchmark-quality.mjs

# OAuth models
IRIS_NO_OAUTH=false IRIS_EXECUTION_MODEL=claude-opus-4-6 node scripts/benchmark-quality.mjs

# Custom timeout (default 600s)
IRIS_BENCHMARK_TIMEOUT=300 IRIS_EXECUTION_MODEL=gemini-2.5-flash node scripts/benchmark-quality.mjs
```

### L3 Executor Only (skip L1/L2 pipeline)

```bash
IRIS_EXECUTION_MODEL=gemini-2.5-flash node scripts/benchmark-l3-executor.mjs
```

### L1 Router Accuracy

```bash
IRIS_EXECUTION_MODEL=gemini-2.5-flash node scripts/benchmark-l1-router.mjs
```
...
```bash
IRIS_EXECUTION_MODEL=ollama:glm-5.1:cloud   # Ollama cloud (free)
IRIS_EXECUTION_MODEL=groq:llama-3.3-70b     # Groq (free)
IRIS_EXECUTION_MODEL=claude:opus-4-6        # Anthropic OAuth
IRIS_EXECUTION_MODEL=gemini:gemini-2.5-flash # Google API key
```

## Full Results

Full benchmark results with pricing: https://iris.ai/benchmarks.html
