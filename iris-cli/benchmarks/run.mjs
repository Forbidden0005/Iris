#!/usr/bin/env node
/**
 * Iris-CLI Benchmark Runner
 *
 * Runs real coding tasks against a fixture Express/TypeScript project
 * to measure the iris-cli pipeline across all tiers (L1-L3).
 *
 * Usage:
...
const IRIS_CLI_DIR = resolve(BENCHMARKS_DIR, '..');
const IRIS_ENTRY = resolve(IRIS_CLI_DIR, 'dist', 'iris.mjs');
...
async function copyFixtureToTmp() {
  const tmpName = `iris-bench-${randomBytes(6).toString('hex')}`;
...
function presetEnv(preset) {
  const base = {
    IRIS_USE_UNIFIED_ROUTER: 'true',
    IRIS_ALLOW_EXECUTE_LOCAL: 'true',
  };

  switch (preset) {
    case 'turbo6':
      return { ...base, IRIS_DUAL_L2_ENABLED: 'false', IRIS_QA_LOOP_ENABLED: 'false' };
    case 'fast6':
      return { ...base, IRIS_DUAL_L2_ENABLED: 'false', IRIS_QA_LOOP_ENABLED: 'true' };
    case 'balanced':
      return { ...base, IRIS_DUAL_L2_ENABLED: 'true', IRIS_QA_LOOP_ENABLED: 'true' };
    case 'quality':
      return { ...base, IRIS_DUAL_L2_ENABLED: 'true', IRIS_QA_LOOP_ENABLED: 'true' };
    default:
      return base;
...
async function runTask(task, workDir, opts) {
  ...
  const args = [IRIS_ENTRY, 'run', '-t', task.prompt, '--json'];
...
async function main() {
  ...
  // Check iris-cli is built
  if (!existsSync(IRIS_ENTRY)) {
    console.error(`iris-cli not built. Run 'npm run build' in ${IRIS_CLI_DIR} first.`);
    console.error(`Expected: ${IRIS_ENTRY}`);
    process.exit(1);
  }
  ...
  for (const task of tasks) {
    ...
    try {
      ...
      if (task.expectedDecision) {
        // Parse decision from the iris-cli JSON envelope in stdout.
  ...
  // Save results JSON
  const outDir = join(IRIS_CLI_DIR, '.iris', 'benchmarks');
