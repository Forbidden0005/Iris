...
const testDir = '/tmp/iris-rag-all-levels-test';
...
try {
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=keyword node ${process.cwd()}/bin/iris.js chat "what does src/auth.ts do?" 2>&1 | head -50`,
...
try {
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=keyword node ${process.cwd()}/bin/iris.js chat "refactor the entire auth system" 2>&1 | head -50`,
    { encoding: 'utf8', maxBuffer: 1024 * 1024, timeout: 30000 }
  );
...
try {
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=keyword node ${process.cwd()}/bin/iris.js run "add rate limiting to auth endpoint" --dry-run 2>&1 | head -50`,
    { encoding: 'utf8', maxBuffer: 1024 * 1024, timeout: 30000 }
  );
...
console.log('  IRIS_RAG_TOKEN_BUDGET=8000 (L1+L3)');
console.log('  IRIS_L2_RAG_TOKEN_BUDGET=4000 (L2 only)');
console.log('  IRIS_RAG_MAX_FILES=10 (L1+L3)');
console.log('  IRIS_L2_RAG_MAX_FILES=5 (L2 only)');
