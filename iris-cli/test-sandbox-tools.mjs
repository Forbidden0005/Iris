#!/usr/bin/env node
/**
 * TEST: Iris-CLI Sandbox + Gemini Tools Integration
 * 
...
function runCrew(task, testDir, extraEnv = {}) {
  const cmd = `node bin/iris.js run -t "${task}" --output "${testDir}" --apply`;
...
async function testSandboxTools() {
  log('\n═══════════════════════════════════════════════════════', BLUE);
  log('🧪 Iris-CLI Sandbox + Tools Test (via CLI)', BLUE);
  log('═══════════════════════════════════════════════════════\n', BLUE);

  const baseDir = mkdtempSync(join(tmpdir(), 'iris-sandbox-test-'));
