 ...
 * COMPLEX TASK TEST
 * 
 * Tests iris-cli with a realistic multi-file task:
...
async function complexTest() {
  ...
  const testDir = mkdtempSync(join(tmpdir(), 'iris-complex-'));
  const crewBin = join(process.cwd(), 'bin/iris.js');
  
  console.log(`📁 Test directory: ${testDir}`);
  console.log(`🔧 Iris CLI: ${crewBin}`);
...
Use proper error handling and include JSDoc comments.`;
    ...
    execSync(`node "${crewBin}" run -t "${task.replace(/"/g, '\\"')}"`, {
      ...
      env: {
        ...
        IRIS_DUAL_L2_ENABLED: useDualL2 ? 'true' : 'false',
      }
    });
