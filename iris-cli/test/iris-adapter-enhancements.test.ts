/**
 * Tests for iris-adapter.ts enhancements:
...
import { GeminiToolAdapter } from '../src/tools/gemini/iris-adapter.js';
...
async function makeTempProject(): Promise<{ dir: string; sandbox: Sandbox; adapter: GeminiToolAdapter }> {
  const dir = await mkdtemp(join(tmpdir(), 'iris-test-'));
  await mkdir(join(dir, '.iris'), { recursive: true });
  const sandbox = new Sandbox(dir);
  const adapter = new GeminiToolAdapter(sandbox);
  return { dir, sandbox, adapter };
}

describe('iris-adapter enhancements', () => {
