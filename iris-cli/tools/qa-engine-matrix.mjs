...
function defaultMatrix() {
  return [
    { name: 'cursor', engine: 'cursor', model: process.env.QA_CURSOR_MODEL || '', agent: 'iris-coder' },
    { name: 'claude-cli', engine: 'claude-cli', model: process.env.QA_CLAUDE_MODEL || '', agent: 'iris-coder' },
    { name: 'codex-cli', engine: 'codex-cli', model: process.env.QA_CODEX_MODEL || '', agent: 'iris-coder' },
    { name: 'gemini-cli', engine: 'gemini-cli', model: process.env.QA_GEMINI_MODEL || '', agent: 'iris-coder' },
    { name: 'opencode', engine: 'opencode', model: process.env.QA_OPENCODE_MODEL || '', agent: 'iris-coder' }
  ];
}
...
async function runCrew(args) {
  return new Promise(resolve => {
    const child = spawn(process.execPath, ['bin/iris.js', ...args], {
      cwd: process.cwd(),
...
async function main() {
  ...
  if (runNegativeControl) {
    ...
    const controlArgs = [
      'dispatch',
      process.env.QA_INVALID_ENGINE_AGENT || 'iris-coder',
