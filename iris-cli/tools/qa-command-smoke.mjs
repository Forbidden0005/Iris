...
const binPath = path.join(repoRoot, 'bin', 'iris.js');
...
async function main() {
  const sandboxDir = await mkdtemp(path.join(tmpdir(), 'iris-cli-smoke-'));
  const cases = [
    ...
    },
    {
      ...
      args: ['dispatch', 'iris-main', 'hello', '--skip-cost-check', '--timeout', '1000', '--gateway', 'http://127.0.0.1:1'],
      code: 1,
