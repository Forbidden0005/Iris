...
test('canUseNativeGeminiToolLoop accepts provider-prefixed Gemini model ids', () => {
  ...
  const originalMode = process.env.IRIS_TOOL_MODE;

  process.env.GEMINI_API_KEY = 'test-key';
  process.env.IRIS_TOOL_MODE = 'auto';
  ...
  if (originalMode === undefined) delete process.env.IRIS_TOOL_MODE;
  else process.env.IRIS_TOOL_MODE = originalMode;
});
...
test('canUseNativeGeminiToolLoop disables native loop in markers mode', () => {
  ...
  process.env.GEMINI_API_KEY = 'test-key';
  process.env.IRIS_TOOL_MODE = 'markers';
