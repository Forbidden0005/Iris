...
test('AgentRouter - dispatch should require agent and task', async () => {
  ...
  await assert.rejects(
    async () => await router.dispatch('iris-coder'),
    { message: 'Agent name and task are required' },
...
test('AgentRouter - getAgentRole should return correct roles', () => {
  ...
  assert.strictEqual(router.getAgentRole('iris-coder'), 'Full Stack Coder');
  assert.strictEqual(router.getAgentRole('iris-qa'), 'Quality Assurance');
  assert.strictEqual(router.getAgentRole('iris-fixer'), 'Bug Fixer');
...
test('AgentRouter - dispatch should auto-inject git context', async () => {
  ...
  try {
    const result = await router.dispatch('iris-coder', 'Implement feature X', {
...
test('AgentRouter - dispatch should forward model/engine/direct/bypass/session metadata', async () => {
  ...
  try {
    const result = await router.dispatch('iris-coder', 'Implement feature Y', {
      gateway: 'http://localhost:5010',
...
test('AgentRouter - dispatch should inject CLI_SYSTEM_PROMPT', async () => {
  ...
  try {
    await router.dispatch('iris-coder', 'Verify prompt injection', {
      gateway: 'http://localhost:5010'
    });

    assert.ok(capturedBody, 'Dispatch body should be captured');
    assert.ok(
      capturedBody.task.includes('You are Gunns') || capturedBody.task.includes('Iris'),
...
test('AgentRouter - dispatch should fail immediately on gateway task error', async () => {
  ...
  try {
    await assert.rejects(
      async () => router.dispatch('iris-coder', 'Do work', {
        gateway: 'http://localhost:5010',
        engine: 'cursor',
        direct: true,
        timeout: '10000'
...
test('AgentRouter - dispatch should fail when done result contains non-zero exit code', async () => {
  ...
  try {
    await assert.rejects(
      async () => router.dispatch('iris-coder', 'Do passthrough work', {
        gateway: 'http://localhost:5010',
        engine: 'cursor',
        direct: true,
        timeout: '10000'
...
test('AgentRouter - dispatch should fail when done result reports success:false', async () => {
  ...
  try {
    await assert.rejects(
      async () => router.dispatch('iris-coder', 'Handle malformed success', {
        gateway: 'http://localhost:5010',
        timeout: '10000'
      }),
...
test('AgentRouter - dispatch should fail on empty done result for direct mode', async () => {
  ...
  try {
    await assert.rejects(
      async () => router.dispatch('iris-coder', 'Expect output', {
        gateway: 'http://localhost:5010',
        direct: true,
        engine: 'cursor',
        timeout: '10000'
...
test('AgentRouter - dispatch should fail on engine provenance mismatch in direct mode', async () => {
  ...
  try {
    await assert.rejects(
      async () => router.dispatch('iris-coder', 'prove routing', {
        gateway: 'http://localhost:5010',
        direct: true,
        engine: 'cursor',
        timeout: '10000'
