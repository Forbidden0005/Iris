...
function makeDeps(opts = {}) {
  ...
  const orchestrator = {
    route: async (_task) => ({ agent: 'iris-fixer' }),
...
async function runAutofixLogic(job, deps, overrides = {}) {
  ...
  try {
    while (iteration < job.config.maxIterations) {
      ...
      const agent = route.agent || 'iris-fixer';
    ...
    const proposalDir = join(job.projectDir, '.iris', 'autofix', 'proposals');
...
before(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'iris-runner-'));
});
