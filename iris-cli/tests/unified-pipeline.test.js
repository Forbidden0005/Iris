...
test('UnifiedPipeline routeOnly maps execute-local to CODE', async () => {
  ...
  assert.equal(out.agent, 'iris-coder');
});
...
test('UnifiedPipeline routeOnly maps execute-parallel to DISPATCH', async () => {
  ...
  assert.equal(out.agent, 'iris-main');
});
...
test('UnifiedPipeline execute execute-parallel runs batched path', async () => {
  ...
  pipeline.l3ExecuteParallel = async () => ({
    success: true,
    results: [{ workUnitId: 'a', persona: 'iris-coder', output: 'A', cost: 0.01 }],
    totalCost: 0.01,
    executionTimeMs: 10
  });
  ...
  assert.match(result.response, /iris-coder/);
...
test('UnifiedPipeline dependency helpers order and batch correctly', () => {
  const pipeline = makePipeline();
  const units = [
    { id: 'a', description: 'a', requiredPersona: 'iris-coder', dependencies: [], estimatedComplexity: 'low', requiredCapabilities: [] },
    { id: 'b', description: 'b', requiredPersona: 'iris-qa', dependencies: ['a'], estimatedComplexity: 'low', requiredCapabilities: [] },
    { id: 'c', description: 'c', requiredPersona: 'iris-fixer', dependencies: ['a'], estimatedComplexity: 'low', requiredCapabilities: [] }
  ];
...
test('UnifiedPipeline normalizeDecision maps CHAT/CODE/DISPATCH shape', () => {
  ...
  // CODE maps to execute-parallel by default (execute-local requires IRIS_ALLOW_EXECUTE_LOCAL=true)
...
test('UnifiedPipeline l3ExecuteParallel injects artifact refs and dependency outputs', async () => {
  ...
  const graph = {
    ...
    totalComplexity: 2,
    requiredPersonas: ['iris-coder', 'iris-qa'],
    estimatedCost: 0.1,
    planningArtifacts: {
      pdd: '# PDD\nspec',
      roadmap: '# ROADMAP\ntasks',
...
test('UnifiedPipeline execute-local runs QA/fixer loop when enabled', async () => {
  const saved = process.env.IRIS_QA_LOOP_ENABLED;
  process.env.IRIS_QA_LOOP_ENABLED = 'true';
  try {
    const pipeline = makePipeline();
    pipeline.l2Orchestrate = async () => ({
      decision: 'execute-local',
    ...
    if (saved === undefined) delete process.env.IRIS_QA_LOOP_ENABLED;
    else process.env.IRIS_QA_LOOP_ENABLED = saved;
  }
});
