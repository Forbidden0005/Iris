/**
 * Unit tests for iris-cli/src/repl/index.ts — helper functions and structures.
 *
 ...
 *
 * Run with: node --import tsx --test iris-cli/tests/unit/repl-commands.test.js
 */
...
describe('AVAILABLE_ENGINES', () => {
  ...
  for (const engine of ['auto', 'cursor', 'claude', 'gemini', 'codex', 'iris-cli']) {
    it(`includes ${engine}`, () => {
...
describe('resolveConfiguredReplModel', () => {
  ...
  it('checks env vars as fallback', () => {
    assert.ok(REPL_SOURCE.includes('IRIS_CHAT_MODEL'));
    assert.ok(REPL_SOURCE.includes('IRIS_ROUTER_MODEL'));
    assert.ok(REPL_SOURCE.includes('IRIS_EXECUTION_MODEL'));
  });
...
describe('listInstalledSkills', () => {
  it('is defined and reads from ~/.iris/skills', () => {
