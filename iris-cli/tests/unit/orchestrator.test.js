/**
 * Unit tests for iris-cli/src/orchestrator/index.ts
 *
 ...
 *
 * Run with: node --import tsx --test iris-cli/tests/unit/orchestrator.test.js
 */
...
function createOrchestrator(overrides = {}) {
  ...
  const restore = () => {
    ...
    delete process.env.IRIS_LEGACY_ROUTER;
  };
...
describe('Orchestrator — agent name normalization', () => {
  ...
  it('route result always has normalized iris-* agent names', async () => {
    try {
      const result = await orch.route('ask the fixer to help');
      assert.ok(result.agent?.startsWith('iris-'), `agent should be normalized: ${result.agent}`);
    } finally {
      restore();
    }
  });
