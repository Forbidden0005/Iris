/**
 * Unit tests for iris-cli/src/memory/agentkeeper.ts
 *
 * Covers:
...
describe('AgentKeeper — record and loadAll', () => {
  ...
  before(() => {
    fs.mkdirSync(path.join(TEST_DIR, '.iris'), { recursive: true });
  ...
  it('record returns an entry with id and timestamp', async () => {
    ...
    assert.equal(entry.agent, 'iris-coder');
  });
...
describe('AgentKeeper — recall similarity', () => {
  ...
  before(async () => {
    fs.mkdirSync(path.join(dir, '.iris'), { recursive: true });
...
describe('AgentKeeper — recall boosts', () => {
  ...
  before(async () => {
    ...
    const storePath = path.join(dir, '.iris', 'agentkeeper.jsonl');
...
describe('AgentKeeper — stats', () => {
  ...
  before(async () => {
    fs.mkdirSync(path.join(dir, '.iris'), { recursive: true });
    keeper = new AgentKeeper(dir, { storageDir: dir, autoCompactEvery: 9999 });
    await keeper.record(makeEntry({ tier: 'planner', agent: 'iris-lead' }));
    await keeper.record(makeEntry({ tier: 'worker', agent: 'iris-coder' }));
    await keeper.record(makeEntry({ tier: 'worker', agent: 'iris-coder' }));
  });
  ...
  it('stats groups by agent', async () => {
    const s = await keeper.stats();
    assert.equal(s.byAgent['iris-lead'], 1);
    assert.equal(s.byAgent['iris-coder'], 2);
  });
