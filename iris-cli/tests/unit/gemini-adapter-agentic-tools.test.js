...
import { GeminiToolAdapter } from '../../src/tools/gemini/iris-adapter.ts';
...
test('ask_user persists pending request files', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'iris-adapter-'));
  try {
    ...
    const latestRaw = await readFile(join(dir, '.iris', 'ask-user-latest.json'), 'utf8');
    ...
    const jsonl = await readFile(join(dir, '.iris', 'ask-user-requests.jsonl'), 'utf8');
...
test('plan mode enter/exit writes state transitions', async () => {
  ...
  try {
    ...
    let state = JSON.parse(await readFile(join(dir, '.iris', 'plan-mode.json'), 'utf8'));
    ...
    state = JSON.parse(await readFile(join(dir, '.iris', 'plan-mode.json'), 'utf8'));
...
test('activate_skill persists deduped skill state', async () => {
  ...
  try {
    ...
    const raw = await readFile(join(dir, '.iris', 'active-skills.json'), 'utf8');
