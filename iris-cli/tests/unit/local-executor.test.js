/**
 * Unit tests for iris-cli src/executor/local.ts
 *
 * Covers:
...
const envKeys = [
  ...
  'IRIS_EXECUTION_MODEL', 'IRIS_CHAT_MODEL', 'IRIS_REASONING_MODEL',
  'IRIS_EXECUTOR_TIMEOUT_MS', 'IRIS_EXECUTION_PROVIDER_ORDER', 'IRIS_NO_OAUTH'
];
...
describe('LocalExecutor — getDefaultModel', () => {
  ...
  it('returns IRIS_EXECUTION_MODEL when set', async () => {
    process.env.IRIS_EXECUTION_MODEL = 'custom-model-v1';
    const executor = new LocalExecutor();
    const model = await callPrivate(executor, 'getDefaultModel');
    assert.equal(model, 'custom-model-v1');
  });

  it('returns IRIS_CHAT_MODEL when IRIS_EXECUTION_MODEL is not set', async () => {
    process.env.IRIS_CHAT_MODEL = 'chat-model-v2';
    const executor = new LocalExecutor();
    const model = await callPrivate(executor, 'getDefaultModel');
    assert.equal(model, 'chat-model-v2');
  });

  it('returns IRIS_REASONING_MODEL as third priority', async () => {
    process.env.IRIS_REASONING_MODEL = 'reason-model-v3';
    const executor = new LocalExecutor();
    const model = await callPrivate(executor, 'getDefaultModel');
  ...
  it('returns gpt-5.4 when only OPENAI_API_KEY is set (no OAuth)', async () => {
    process.env.IRIS_NO_OAUTH = 'true';
  ...
  it('prefers env model over API key detection', async () => {
    process.env.IRIS_EXECUTION_MODEL = 'my-custom-model';
    process.env.OPENAI_API_KEY = 'test-key';
    const executor = new LocalExecutor();
    const model = await callPrivate(executor, 'getDefaultModel');
...
describe('LocalExecutor — getTimeoutMs (via timeoutMs field)', () => {
  ...
  it('reads from IRIS_EXECUTOR_TIMEOUT_MS env var', () => {
    process.env.IRIS_EXECUTOR_TIMEOUT_MS = '120000';
    const executor = new LocalExecutor();
    assert.equal(executor['timeoutMs'], 120000);
  });

  it('falls back to 90000 for invalid values', () => {
    process.env.IRIS_EXECUTOR_TIMEOUT_MS = 'not-a-number';
    const executor = new LocalExecutor();
    assert.equal(executor['timeoutMs'], 90000);
  });

  it('falls back to 90000 for values below 1000', () => {
    process.env.IRIS_EXECUTOR_TIMEOUT_MS = '500';
    const executor = new LocalExecutor();
    assert.equal(executor['timeoutMs'], 90000);
  });
});
...
describe('LocalExecutor — getConfiguredProviderOrder', () => {
  ...
  it('respects IRIS_EXECUTION_PROVIDER_ORDER env var', async () => {
    process.env.IRIS_NO_OAUTH = 'true';
    process.env.IRIS_EXECUTION_PROVIDER_ORDER = 'groq,openai';
    process.env.GROQ_API_KEY = 'test';
    process.env.OPENAI_API_KEY = 'test';
    const executor = new LocalExecutor();
    const order = await callPrivate(executor, 'getConfiguredProviderOrder');
