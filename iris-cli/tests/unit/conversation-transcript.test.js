...
const TEST_DIR = join('/tmp', `iris-transcript-test-${Date.now()}`);
...
test('appendTurn creates JSONL file with one line per turn', async () => {
  ...
  const raw = readFileSync(join(TEST_DIR, '.iris', 'transcript-test-1.jsonl'), 'utf8');
...
test('loadTurns skips corrupt lines gracefully', async () => {
  ...
  await appendFile(join(TEST_DIR, '.iris', 'transcript-test-1.jsonl'), 'NOT VALID JSON\n');
