...
describe('Summon', async () => {
  ...
  it('lists all personas', () => {
    ...
    assert.ok(personas.some(p => p.id === 'iris-qa'));
    assert.ok(personas.some(p => p.id === 'iris-coder-back'));
    assert.ok(personas.some(p => p.id === 'iris-security'));
  });

  it('gets persona by id', () => {
    const qa = getPersona('iris-qa');
    assert.ok(qa);
    assert.equal(qa.id, 'iris-qa');
    assert.ok(qa.systemPromptAddition.includes('test'));
  });

  it('returns undefined for unknown persona', () => {
    const p = getPersona('iris-nonexistent');
