...
const TEST_DIR = join('/tmp', `iris-hooks-test-${Date.now()}`);
...
test('loadHookConfig loads valid hooks.json', async () => {
  await mkdir(join(TEST_DIR, '.iris'), { recursive: true });
  await writeFile(join(TEST_DIR, '.iris', 'hooks.json'), JSON.stringify({
    hooks: {
