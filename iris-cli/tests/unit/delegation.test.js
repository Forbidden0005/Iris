...
describe('DelegationTuner', async () => {
  ...
  describe('rankCandidates', () => {
    it('ranks iris-fixer highest for bug fixes', () => {
      ...
      assert.ok(top.persona === 'iris-fixer' || top.persona === 'executor-code');
    });

    it('ranks iris-qa highest for test tasks', () => {
      const tuner = new DelegationTuner();
      const task = analyzeTask('write tests for the parser', ['src/parser.ts']);
      const candidates = tuner.rankCandidates(task);
      assert.ok(candidates.some(c => c.persona === 'iris-qa'));
      // iris-qa should be ranked high
      const qaRank = candidates.findIndex(c => c.persona === 'iris-qa');
      assert.ok(qaRank < 3, `iris-qa ranked ${qaRank}, expected top 3`);
    });

    it('ranks iris-copywriter highest for docs', () => {
      const tuner = new DelegationTuner();
      const task = analyzeTask('write documentation for the API', ['README.md']);
      const candidates = tuner.rankCandidates(task);
      const top = candidates[0];
      assert.equal(top.persona, 'iris-copywriter');
    });
  ...
  describe('performance history', () => {
    ...
    it('penalizes personas with recent failures', () => {
      ...
      for (let i = 0; i < 3; i++) {
        tuner.recordPerformance({
          persona: 'iris-coder', model: 'x', taskType: 'fix-bug',
      ...
      const candidates = tuner.rankCandidates(task);
      const coder = candidates.find(c => c.persona === 'iris-coder');
      assert.ok(coder);
    ...
    it('exports and imports history', () => {
      const tuner = new DelegationTuner();
      tuner.recordPerformance({
        persona: 'iris-qa', model: 'x', taskType: 'add-test',
  ...
  describe('bestCandidate', () => {
    it('returns top candidate', () => {
      ...
      assert.ok(best);
      assert.equal(best.persona, 'iris-copywriter');
    });
