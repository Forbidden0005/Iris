/**
 * Unit tests for iris-cli src/session/manager.ts
 *
 * Covers:
 *  - SessionManager constructor: sets up paths correctly
 *  - ensureInitialized: creates .iris directory and default files
...
const TEST_BASE_DIR = path.join(os.tmpdir(), `iris-session-test-${process.pid}`);
...
describe('SessionManager — ensureInitialized', () => {
  ...
  it('creates .iris directory and default files', async () => {
    const sm = new SessionManager(TEST_BASE_DIR);
    await sm.ensureInitialized();

    const stateDir = path.join(TEST_BASE_DIR, '.iris');
    assert.ok(fs.existsSync(stateDir), '.iris directory should exist');
  ...
  it('session.json contains a valid UUID sessionId', async () => {
    ...
    const raw = fs.readFileSync(path.join(TEST_BASE_DIR, '.iris', 'session.json'), 'utf8');
  ...
  it('cost.json has correct initial structure', async () => {
    ...
    const raw = fs.readFileSync(path.join(dir, '.iris', 'cost.json'), 'utf8');
    const data = JSON.parse(raw);
...
describe('SessionManager — appendHistory', () => {
  ...
  it('adds an entry to the session history', async () => {
    ...
    await sm.appendHistory({ input: 'hello', output: 'world', route: 'direct', agent: 'iris-coder' });
    ...
    assert.equal(session.history[0].agent, 'iris-coder');
