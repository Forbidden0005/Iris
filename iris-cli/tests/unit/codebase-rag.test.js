...
process.env.IRIS_EMBEDDING_PROVIDER = 'local';
...
describe('CodebaseIndex', () => {
  ...
  test('stats reports initial state', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-stats-'));
  ...
  test('isReady returns false before indexing', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-ready-'));
    const idx = CodebaseIndex.getInstance(dir);
    assert.equal(idx.isReady(), false);
  });

  test('ensureIndex builds and queries local index', async (t) => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-test-'));
    await mkdir(join(dir, '.iris', 'rag-cache'), { recursive: true });
  ...
  test('ensureIndex is incremental — skips unchanged files', async (t) => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-incr-'));
    await mkdir(join(dir, '.iris', 'rag-cache'), { recursive: true });
  ...
  test('index persists to disk and reloads', async (t) => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-persist-'));
    await mkdir(join(dir, '.iris', 'rag-cache'), { recursive: true });
    ...
    assert.ok(existsSync(join(dir, '.iris', 'rag-cache', 'embeddings.json')),
      'embeddings.json should exist');
    assert.ok(existsSync(join(dir, '.iris', 'rag-cache', 'index-meta.json')),
      'index-meta.json should exist');

    const meta = JSON.parse(await readFile(join(dir, '.iris', 'rag-cache', 'index-meta.json'), 'utf8'));
...
describe('autoLoadRelevantFiles', () => {
  ...
  test('keyword mode finds files by grep', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'iris-rag-kw-'));
