...
// Load .env from iris-cli root (needed when spawned by studio or other processes)
...
// Load API keys from ~/.iris/iris.json providers (only if not already set)
...
if (existsSync(swarmCfgPath)) {
  try {
    ...
    // Also load env block (IRIS_CHAT_MODEL, IRIS_EXECUTION_MODEL, etc.)
...
if (args.length === 1 && (args[0] === '--help' || args[0] === '-h')) {
  console.log(`Usage: iris [options] [command]

Commands:
  chat            Chat with Iris (routed to best agent)
...
Run "iris <command> --help" for command-specific options and examples.
...
(async () => {
  try {
    const { main } = await import('../dist/iris.mjs');
    await main(args);
  } catch (error) {
