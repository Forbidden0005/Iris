#!/usr/bin/env node
/**
 * Minimal APPS-style benchmark runner for iris-cli.
 *
...
async function main() {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    const runDir = path.join(os.tmpdir(), `iris-bench-apps-${Date.now()}-${id}`);
  ...
  const outDir = path.join(process.cwd(), '.iris', 'benchmarks');
