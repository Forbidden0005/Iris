...
async function runFullPipeline() {
  ...
  process.env.IRIS_ALLOW_CRITICAL = 'true'; // Bypass policy validator for benchmark
  process.env.IRIS_CHAT_MODEL = 'grok-4-1-fast-reasoning';
  process.env.IRIS_REASONING_MODEL = 'grok-4-1-fast-reasoning';
  process.env.IRIS_EXECUTION_MODEL = 'grok-4-1-fast-reasoning';
  ...
  try {
    ...
    const hasPackageJson = /package\.json|"name":\s*"iris-vscode"/i.test(result.response);
    ...
    console.log('❌ No iris-qa check of all work');
    console.log('❌ No verification that files integrate');
    console.log('❌ No iris-fixer for issues');
    ...
    console.log('2. L2 assigns to iris-qa persona');
    console.log('3. iris-qa audits all work units for:');
    console.log('   - Integration issues');
    console.log('   - Missing dependencies');
    console.log('   - Code quality problems');
    console.log('   - Security issues');
    console.log('4. If issues found:');
    console.log('   → L2 assigns to iris-fixer');
    console.log('   → iris-fixer makes corrections');
    console.log('   → Back to iris-qa for re-check');
    console.log('5. iris-qa signs off ✅');
