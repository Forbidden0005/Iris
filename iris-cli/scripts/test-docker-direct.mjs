...
const TEST_DIR = `/tmp/iris-docker-direct-${Date.now()}`;
...
async function main() {
  ...
  fs.mkdirSync(`${TEST_DIR}/.iris`, { recursive: true });
...
`
  ...
  const tempDir = `/tmp/iris-docker-temp-${Date.now()}`;
