 ...
 *
 * Requires:
 *   - Fixture project at /tmp/iris-tune-bench (git repo with src/math.ts, test/math.test.ts, package.json)
 ...
 *   mkdir -p /tmp/iris-tune-bench && cd /tmp/iris-tune-bench && git init
...
const IRIS_CLI = new URL('..', import.meta.url).pathname.replace(/\/$/, '');
const IRIS_ROOT = new URL('../..', import.meta.url).pathname.replace(/\/$/, '');

const { runAgenticWorker, Sandbox } = await import(`${IRIS_CLI}/dist/engine.mjs`);
const { scoreTaskTrajectory } = await import(`${IRIS_ROOT}/lib/autoharness/index.mjs`);
...
const FIXTURE_DIR = '/tmp/iris-tune-bench';
...
async function runOne(provider) {
  ...
  delete process.env.IRIS_NO_OAUTH;
...
const outPath = `${IRIS_CLI}/.iris/benchmarks/provider-compare-latest.json`;
try {
  const { mkdir, writeFile } = await import('node:fs/promises');
  await mkdir(`${IRIS_CLI}/.iris/benchmarks`, { recursive: true });
