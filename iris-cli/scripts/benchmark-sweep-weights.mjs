...
const { runAgenticWorker, Sandbox, RunEngine } = await import(`${IRIS_CLI}/dist/engine.mjs`);
const { scoreTaskTrajectory } = await import(`${IRIS_ROOT}/lib/autoharness/index.mjs`);
const { computeAdaptiveWeights } = await import(`${IRIS_CLI}/dist/engine.mjs`).then(m => m).catch(() => ({}));
...
// Save results
const outPath = join(IRIS_CLI, '.iris', 'benchmarks', 'sweep-latest.json');
try {
  mkdirSync(join(IRIS_CLI, '.iris', 'benchmarks'), { recursive: true });
