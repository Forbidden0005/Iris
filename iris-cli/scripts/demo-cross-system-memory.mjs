...
import { AgentMemory } from '../dist/iris.mjs';
import { existsSync } from 'node:fs';

console.log('=== Cross-System Memory Demo ===\n');

// Check if IRIS_MEMORY_DIR is set
const memoryDir = process.env.IRIS_MEMORY_DIR;
if (memoryDir) {
  console.log(`✅ Shared memory enabled: ${memoryDir}`);
} else {
  console.log('⚠️  Using local memory (CLI-only mode)');
  console.log('   Set IRIS_MEMORY_DIR=/path/to/shared for cross-system sharing\n');
}

// Simulate Cursor storing user preferences
console.log('--- Cursor Agent ---');
const cursorMemory = new AgentMemory('iris-lead');
...
const cliMemory = new AgentMemory('iris-lead');
...
console.log('\n--- Gateway Agent ---');
const gatewayMemory = new AgentMemory('iris-lead');
...
const memoryFile = `${storageDir}/.iris/agent-memory/iris-lead.json`;
...
console.log('  1. Add to .env: IRIS_MEMORY_DIR=shared-memory');
