...
const DEFAULT_TASKS = [
  'Create a file named README.md in the project root containing exactly two lines: "# Benchmark Fixture" and "Generated by iris-cli benchmark."',
...
function parseArgs(argv) {
  const out = {
    ...
    tasksFile: '',
    out: '.iris/benchmarks/presets-latest.json',
    timeoutMs: Number(process.env.IRIS_BENCH_TIMEOUT_MS || 300000),
    maxRegressionPct: Number(process.env.IRIS_BENCH_MAX_REGRESSION_PCT || 25),
    minPassRate: Number(process.env.IRIS_BENCH_MIN_PASS_RATE || 0.5),
...
function resolvePresetEnv(provider, preset) {
  ...
  const base = {
    ...
    IRIS_L2A_MODEL: process.env.IRIS_L2A_MODEL || plannerModel,
    IRIS_L2B_MODEL: process.env.IRIS_L2B_MODEL || plannerModel,
    IRIS_REASONING_MODEL: process.env.IRIS_REASONING_MODEL || plannerModel,
    IRIS_JSON_REPAIR_MODEL: process.env.IRIS_JSON_REPAIR_MODEL || plannerModel,
    IRIS_EXECUTION_MODEL: process.env.IRIS_EXECUTION_MODEL || workerModel
  };
  ...
  switch (preset) {
    ...
    case 'quality':
      return {
        ...
        IRIS_L2_EXTRA_VALIDATORS: process.env.IRIS_L2_EXTRA_VALIDATORS || (plannerModel ? plannerModel : '')
      };
    default:
      return base;
  }
...
async function main() {
  ...
  const results = [];
  const crewCliEntry = resolve(cwd, 'dist', 'iris.mjs');
