...
# Setup test project
TEST_DIR="/tmp/iris-docker-sandbox-test-$(date +%s)"
...
echo ""
echo "Test 2: Using iris-cli to fix and test in Docker sandbox"
echo ""

# Use iris-cli to fix the file
cd iris-cli
...
echo "Running: iris run --task \"$TASK\""
echo ""
...
./dist/iris.mjs run --task "$TASK" --yes 2>&1 | tee /tmp/iris-docker-test.log
...
if grep -q "✓" /tmp/iris-docker-test.log && grep -q "npm test" /tmp/iris-docker-test.log; then
  ...
  echo "Check log: /tmp/iris-docker-test.log"
fi
