...
# Setup test project
TEST_DIR="/tmp/iris-docker-sandbox-direct-$(date +%s)"
mkdir -p "$TEST_DIR/tests" "$TEST_DIR/src" "$TEST_DIR/.iris"
...
# Now test with Docker using iris-cli's tool executor
...
node << 'NODEEOF'
import { Sandbox } from './dist/iris.mjs';
import { DockerSandbox } from './dist/iris.mjs';

const TEST_DIR = process.argv[1];
const sandbox = new Sandbox(`${TEST_DIR}/.iris`);
await sandbox.init();
