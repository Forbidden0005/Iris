 ...
 *
 * Usage: IRIS_EXECUTION_MODEL=gemini-2.5-flash node scripts/benchmark-l1-router.mjs
 */
...
async function testRouter(model) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'iris-l1-bench-'));
  ...
  // Import the pipeline
  const { UnifiedPipeline } = await import('../dist/iris.mjs');
