...
const RUN_STATUS_FILE = process.env.RUN_STATUS_FILE || '/tmp/iris-pipeline-run-status.jsonl';
...
const TASK = `Build MVP Phase 1 VS Code extension for Iris.
...
${generatedOutput}`;
  ...
  return executor.execute(prompt, {
    model: process.env.IRIS_EXECUTION_MODEL,
    temperature: 0.2,
...
async function qaAudit(executor, baseDir, round) {
  ...
  const prompt = `You are iris-qa. Audit this generated project.
...
${snapshot}`;
  ...
  const res = await executor.execute(prompt, {
    model: process.env.IRIS_QA_MODEL || process.env.IRIS_CHAT_MODEL || process.env.IRIS_EXECUTION_MODEL,
    temperature: 0.1,
    maxTokens: 4000
  });
...
async function fixerPass(executor, baseDir, issues, round) {
  ...
  const prompt = `You are iris-fixer. Fix only the QA issues below.
...
async function run() {
  ...
  // Core pipeline flags
  process.env.IRIS_USE_UNIFIED_ROUTER = 'true';
  process.env.IRIS_DUAL_L2_ENABLED = 'true';
  process.env.IRIS_ALLOW_CRITICAL = process.env.IRIS_ALLOW_CRITICAL || 'true';
  
  // Model selection (defaults to Grok, can override)
  process.env.IRIS_CHAT_MODEL = process.env.IRIS_CHAT_MODEL || 'grok-4-1-fast-reasoning';
  process.env.IRIS_REASONING_MODEL = process.env.IRIS_REASONING_MODEL || 'grok-4-1-fast-reasoning';
  process.env.IRIS_EXECUTION_MODEL = process.env.IRIS_EXECUTION_MODEL || 'grok-4-1-fast-reasoning';
  
  // Context pack optimizations (reduce token spend by 13%, time by 16%)
  process.env.IRIS_CONTEXT_BUDGET_CHARS = process.env.IRIS_CONTEXT_BUDGET_CHARS || '5000';
  process.env.IRIS_CONTEXT_MAX_CHUNKS = process.env.IRIS_CONTEXT_MAX_CHUNKS || '6';
  process.env.IRIS_CONTEXT_PACK_TTL_HOURS = process.env.IRIS_CONTEXT_PACK_TTL_HOURS || '24';
  ...
  process.env.IRIS_QA_LOOP_ENABLED = process.env.IRIS_QA_LOOP_ENABLED || 'false';
  process.env.IRIS_QA_MAX_ROUNDS = process.env.IRIS_QA_MAX_ROUNDS || '3';
