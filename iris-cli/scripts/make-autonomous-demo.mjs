...
async function main() {
    ...
    try {
        ...
        // 1. Initial Prompt
        await typeCommand('iris plan "implement natural language shell translation"');
        await appendOutput('<div class="info">[INFO] 🧠 Routing to iris-pm...</div>', 20);
        await appendOutput(`
...
5. Integrate "iris shell" command into CLI entry point.
...
<span class="success">+   const result = await router.dispatch(\'iris-main\', ...);</span>
...
&gt;&gt;&gt;&gt;&gt;&gt; REPLACE</div>`, 40);
        ...
        await appendOutput('<div class="success">[SUCCESS] Integrated "iris shell" into src/cli/index.ts</div>', 30);

        // 6. Test it!
        await clear();
        await appendOutput('<div class="info">--- Testing the new feature ---</div>', 20);
        await typeCommand('iris shell "list all large files in src"');
