...
async function runPipelineBenchmark() {
  ...
  const config = {
    ...
    chatModel: process.env.IRIS_CHAT_MODEL || 'auto',
    reasoningModel: process.env.IRIS_REASONING_MODEL || 'auto',
    executionModel: process.env.IRIS_EXECUTION_MODEL || 'auto'
  };
  ...
  if (!config.unifiedRouter) {
    console.log('⚠️  IRIS_USE_UNIFIED_ROUTER not enabled');
    console.log('   Set: export IRIS_USE_UNIFIED_ROUTER="true"\n');
  }
