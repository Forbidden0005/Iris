 ...
 *
 * Usage: IRIS_EXECUTION_MODEL=gemini-2.5-flash node scripts/benchmark-l3-executor.mjs
 *        IRIS_EXECUTION_MODEL=ollama:glm-5.1:cloud node scripts/benchmark-l3-executor.mjs
 */
...
function runCommand(cmd, cwd) {
  try {
    const output = execSync(cmd, { cwd, stdio: 'pipe', encoding: 'utf8', timeout: Number(process.env.IRIS_BENCHMARK_TIMEOUT || 600) * 1000 });
...
async function runTask(task, model) {
  const dir = await mkdtemp(join(tmpdir(), `iris-l3-${task.id}-`));
  ...
  try {
    ...
    const result = await runAgenticWorker(task.description, sandbox, {
      ...
      maxTurns: 25,
      projectDir: dir,
      verbose: Boolean(process.env.IRIS_VERBOSE),
    });
...
const model = process.env.IRIS_EXECUTION_MODEL || 'default';
