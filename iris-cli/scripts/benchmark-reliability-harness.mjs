#!/usr/bin/env node
/**
 * Reliability benchmark harness for iris-cli dispatch.
 *
 * What it does:
 * 1) Creates an isolated seed project per run under /tmp
 * 2) Dispatches a coding task to an agent via iris-cli (JSON mode)
...
test("persists and restores retry state", async () => {
  const statePath = path.join(os.tmpdir(), "iris-bench-retry-state.json");
...
async function main() {
  ...
  const outputDir = args.outputDir || path.join(process.cwd(), ".iris", "benchmarks");
  await fs.mkdir(outputDir, { recursive: true });

  const results = [];
  for (let i = 0; i < args.runs; i += 1) {
    const runId = i + 1;
    const runDir = path.join(os.tmpdir(), `iris-bench-retry-${stamp}-run${runId}`);
    ...
    // Use compiled iris.mjs for standalone execution
    const crewCliPath = path.join(process.cwd(), "dist", "iris.mjs");
    const dispatchArgs = [
      "run",
      "-t", prompt,
      "--json",
