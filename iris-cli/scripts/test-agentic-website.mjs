 ...
 * Loads API keys from ~/.iris/iris.json
 */
...
if (!existsSync(configPath)) {
  console.error('❌ No iris.json at', configPath);
  process.exit(1);
}
...
if (!env.XAI_API_KEY && !env.GEMINI_API_KEY && !env.OPENAI_API_KEY && !env.DEEPSEEK_API_KEY && !env.GROQ_API_KEY && !env.ANTHROPIC_API_KEY && !env.OPENROUTER_API_KEY) {
  console.error('❌ No API keys in iris.json (need xai/google/openai/deepseek/groq/anthropic/openrouter)');
  process.exit(1);
}
...
const projectDir = '/tmp/iris-agentic-test';
const crewPath = join(process.cwd(), 'bin', 'iris.js');
