...
<div class="info"> ╔═══════════════════════════════════════════════════════════════════════════╗
 ...
 ╚═══════════════════════════════════════════════════════════════════════════╝</div>`, 45);
        ...
        // 2. Explore Mode
        await typeCommand('iris explore "refactor database to sqlite3"');
        ...
        // 3. Preview
        await typeCommand('iris preview explore-clean');
        await appendOutput(`
...
... (18 files changed)</div>`, 90);
        ...
        // 4. LSP Check
        await typeCommand('iris lsp check src/db.js');
        await appendOutput('<div class="success">[SUCCESS] No type errors found in the staged sandbox version.</div>', 60);

        // 5. Parallel Workers
        await clear();
        await typeCommand('iris plan "add CRUD for products and orders" --parallel --concurrency 2');
        ...
        // 6. Blast Radius
        await typeCommand('iris blast-radius');
        await appendOutput(`
...
Recursive Impact: 12 modules potentially affected.</div>`, 90);
        ...
        // 7. Apply & Auto-Fix
        await typeCommand('iris apply --check "npm test"');
        ...
        await appendOutput('<div class="info">[INFO] Attempting auto-fix by dispatching to iris-fixer...</div>', 45);
