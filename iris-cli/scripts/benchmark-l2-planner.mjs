 ...
 * Uses IRIS_FORCE_L2=true to get the plan without executing L3 workers.
 *
 * Scores:
 *   - Does it decompose into reasonable units?
 *   - Are dependencies correct (tests depend on implementation)?
 *   - Are personas appropriate (backend → iris-coder-back, tests → iris-qa)?
...
const TASKS = [
  {
    ...
    expectPersonas: ['iris-coder', 'iris-coder-back', 'iris-qa', 'iris-copywriter'],
  ...
  },
  {
    ...
    expectPersonas: ['iris-coder', 'iris-qa'],
  ...
  },
  {
    ...
    expectPersonas: ['iris-coder-back', 'iris-security', 'iris-qa', 'iris-copywriter'],
    expectDependencies: true,
...
function scorePlan(task, plan) {
  ...
  const expectedHits = task.expectPersonas.filter(p =>
    [...usedPersonas].some(up => up.includes(p.replace('iris-', '')) || p.includes(up.replace('iris-', '')))
  );
...
async function runTask(task) {
  const dir = await mkdtemp(join(tmpdir(), `iris-l2-bench-${task.id}-`));
  ...
  try {
    ...
    // The pipeline writes logs to cwd/.iris/ (which may be the iris-cli dir, not the temp dir)
    ...
    const searchDirs = [
      join(dir, '.iris', 'pipeline-runs'),
      join(process.cwd(), '.iris', 'pipeline-runs')
    ];
...
console.log(`Model: ${process.env.IRIS_EXECUTION_MODEL || process.env.IRIS_L2A_MODEL || 'default'}\n`);
...
if (outFile) {
  await writeFile(outFile, JSON.stringify({ ts: new Date().toISOString(), model: process.env.IRIS_EXECUTION_MODEL || 'default', results, avg }, null, 2));
  console.log(`\nSaved: ${outFile}`);
}
