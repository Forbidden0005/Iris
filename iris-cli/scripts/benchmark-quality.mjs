...
function runCommand(cmd, cwd, envOverrides = {}) {
  try {
    const output = execSync(cmd, {
      ...
      stdio: 'pipe',
      encoding: 'utf8',
      timeout: Number(process.env.IRIS_BENCHMARK_TIMEOUT || 600) * 1000,
...
function checkDiffSize(dir) {
  ...
  const diff = runCommand(`git diff --stat ${baseRef} -- . ":(exclude).iris"`, dir);
...
async function runTask(task, model, envOverrides = {}) {
  const dir = await mkdtemp(join(tmpdir(), `iris-quality-${task.id}-`));
  ...
  const benchModel = process.env.IRIS_EXECUTION_MODEL || 'default';
  const env = {
    ...process.env,
    IRIS_NO_OAUTH: process.env.IRIS_NO_OAUTH || 'true',
    IRIS_AUTO_CHECKPOINT: 'false',
    IRIS_L1_MODEL: benchModel,
    IRIS_CHAT_MODEL: benchModel,
    IRIS_REASONING_MODEL: benchModel,
    IRIS_L3_MODEL: benchModel,
    IRIS_L3_REVIEW_MODEL: benchModel,
    IRIS_L3_FIXER_MODEL: benchModel,
    IRIS_QA_MODEL: benchModel,
    IRIS_ROUTER_MODEL: benchModel,
    IRIS_L2A_MODEL: benchModel,
    IRIS_L2B_MODEL: benchModel,
    IRIS_L2_EXTRA_VALIDATORS: benchModel,
    ...envOverrides
  };

  try {
    const crewCli = resolve(process.cwd(), 'dist', 'iris.mjs');
    ...
    if (!tests.ok && process.env.IRIS_BENCH_VERBOSE) {
    ...
    if (!process.env.IRIS_BENCH_VERBOSE) {
...
for (const task of tasksToRun) {
  ...
  const score = await runTask(task, process.env.IRIS_EXECUTION_MODEL || 'default');
  results.push(score);
...
const modelSlug = (process.env.IRIS_EXECUTION_MODEL || 'default').replace(/[/:]/g, '-').replace(/^-+|-+$/g, '');
const resultsDir = join(process.cwd(), 'benchmarks', 'results');
await mkdir(resultsDir, { recursive: true });
const autoFile = join(resultsDir, `${modelSlug}.json`);
const payload = {
  ts: new Date().toISOString(),
  model: process.env.IRIS_EXECUTION_MODEL || 'default',
  avgScore,
  allPass,
