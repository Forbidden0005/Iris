...
export class ApiClient {
  ...
  constructor() {
    const config = vscode.workspace.getConfiguration('iris');
  ...
  async chat(message: string, context?: string): Promise<ChatResponse> {
    ...
    try {
      const requestBody: ChatRequest = {
        message,
        sessionId: vscode.workspace.getConfiguration('iris').get('sessionId', 'vscode-extension'),
        context,
        mode: vscode.workspace.getConfiguration('iris').get('mode', 'connected')
      };
      ...
      throw new Error(`Failed to connect to iris: ${(error as Error).message}`);
    }
  }
