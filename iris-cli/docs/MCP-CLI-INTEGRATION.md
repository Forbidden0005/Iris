...
1. `iris-cli` manages MCP client configs (`iris mcp ...`) for tools like Cursor/Claude/OpenCode.
2. `iris-cli` also serves its own MCP server in standalone mode.
3. The iris gateway (parent repo) hosts the main MCP server at `http://127.0.0.1:5020/mcp`.

## Supported AI Coding Tools

iris MCP integration works with:
...
All 5 tools can connect to **both** iris MCP servers (main gateway + iris-cli standalone).

## Two MCP Servers Available

### iris-cli MCP Server
**Port:** 4097 (configurable)  
**Start:** `iris serve --port 4097`  
...
4. `chat_stinki` - Talk to iris-lead
5. `iris_status` - System status
...
# Main iris: npm run restart-all (or: node scripts/mcp-server.mjs)
# iris-cli: iris serve --port 4097

# 2) Add iris gateway MCP
gemini mcp add iris "http://127.0.0.1:5020/mcp" \
  --transport http \
  --header "Authorization: Bearer $(cat ~/.iris/iris.json | python3 -c 'import json,sys; print(json.load(sys.stdin)[\"rt\"][\"authToken\"])')" \
  --description "iris main MCP server - 20 agents + 46 skills" \
  --trust

# 3) Add iris-cli MCP
gemini mcp add iris-cli "http://127.0.0.1:4097/mcp" \
  --transport http \
  --header "Authorization: Bearer $(cat ~/.iris/iris.json | python3 -c 'import json,sys; print(json.load(sys.stdin)[\"rt\"][\"authToken\"])')" \
  --description "iris-cli MCP server - unified routing + sandbox" \
  --trust
...
gemini "list all available mcp tools" --allowed-mcp-server-names iris iris-cli
```
...
### Install iris-cli MCP (Standalone Tools)

```bash
# 1) Start iris-cli server
iris serve --port 4097

# 2) Export token (if not already in .zshrc)
export IRIS_TOKEN="$(node -e "const fs=require('fs');const os=require('os');const p=os.homedir()+'/.iris/iris.json';const j=JSON.parse(fs.readFileSync(p,'utf8'));process.stdout.write(j.rt?.authToken||'')")"

# 3) Add to Codex
codex mcp add iris-cli --url "http://127.0.0.1:4097/mcp" --bearer-token-env-var IRIS_TOKEN

# 4) Verify
codex mcp get iris-cli
```

### Install iris Gateway MCP (All Agents + Skills)
...
### Use iris-cli MCP When You Want:
...
```bash
codex mcp list
# iris-cli   http://127.0.0.1:4097/mcp  ← Routing + sandbox
# iris  http://127.0.0.1:5020/mcp  ← Agents + skills
```

## Install In iris-cli MCP Store (optional)

This syncs to local client config files for Cursor / Claude / OpenCode, and can also register Codex via `codex mcp add`:

```bash
iris mcp add iris --url http://127.0.0.1:5020/mcp --bearer-token-env-var IRIS_TOKEN
iris mcp add iris --url http://127.0.0.1:5020/mcp --bearer-token-env-var IRIS_TOKEN --client codex
iris mcp list
iris mcp doctor
```

## What `iris-cli` Serves vs MCP

`iris-cli` serves unified HTTP APIs via:
...
2. `/mcp` endpoint (native MCP tools in `iris-cli`)

`--mode standalone` is still accepted as a compatibility alias, but `connected` is no longer supported.

Use gateway `:5020/mcp` for full iris agents + skills.

## Troubleshooting

1. Check iris-cli MCP is up: `curl http://127.0.0.1:4097/mcp/health`
2. Check gateway MCP is up: `curl http://127.0.0.1:5020/health`
3. Check iris-lead is up: `curl http://127.0.0.1:5010/api/agents -H "Authorization: Bearer $IRIS_TOKEN"`
4. Re-check MCP configs: `codex mcp get iris-cli` and `codex mcp get iris`
5. Validate local config health: `iris mcp doctor`
6. Check token is set: `echo $IRIS_TOKEN`
