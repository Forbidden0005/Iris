...
All notable changes to the iris-cli project.
...
- fix: brand lowercase "iris" everywhere (23 source files)
...
- **Budget enforcement**: `IRIS_MAX_COST_USD` and `IRIS_MAX_TURNS` gates halt autonomous loops before overrun.
- **History clearing**: `/clear` command resets conversation history in the REPL without restarting the session.
- **Scratchpad memory**: agents can write intermediate reasoning to a local scratchpad (`~/.iris/scratchpad/`) that persists across turns but is excluded from the context window.
...
- **Reactive context compaction**: compaction now triggers automatically when token usage crosses a configurable threshold (`IRIS_COMPACT_THRESHOLD`, default 85% of context window), rather than only at turn boundaries.
...
#### Other Additions
- Dashboard Testing tab integration: iris-cli test results (906 tests) stream to the Testing tab via SSE in real time.
- `iris doctor` now reports test suite health alongside API key and gateway checks.
...
- Published to npm as `iris-cli@0.3.5`.

### Changed
- Multi-turn driver (`src/executor/multi-turn-drivers.ts`): refactored to support streaming tool execution and reactive compaction. Legacy blocking path still available under `IRIS_LEGACY_EXECUTOR=1`.
...
- Gemini tool adapter (`src/tools/gemini/iris-adapter.ts`): updated to handle new tool schemas and post-sampling hook registration.
- Autonomous loop (`src/worker/autonomous-loop.ts`): integrates budget gates, abort signal handling, and reactive compaction.

### Fixed
- Dynamic Gemini declaration builder: `IRIS_GEMINI_DYNAMIC_DECLARATIONS` default now correctly `true` (was `false` due to env-coercion bug).
...
- Lowercase brand name: `iris` normalized everywhere (was mixed `Iris` / `iris` in banners, logs, and npm metadata).
...
- `IRIS_TEST_MODE` guard: strict `"true"` equality check (not `"1"`) prevents test runs from writing to real config directories.
...
- Self-consistency gate (`IRIS_SELF_CONSISTENCY_GATE_ENABLED`) validates synthesized output against worker evidence before returning a result.
- Adaptive QA rounds: small edits skip full QA cycle when diff size is below `IRIS_QA_SMALL_EDIT_THRESHOLD`.

### Changed
- iris-cli version bumped 0.3.1 → 0.3.2.
- All 6 CLI engine session-resume paths (Claude Code, Cursor, Codex, Gemini CLI, OpenCode, iris-cli) verified end-to-end via new integration tests.
...
This release marks the completion of Phase 5, transforming `iris-cli` from a simple gateway client into a sophisticated, speculative multi-agent orchestrator with deep developer experience features.

### ✅ Added

#### Intelligence & Speculative Execution
- **Speculative Explore (`iris explore`)**: Automated execution of a task across 3 parallel sandbox branches using different strategic prompts (Minimal, Clean, Pragmatic).
...
- **LSP Integration**: Built-in support for TypeScript type-checking (`iris lsp-check`) and autocomplete suggestions (`iris lsp-complete`).
...
- **Local RAG Search (`iris docs`)**: TF-IDF ranked search over local documentation and markdown files with automatic context injection.

#### Core CLI & DevEx
- **Natural Language Shell (`iris shell`)**: GitHub Copilot CLI-style translation of natural language into OS-specific shell commands with interactive execution.
- **Interactive PTY (`iris exec`)**: Full pseudo-terminal support for running interactive tools like `vim`, `htop`, or custom scripts directly via the agent.
- **Repository Mapping**: Visual dependency-aware codebase graph generation (`iris map --graph`).
- **Image Context**: Visual ingestion support—attach screenshots or images to tasks using the `--image` flag.
- **Speculative Implementation**: Enhanced sandbox branching (`iris branch`, `iris switch`, `iris merge`) for non-destructive experimentation.
- **Token Caching**: Automated local caching of model outputs to reduce costs and latency for repeated tasks.

#### Operational Hardening
- **GitHub Intelligence**: Natural language flows for issues and PRs, including a health check tool (`iris github doctor`) and safe preview (`--dry-run`).
- **Model Policies**: Centralized model tier configuration, fallback chains, and max-cost gates in `.iris/model-policy.json`.
...
#### Infrastructure
- **3-Tier LLM Architecture**: Optimized routing (Gemini 2.0 Flash) -> Planning (Claude 3.5 Sonnet) -> Execution (Worker Iris).
