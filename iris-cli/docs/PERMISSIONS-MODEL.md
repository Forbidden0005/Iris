# iris-cli Permissions Model

This is the canonical explanation of what `iris-cli` can do in each mode and how approvals work.

## Core rule

`iris-cli` is local-first, but writes should remain inspectable and reversible whenever possible.

The main safety mechanism is the sandbox:

- edits accumulate in `.iris/sandbox.json`
...
|---|---|---|---|---|---|---|
| `iris chat` standalone | Yes | Yes | Via `/apply` or auto-apply | Limited through runtime/tool path | Yes | Depends on REPL mode / flags |
| `iris run` / `iris dispatch` / `iris plan` | Yes | Yes | Via apply path or explicit auto-approve settings | Yes, if task/tools require it | Yes | Depends on flags and execution policy |
...
| Agent-assigned execution inside Iris | Yes | Depends on agent tool policy | Depends on engine/runtime path | Depends on agent/engine | Yes | Controlled by assigned engine + swarm policy |
