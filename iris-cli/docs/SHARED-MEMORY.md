...
`iris-cli` features a multi-layered memory system designed for cognitive continuity across sessions, models, and teams.

## 🧠 Memory Layers

### 1. AgentKeeper (Task Memory)
Stores the results of every task executed by the iris in a local, append-only JSONL store (`.iris/agentkeeper.jsonl`).
...
- **Commands**: 
  - `iris memory "search query"`: Manually recall prior task results.
  - `iris memory-compact`: Compact the store and deduplicate entries.
...
- **Storage**: Can be shared across systems using the `IRIS_MEMORY_DIR` environment variable.
...
- **Commands**:
  - `iris docs "how to deploy"`: Search local documentation.
  - Used automatically when the `--docs` flag is passed to `chat` or `dispatch`.

---

## 🤝 Team Synchronization (`iris sync`)

The `sync` system allows engineering teams to share AI "experience" without sharing sensitive code.

### How it works
1. **Upload**: `iris sync --upload` packages your local session history and AI correction patterns.
2. **Privacy**: Before upload, entries are filtered through `.iris/privacy.json`. You can choose to redact prompts, original code, or tags.
...
4. **Download**: `iris sync --download` pulls team-wide corrections and merges them into your local AgentKeeper, allowing you to benefit from your teammates' AI interactions.
