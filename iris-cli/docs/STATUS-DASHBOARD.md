...
The `iris status` command shows a **real-time** orchestration dashboard with live metrics from your actual system.
...
### Active Agents
Number of running agent processes (iris-coder, iris-fixer, etc.) detected via process list.
...
// Active agents
execSync('ps aux | grep -E "iris-|gateway-bridge" | wc -l')

// Task queue
readFile('.iris/autofix/queue.json')

// Models
readFile('~/.iris/iris.json')
```
...
1. **Users can verify immediately** - Run `iris status` and see it work
...
$ iris plan "add authentication" --parallel
```
...
cat .iris/autofix/queue.json

# Add a test task
iris autofix enqueue "fix linter errors"
```
