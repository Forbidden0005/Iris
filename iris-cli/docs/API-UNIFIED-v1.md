...
1. `connected` — dashboard/clients call iris (`iris-lead` + gateway/RT).
2. `standalone` — dashboard/clients call local `iris-cli` runtime directly.
...
- Primary for iris dashboard.
- Backed by one-level-up services (`iris-lead`, `gateway-bridge`, RT bus).
...
- Backed by local `iris-cli` runtime (`Orchestrator`, `UnifiedPipeline`, `Sandbox`, `SessionManager`).
- Served through a local HTTP adapter (recommended command: `iris serve`).

## Runtime Selection

Use one selector for all interfaces:

- `IRIS_INTERFACE_MODE=connected|standalone`

Optional:

- `IRIS_LEAD_URL=http://127.0.0.1:5010` (connected)
- `IRIS_API_PORT=4317` (standalone local API)
...
{
  "agents": [
    { "id": "iris-coder", "status": "online", "model": "..." }
  ]
}
```
...
- `iris chat` -> `/v1/chat`
- `iris dispatch` -> `/v1/tasks`
- `iris preview/apply/rollback` -> `/v1/sandbox*`
- `iris trace <id>` -> `/v1/traces/:traceId`

## Compatibility Notes

- Current connected stack already supports the core primitives via `iris-lead` endpoints.
