...
Phase-aware resume:
- `iris run --resume <traceId> --from-phase execute` reuses prior plan artifact when available.
- `iris run --resume <traceId> --from-phase validate` reuses prior plan + prior validate input when available.

## `iris capabilities --json`
