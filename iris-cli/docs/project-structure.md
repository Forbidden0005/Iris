# iris-cli Project Structure (v0.1.0-alpha)

## 🏎️ Overview
`iris-cli` is a high-performance multi-agent orchestrator that bridges your terminal with 20+ specialized AI agents. It uses a **3-Tier LLM Architecture** (Routing -> Planning -> Execution) and a safety-first **Cumulative Diff Sandbox**.
...
iris-cli/
├── bin/
│   └── iris.js              # Executable CLI entry point
├── dist/                    # Bundled output (dist/iris.mjs)
...
- **Bundler**: `esbuild` (produces a single `dist/iris.mjs`).
- **Runtime**: Node.js 20+ (ESM).
- **Communication**: HTTP REST + WebSocket RT Bus to the Iris gateway.

---
**iris-cli** is built for speed, safety, and scale. 🚀
