...
iris-cli is the terminal interface for iris. It provides 45+ built-in tools, 6 LLM providers, 7 execution engines, and a tiered autonomy system that controls how much the agent can do without approval.

---

## Tiered Autonomy (L1/L2/L3)

iris-cli uses a three-tier system to control tool access:
...
OAuth tokens are discovered automatically. If a valid OAuth token exists, the API key is not needed. This means users with Claude Pro, Cursor Pro, or a Google account can use iris-cli at zero marginal cost for those providers.

---

## Execution Engines

7 engines for running tasks. iris-cli can dispatch to any of them:

1. **Direct API** -- Raw provider API calls. Most flexible, works with all 6 providers.
2. **iris-cli native** -- Built-in tool loop with 45+ tools. Default for L3 tasks.
...
```bash
iris chat "refactor auth" --engine claude-code
iris chat "add tests" --engine codex
```
...
2. **RAG** -- TF-IDF + embeddings over the codebase. Relevant files are injected automatically based on the current query. `iris docs <query>` searches project documentation explicitly.
...
Sessions are stored as JSONL for crash safety. If iris-cli is killed mid-conversation, `iris repl --resume` picks up where it left off. Sessions are keyed by `(engine, projectDir, sessionId)` so multiple chats against the same project do not interfere.

---

## Cost Tracking

Every API call is logged with token counts and USD cost. `iris cost` shows the running total. Budget enforcement stops execution when a `--budget` limit is hit:

```bash
iris auto "fix all lint errors" --budget 0.50
```
...
After the LLM produces a response, iris-cli can run automatic checks before applying changes:
...
```
spawn_agent("iris-qa", "write integration tests for auth") -> session_id
...
For parallel agent work, iris-cli creates git worktrees so each agent operates on an isolated branch without interfering with the working directory:

```bash
iris auto "refactor auth" --worktree
```
...
Cross-session semantic memory stored in `.iris/agentkeeper.jsonl`. The agent records facts, decisions, and corrections. On future sessions, relevant memories are recalled automatically via semantic similarity with deduplication.

```bash
iris memory "what did we decide about the auth schema?"
iris memory-compact   # deduplicate and prune stale entries
```
...
`iris doctor` runs 10 provider health checks to verify that API keys, OAuth tokens, and CLI installations are working:
...
```bash
iris chat "what's wrong with this UI?" --image screenshot.png
```
...
| Feature | Claude Code | Codex CLI | Gemini CLI | Cursor CLI | OpenCode | **iris-cli** |
|---------|------------|-----------|------------|------------|----------|--------------|
...
```bash
iris chat "<prompt>"              # One-shot query
iris auto "<task>"                # Autonomous loop with iteration limit
iris plan "<task>"                # Generate step-by-step plan (L2), then execute (L3)
iris dispatch <agent> "<task>"    # Send task to a specific agent
iris config show                  # Show resolved config (team + user layers)
iris github "<natural language>"  # GitHub operations via gh with confirmation gates
iris map --graph                  # Repository dependency graph
iris x-search "<query>"          # X/Twitter search via xAI
iris autofix enqueue "<task>"     # Queue background auto-fix job
iris autofix worker               # Run background job processor
iris shell "<description>"        # Natural language to shell command
iris docs "<query>"               # RAG search over project docs
iris checkpoint list              # List resumable execution checkpoints
```
