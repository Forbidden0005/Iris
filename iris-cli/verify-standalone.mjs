...
async function main() {
  section('🔍 IRIS-CLI VERIFICATION REPORT');
  ...
  if (existsSync(pipelinePath)) {
    grepFile(pipelinePath, 'IRIS_DUAL_L2_ENABLED', 'Pipeline checks IRIS_DUAL_L2_ENABLED flag');
  ...
  checkFile('src/tools/gemini/iris-adapter.ts', 'Iris adapter for Gemini tools');
  ...
  if (existsSync('src/tools/gemini/iris-adapter.ts')) {
    grepFile('src/tools/gemini/iris-adapter.ts', 'sandbox.stage', 'Gemini adapter uses sandbox.stage()');
  }
  ...
  if (existsSync(sandboxPath)) {
    ...
    const hasDockerCheck = content.includes('IRIS_DOCKER_SANDBOX') || 
  ...
  checkFile('dist/iris.mjs', 'Built CLI exists');
  ...
  const results = {
    ...
    'Build Status': existsSync('dist/iris.mjs'),
  };
  ...
  log('See: iris-cli/README.md', CYAN);
