...
const testDir = '/tmp/iris-rag-test';
...
try {
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=keyword node ${process.cwd()}/bin/iris.js run "add rate limiting to auth endpoint" --dry-run 2>&1 || true`,
...
try {
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=import-graph node ${process.cwd()}/bin/iris.js run "update auth to use config" --dry-run 2>&1 || true`,
    { encoding: 'utf8', maxBuffer: 1024 * 1024 }
  );
...
if (process.env.OPENAI_API_KEY) {
  ...
  try {
    const output = execSync(
      `cd ${testDir} && IRIS_RAG_MODE=semantic node ${process.cwd()}/bin/iris.js run "improve authentication security" --dry-run 2>&1 || true`,
...
try {
  ...
  execSync(
    `cd ${testDir} && node ${process.cwd()}/bin/iris.js run "read auth.ts" --dry-run 2>&1 || true`,
    { encoding: 'utf8', stdio: 'ignore' }
  );
  
  const output = execSync(
    `cd ${testDir} && IRIS_RAG_MODE=keyword node ${process.cwd()}/bin/iris.js run "update this file" --dry-run 2>&1 || true`,
    { encoding: 'utf8', maxBuffer: 1024 * 1024 }
  );
...
console.log('  IRIS_RAG_MODE=keyword|import-graph|semantic|off');
console.log('  IRIS_RAG_TOKEN_BUDGET=8000 (default)');
console.log('  IRIS_RAG_MAX_FILES=10 (default)');
console.log('  IRIS_RAG_CACHE_DIR=.iris/rag-cache (default)');
