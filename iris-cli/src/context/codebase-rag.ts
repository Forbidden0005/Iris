...
function detectEmbeddingProvider(): EmbeddingProvider {
  const explicit = String(process.env.IRIS_EMBEDDING_PROVIDER || '').toLowerCase();
...
export class CodebaseIndex {
  ...
  private constructor(cwd: string) {
    this.cwd = cwd;
    this.cacheDir = join(cwd, '.iris', 'rag-cache');
  ...
  private listFiles(): string[] {
    try {
      const maxFiles = Number(process.env.IRIS_RAG_MAX_FILES || 2000);
  ...
  async ensureIndex(opts?: { onProgress?: (done: number, total: number) => void }): Promise<{ indexed: number; skipped: number; removed: number }> {
    ...
    try {
      ...
      const batchSize = Number(process.env.IRIS_RAG_BATCH_SIZE || 20);
...
export async function autoLoadRelevantFiles(
  ...
  const rawMode = options.mode || String(process.env.IRIS_RAG_MODE || 'auto').toLowerCase() as RagMode;
  const tokenBudget = options.tokenBudget || Number(process.env.IRIS_RAG_TOKEN_BUDGET || 8000);
  const maxFiles = options.maxFiles || Number(process.env.IRIS_RAG_MAX_FILES_LOAD || 10);
...
export async function rebuildEmbeddingsIndex(
  ...
  const dir = cacheDir || join(cwd, '.iris', 'rag-cache');
