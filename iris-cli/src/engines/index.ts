...
const logger = new Logger({ level: process.env.IRIS_LOG_LEVEL || 'info' });
...
function sessionLayerEnabled(): boolean {
  const raw = String(process.env.IRIS_ENGINE_SESSION_ENABLED || 'true').trim().toLowerCase();
  return raw !== 'false' && raw !== '0' && raw !== 'off' && raw !== 'no';
}

function nativeEngineSessionEnabled(): boolean {
  const raw = String(process.env.IRIS_ENGINE_NATIVE_SESSION_ENABLED || 'true').trim().toLowerCase();
  return raw !== 'false' && raw !== '0' && raw !== 'off' && raw !== 'no';
}
  ...
  try {
    if (engineStore) {
      await engineStore.appendTurn({
        ...
        model: options.model,
        durationMs,
        keepTurns: Number(process.env.IRIS_ENGINE_SESSION_KEEP_TURNS || 20)
      });
    }
    if (transcriptStore) {
      await transcriptStore.appendTurn({
        ...
        text: prompt,
        engine,
        keepTurns: Number(process.env.IRIS_CONVERSATION_KEEP_TURNS || 40)
      });
      await transcriptStore.appendTurn({
        sessionId,
        role: 'assistant',
...
export async function runClaudeCli(prompt: string, options: EngineRunOptions = {}): Promise<EngineRunResult> {
  ...
  if (process.env.IRIS_CLAUDE_SKIP_PERMISSIONS !== 'false') {
...
export async function runCursorCli(prompt: string, options: EngineRunOptions = {}): Promise<EngineRunResult> {
  ...
  const cursorDefault = process.env.IRIS_CURSOR_MODEL || 'composer-2-fast';
