...
export class NativeEngineSessionManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    ...
    this.stateDir = join(this.baseDir, '.iris');
...
export function buildEngineShellCommand(engine: string, prompt: string, model?: string, cwd?: string): string {
  ...
  if (e === 'claude-cli') return `claude -p --setting-sources user --output-format stream-json --verbose${String(process.env.IRIS_CLAUDE_SKIP_PERMISSIONS || 'true') !== 'false' ? ' --dangerously-skip-permissions' : ''} -- ${p}`;
