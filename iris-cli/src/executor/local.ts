...
const EXECUTOR_SYSTEM_PROMPT = `You are the conversational interface for iris CLI.
...
- Match iris-lead's personality: sharp, direct, no filler
...
export class LocalExecutor {
  ...
  private async getConfiguredProviderOrder(): Promise<string[]> {
    const envOrder = String(process.env.IRIS_EXECUTION_PROVIDER_ORDER || "")
      .split(",")
  ...
  private async resolveProviderAuth(provider: string): Promise<ProviderAuth | null> {
    switch (provider) {
      case 'openai': {
        ...
        if (process.env.IRIS_NO_OAUTH !== 'true') {
      ...
      case 'gemini': {
        ...
        if (process.env.IRIS_NO_OAUTH === 'true') return null;
  ...
  async execute(task: string, options: ExecutorOptions = {}): Promise<ExecutorResult> {
    ...
    if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Executor] Model: ${model}, Providers: [${providers.join(', ')}]`);
    
    for (const provider of providers) {
      try {
        const auth = await this.resolveProviderAuth(provider);
        const authBadge = auth?.isOAuth ? 'OAuth' : 'API';
        if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') {
  ...
  private getTimeoutMs(): number {
    const raw = Number(process.env.IRIS_EXECUTOR_TIMEOUT_MS || 90000);
  ...
  private async getDefaultModel(): Promise<string> {
    ...
    const envModel = process.env.IRIS_EXECUTION_MODEL || process.env.IRIS_CHAT_MODEL || process.env.IRIS_REASONING_MODEL;
  ...
  private async executeWithGrok(task: string, options: ExecutorOptions, systemPrompt: string, sessionId?: string): Promise<ExecutorResult | null> {
    ...
    const model = options.model || process.env.IRIS_EXECUTION_MODEL || 'grok-4-1-fast-reasoning';

    try {
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Grok] Starting API call (model: ${model})...`);
      ...
      const stream = !isStreamingDisabled() && options.jsonMode !== true && process.env.IRIS_GROK_STREAMING !== '0';
      const requestBody = {
        model,
        messages: [
          { role: 'system', content: systemPrompt },
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Grok] Response received in ${Date.now() - callStart}ms (status: ${response.status})`);
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Grok] Exception: ${(err as Error).message}`);
  ...
  private async executeWithGeminiCodeAssist(
    ...
    const requestBody = {
      ...
      user_prompt_id: options.sessionId || `iris-${Date.now()}`,
      request: {
        contents: [{
          role: 'user',
  ...
  private async executeWithDeepSeek(task: string, options: ExecutorOptions, systemPrompt: string): Promise<ExecutorResult | null> {
    ...
    if (!key) {
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log('[DeepSeek] No API key found');
      return null;
    }
    ...
    try {
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[DeepSeek] Response received (status: ${response.status})`);
      ...
      if (stream && response.body) {
        const result = await streamOpenAIResponse(response, writeToStdout);
        if (result.text) process.stdout.write('\n');
        if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[DeepSeek] ✓ Success (${result.usage?.prompt_tokens || 0} in, ${result.usage?.completion_tokens || 0} out)`);
        return {
          success: true, result: result.text, model,
          promptTokens: result.usage?.prompt_tokens,
          completionTokens: result.usage?.completion_tokens,
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[DeepSeek] ✓ Success (${data?.usage?.prompt_tokens || 0} in, ${data?.usage?.completion_tokens || 0} out)`);
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[DeepSeek] Exception: ${(err as Error).message}`);
  ...
  private async executeWithAnthropic(
    ...
    try {
      if (auth.isOAuth) {
        const oauthModel = options.model || String(process.env.IRIS_OAUTH_CLAUDE_MODEL || 'claude-sonnet-4-6');
        ...
        const bodyObj = {
          ...
          metadata: { user_id: `user_iris_session_${sessionId || randomUUID()}` },
          system: systemBlocks,
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Anthropic] Starting API call (model: ${model})...`);
      const callStart = Date.now();
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Anthropic] Response received in ${Date.now() - callStart}ms (status: ${response.status})`);
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[Anthropic] Exception: ${(err as Error).message}`);
  ...
  private async executeWithOpenAI(
    ...
    if (!auth) {
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log('[OpenAI] No API key found');
      return null;
    }

    const model = options.model || (auth.isOAuth ? String(process.env.IRIS_OAUTH_OPENAI_MODEL || 'gpt-5.4') : 'gpt-4o');
    
    if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[OpenAI] Starting API call (model: ${model})...`);
    ...
    try {
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[OpenAI] Response received (status: ${response.status})`);
      ...
      if (stream && response.body) {
        const result = await streamOpenAIResponse(response, writeToStdout);
        if (result.text) process.stdout.write('\n');
        if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[OpenAI] ✓ Success (${result.usage?.prompt_tokens || 0} in, ${result.usage?.completion_tokens || 0} out)`);
        return {
          success: true, result: result.text, model,
          promptTokens: result.usage?.prompt_tokens,
          completionTokens: result.usage?.completion_tokens,
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[OpenAI] ✓ Success (${usage?.prompt_tokens || 0} in, ${usage?.completion_tokens || 0} out)`);
      ...
      if (process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true') console.log(`[OpenAI] Exception: ${(err as Error).message}`);
