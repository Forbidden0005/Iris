/**
 * iris-cli RAG API Server
 * 
 * Exposes codebase RAG (keyword + import graph + semantic) as HTTP API
 * so iris-lead, Telegram, Dashboard can search code files.
 * 
 * Usage:
 *   iris serve --port 5030
...
export async function startRagServer(options: RagServerOptions = {}) {
  ...
  app.get('/health', (req, res) => {
    res.json({ ok: true, service: 'iris-cli-rag', version: '1.0.0' });
  });
  ...
  return new Promise<void>((resolve) => {
    app.listen(port, host, () => {
      console.log(`✅ iris-cli RAG API server running on http://${host}:${port}`);
