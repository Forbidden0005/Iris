...
import { GeminiToolAdapter } from '../tools/gemini/iris-adapter.js';
...
function readRtToken(): string {
  try {
    const p = join(homedir(), '.iris', 'iris.json');
...
async function handleStandaloneChat(options: UnifiedServerOptions, body: Record<string, unknown>) {
  ...
  // iris-cli is a code execution engine — always use the full pipeline
...
async function handleConnectedChat(options: UnifiedServerOptions, body: Record<string, unknown>) {
  ...
  if (control.passthroughRequested || control.engine) {
    try {
      const agent = String(body?.agent || 'iris-main');
...
async function handleOpenAIChatCompletions(options: UnifiedServerOptions, body: Record<string, unknown>) {
  const model = String(body?.model || 'iris');
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/api/rag/stats') {
        ...
        try {
          ...
          const query = getQuery(req);
          const projectDir = query.get('projectDir') || options.projectDir;
          const cacheDir = process.env.IRIS_RAG_CACHE_DIR || `${projectDir}/.iris/rag-cache`;
      ...
      if (req.method === 'GET' && path === '/v1/models') {
        ...
        return json(res, 200, {
          object: 'list',
          data: [
            { id: 'iris', object: 'model', created: 1700000000, owned_by: 'iris' },
            ...agents
          ]
        });
      }
      ...
      if (req.method === 'GET' && path === '/v1/status') {
        ...
        return json(res, 200, {
          ...
          l2: {
            unifiedRouter: process.env.IRIS_USE_UNIFIED_ROUTER === 'true',
            dualL2: process.env.IRIS_DUAL_L2_ENABLED === 'true'
          },
          queueDepth,
          pipeline: {
