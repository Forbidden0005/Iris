...
export async function createSrcBatchPlan(options: SrcBatchPlanOptions, cwd = process.cwd()): Promise<{
  ...
  const specPath = options.specPath || '.iris/src-batch.spec.yaml';
  const full = join(cwd, specPath);
  await mkdir(join(full, '..'), { recursive: true });

  const yaml = [
    'name: iris-src-batch-plan',
    'description: Generated by iris src batch-plan',
    `on: ${repos[0]}`,
    'steps:',
    `  - run: |`,
    ...
    '  body: |',
    '    Generated by `iris src batch-plan` in dry-run mode.'
  ].join('\n');
