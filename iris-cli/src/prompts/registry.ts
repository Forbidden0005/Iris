...
export const PROMPT_TEMPLATES: Record<string, PromptTemplate> = {
  ...
  'router-v1': {
    ...
    version: '1.0.0',
    role: 'Router (Tier 1)',
    basePrompt: `You are an intelligent task router for iris-cli.
...
{"decision":"CHAT|CODE|DISPATCH","agent":"iris-xxx if needed","task":"reformulated","response":"if CHAT"}`,
...
- \`iris-coder\` - Complex multi-file features
- \`iris-coder-front\` - React/Vue/UI components
- \`iris-coder-back\` - API endpoints, services, databases
- \`iris-frontend\` - CSS/styling/animations

**Quality Assurance:**
- \`specialist-qa\` - Test generation, contract testing, DOD validation
- \`iris-qa\` - Full audits, security + functionality

**Architecture:**
- \`specialist-pm\` - Planning artifacts only (you already used this!)
- \`iris-architect\` - System design, infrastructure

**Specialized:**
- \`iris-security\` - Security audits (OWASP)
- \`iris-github\` - Git operations, PRs
- \`iris-copywriter\` - Documentation
  ...
  "units": [
    {
      ...
      "requiredPersona": "executor-code|iris-coder|iris-coder-front|iris-coder-back|specialist-qa|iris-qa|etc",
...
❌ Missing file paths
❌ Wrong persona assignments (e.g., iris-qa writing code)
...
- \`iris-security\`: security-review, risk-assessment, policy-enforcement
- \`iris-coder-front\`: frontend, ui, ux, component-design
- \`iris-coder-back\`: backend, api-design, data-modeling
...
// Standalone persona coverage aligned with iris's broader 20-role roster.
