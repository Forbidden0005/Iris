...
export class DualL2Planner {
  ...
  private getReasoningModel(): string | undefined {
    const model = String(process.env.IRIS_REASONING_MODEL || '').trim();
    return model || undefined;
  }
  
  private getChatModel(): string | undefined {
    // For structured JSON outputs, use fast chat models, not reasoning models
    const chatModel = String(process.env.IRIS_CHAT_MODEL || '').trim();
  ...
  private getL2AModel(): string | undefined {
    const model = String(process.env.IRIS_L2A_MODEL || '').trim();
    if (model) return model;
    return this.getChatModel();
  }

  private getL2BModel(): string | undefined {
    const model = String(process.env.IRIS_L2B_MODEL || '').trim();
    if (model) return model;
    return this.getChatModel();
  }
    ...
    const parsed = await parseJsonObjectWithRepair(cleaned, {
      ...
      repair: async (repairPrompt: string) => {
        const repaired = await this.executor.execute(repairPrompt, {
          model: String(process.env.IRIS_JSON_REPAIR_MODEL || this.getChatModel() || this.getReasoningModel() || '').trim() || undefined,
          temperature: 0,
          maxTokens: 1500
        });
  ...
  private async agenticRepoScan(task: string): Promise<string> {
    ...
    try {
      // 1. Project structure
      const tree = this.shellSafe(`find ${cwd} -maxdepth 2 -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/dist/*' -not -path '*/.iris/*' | head -60`);
      if (tree) {
  ...
  private async generatePlanningArtifacts(
    ...
    const baseDir = process.env.IRIS_PIPELINE_ARTIFACT_DIR
      ? resolve(process.env.IRIS_PIPELINE_ARTIFACT_DIR)
      : resolve(process.cwd(), '.iris', 'pipeline-artifacts', traceId);
      ...
      content: `Return ONLY valid JSON with this structure:
{
  "units": [
    {
      ...
      "requiredPersona": "iris-coder-back",
  ...
  "requiredPersonas": ["iris-coder-back", "iris-qa", "iris-copywriter"],
  "estimatedCost": 0.001
}
...
- Use requiredPersona to indicate the TYPE of work. Available personas:
  ...
  - "iris-coder-back" — backend/API implementation
  - "iris-coder-front" — frontend/UI implementation
  - "iris-qa" — writing tests, test repair, validation
  - "iris-copywriter" — documentation, README, guides
  - "iris-security" — security review, auth implementation
...
Available capability matrix (iris-cli standalone mode):
- All personas run locally via the same L3 executor with persona-specific prompts
- Supported personas: executor-code, iris-coder-back, iris-coder-front, iris-qa, iris-copywriter, iris-security, iris-fixer
...
- APPROVE all supported personas (executor-code, iris-coder-back, iris-qa, iris-copywriter, etc.) — they all run locally
...
APPROVE: All supported personas (executor-code, iris-coder-back, iris-qa, iris-copywriter, iris-security, iris-fixer, iris-coder-front)
