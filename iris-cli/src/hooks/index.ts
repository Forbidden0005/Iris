 ...
 * Configuration lives in .iris/hooks.json or IRIS_HOOKS_FILE env var.
 *
 ...
 *
 * Example .iris/hooks.json:
 * {
 *   "hooks": {
 *     "PreToolUse": [
 *       { "matcher": "shell|run_cmd", "command": "node .iris/validate-shell.js", "timeout": 5000 }
 *     ],
 *     "PostToolUse": [
 *       { "matcher": ".*", "command": "node .iris/log-tool.js", "timeout": 3000 }
 *     ]
 *   }
 * }
 */
...
function getConfigPath(baseDir: string = process.cwd()): string {
  return process.env.IRIS_HOOKS_FILE || join(baseDir, '.iris', 'hooks.json');
}
