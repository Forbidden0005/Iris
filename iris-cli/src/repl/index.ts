...
const AVAILABLE_ENGINES = ['auto', 'cursor', 'cursor-cli', 'claude', 'claude-cli', 'gemini', 'gemini-cli', 'codex', 'codex-cli', 'iris-cli'];
...
async function listInstalledSkills(): Promise<Array<{ name: string; type: 'knowledge' | 'api'; path: string }>> {
  const skillsRoot = join(homedir(), '.iris', 'skills');
...
function resolveConfiguredReplModel(repoConfig?: Required<RepoConfig>): string {
  ...
  const envCandidates = [
    process.env.IRIS_CHAT_MODEL,
    process.env.IRIS_ROUTER_MODEL,
    process.env.IRIS_EXECUTION_MODEL
  ].map(value => String(value || '').trim()).filter(Boolean);
  if (envCandidates.length > 0) return envCandidates[0];

  const swarmCfg = readJsonFile(join(homedir(), '.iris', 'iris.json')) || {};
  ...
  const sharedCandidates = [
    sharedEnv.IRIS_CHAT_MODEL,
    sharedEnv.IRIS_ROUTER_MODEL,
    sharedEnv.IRIS_EXECUTION_MODEL
...
function buildModelSummary(projectDir: string, state: ReplState): ModelSummary {
  const envMode = String(process.env.IRIS_INTERFACE_MODE || '').toLowerCase();
  const mode: 'connected' | 'standalone' =
    envMode === 'connected' ? 'connected' : (state.useGateway ? 'connected' : 'standalone');

  const policyPath = join(projectDir, '.iris', 'model-policy.json');
...
function printModelSummary(summary: ModelSummary) {
  ...
  console.log(`  Agent models (~/.iris/iris.json): ${summary.agentModels.length ? summary.agentModels.join(', ') : '(none found)'}`);
  console.log(`  Providers configured: ${summary.providerKeys.length ? summary.providerKeys.join(', ') : '(none found)'}`);
  console.log(chalk.gray('\n  Change models with: /model, /stack, .iris/model-policy.json, ~/.iris/iris.json\n'));
}

async function buildRepoBootstrap(projectDir: string): Promise<RepoBootstrap> {
  const ignored = new Set(['.git', 'node_modules', '.iris', 'dist']);
...
function answerLocalMetaQuestion(input: string, summary: ModelSummary): string | null {
  ...
  if (/\b(solo mode|standalone mode|connected mode|are you in solo mode)\b/.test(lower)) {
    ...
    return 'You are in connected mode. Requests route through iris-lead/gateway for multi-agent orchestration.';
  }
  ...
  if (
    ...
    return [
      ...
      `Agent models: ${agents}.`,
      'Use /stack for full details, then change via /model, /stack, .iris/model-policy.json, or ~/.iris/iris.json.'
    ].join(' ');
  }

  if (/\b(change|modify|set|update).*(models?|model)\b/.test(lower)) {
    return 'Yes. Use /stack (models and providers per tier), or edit .iris/model-policy.json and ~/.iris/iris.json for persistent model changes.';
  }
...
function answerFromBootstrap(input: string, summary: ModelSummary, bootstrap: RepoBootstrap): string | null {
  ...
  if (/^(hi|hey|hello|yo|sup|hola|howdy|hej|oi|what'?s? up|wh?at up|how('?s it going|'?re you|( are)? you doin|( are)? ya)|good (morning|afternoon|evening)|gm|gn)\b/.test(lower)) {
    const greetings = [
      ...
      'What\'s up! I\'m your coding iris. Drop a task or ask /help for commands.',
    ];
  ...
  if (
    /\b(how does this system work|explain (the )?(system|architecture)|what is iris-cli|tell me about iris-cli)\b/.test(lower)
  ) {
    const docs = bootstrap.docs.slice(0, 5).join(', ') || '(no docs indexed)';
    const keys = bootstrap.keyFiles.slice(0, 6).join(', ') || '(no key files found)';
    return [
      `iris CLI is a multi-layer orchestrator in ${summary.mode} mode.`,
...
function printHelp(uiMode: 'repl' | 'tui' = 'repl') {
  ...
  console.log(chalk.blue.bold('║                       IRIS REPL COMMANDS                             ║'));
  console.log(chalk.blue.bold('╚══════════════════════════════════════════════════════════════════════╝\n'));
  ...
  console.log('    /engine <name>     Switch engine (auto|cursor|claude|gemini|codex|iris-cli)');
...
function buildPrompt(state: ReplState, isProcessing: boolean, uiMode: 'repl' | 'tui' = 'repl'): string {
  const prefix = uiMode === 'tui' ? 'iris-tui' : 'iris';
...
export async function startRepl(options: ReplOptions): Promise<void> {
  ...
  const envInterfaceMode = String(process.env.IRIS_INTERFACE_MODE || '').toLowerCase();
      ...
      const ifaceAnswer = await inquirer2.prompt([
        {
          ...
          choices: [
            ...
            {
              name: 'connected - Route via iris-lead/gateway specialists',
              value: 'connected',
              short: 'connected'
            }
          ],
  ...
  process.env.IRIS_INTERFACE_MODE = selectedInterfaceMode;
  ...
  const bannerSeenFile = join(projectDir, '.iris', 'repl-banner-seen');
  const replAuditPath = join(projectDir, '.iris', 'repl-events.jsonl');
  ...
  if (shouldRenderBanner) {
    ...
    try {
      await mkdir(join(projectDir, '.iris'), { recursive: true });
  ...
  const handleSlashCommand = async (rawInput: string): Promise<boolean> => {
    ...
    if (command === '/info') {
      ...
      const preferredEngines = Array.isArray(repoConfig?.cli?.preferredEngines)
        ...
        : [];
      const chatModel = String(process.env.IRIS_CHAT_MODEL || '(env not set)');
      const routerModel = String(process.env.IRIS_ROUTER_MODEL || '(env not set)');
      const reasoningModel = String(process.env.IRIS_REASONING_MODEL || '(env not set)');
      const l2aModel = String(process.env.IRIS_L2A_MODEL || '(env not set)');
      const l2bModel = String(process.env.IRIS_L2B_MODEL || '(env not set)');
      const qaModel = String(process.env.IRIS_QA_MODEL || '(env not set)');
      const l1Model = String(process.env.IRIS_L1_MODEL || '(env not set)');
      const l3Model = String(process.env.IRIS_L3_MODEL || '(env not set)');
      const l3ReviewModel = String(process.env.IRIS_L3_REVIEW_MODEL || '(env not set)');
      const l3FixerModel = String(process.env.IRIS_L3_FIXER_MODEL || '(env not set)');
      const extraValidators = String(process.env.IRIS_L2_EXTRA_VALIDATORS || '(env not set)');
      const executionModel = String(process.env.IRIS_EXECUTION_MODEL || '(env not set)');
      const maxParallelWorkers = String(process.env.IRIS_MAX_PARALLEL_WORKERS || '(env not set)');
      ...
      console.log(`    IRIS_CHAT_MODEL           : ${chatModel}`);
      console.log(`    IRIS_L1_MODEL             : ${l1Model}`);
      console.log(`    IRIS_ROUTER_MODEL         : ${routerModel}`);
      console.log(`    IRIS_REASONING_MODEL      : ${reasoningModel}`);
      console.log(`    IRIS_L2A_MODEL            : ${l2aModel}`);
      console.log(`    IRIS_L2B_MODEL            : ${l2bModel}`);
      console.log(`    IRIS_QA_MODEL             : ${qaModel}`);
      console.log(`    IRIS_L3_MODEL             : ${l3Model}`);
      console.log(`    IRIS_L3_REVIEW_MODEL      : ${l3ReviewModel}`);
      console.log(`    IRIS_L3_FIXER_MODEL       : ${l3FixerModel}`);
      console.log(`    IRIS_L2_EXTRA_VALIDATORS  : ${extraValidators}`);
      console.log(`    IRIS_EXECUTION_MODEL      : ${executionModel}`);
      console.log(`    IRIS_MAX_PARALLEL_WORKERS : ${maxParallelWorkers}`);
    ...
    if (command === '/stack') {
      ...
      const tierShortcuts: Record<string, { env: string; label: string; replKey?: keyof ReplState }> = {
        'l1':      { env: 'IRIS_L1_MODEL', label: 'L1 (chat)', replKey: 'model' },
        'l2':      { env: 'IRIS_REASONING_MODEL', label: 'L2 (reasoning)' },
        'l2a':     { env: 'IRIS_L2A_MODEL', label: 'L2A (decomposer)' },
        'l2b':     { env: 'IRIS_L2B_MODEL', label: 'L2B (validator)' },
        'l3':      { env: 'IRIS_L3_MODEL', label: 'L3 (worker)' },
        'qa':      { env: 'IRIS_QA_MODEL', label: 'L3 QA' },
        'fixer':   { env: 'IRIS_L3_FIXER_MODEL', label: 'L3 Fixer' },
        'review':  { env: 'IRIS_L3_REVIEW_MODEL', label: 'L3 Review' },
      };
      ...
      const stackFieldMap: Record<string, string> = {
        'l1-model': 'IRIS_L1_MODEL',
        'router-model': 'IRIS_ROUTER_MODEL',
        'reasoning-model': 'IRIS_REASONING_MODEL',
        'l2a-model': 'IRIS_L2A_MODEL',
        'l2b-model': 'IRIS_L2B_MODEL',
        'qa-model': 'IRIS_QA_MODEL',
        'l3-model': 'IRIS_L3_MODEL',
        'l3-review-model': 'IRIS_L3_REVIEW_MODEL',
        'l3-fixer-model': 'IRIS_L3_FIXER_MODEL',
        'extra-validators': 'IRIS_L2_EXTRA_VALIDATORS',
        'max-parallel-workers': 'IRIS_MAX_PARALLEL_WORKERS'
      };
      ...
      if (subcommand !== 'show' && !tierShortcuts[subcommand] && !stackFieldMap[subcommand]
        ...
        process.env.IRIS_L1_MODEL = subcommand;
        try {
      ...
      if (subcommand === 'show') {
        ...
        console.log(`    Model  : ${replState.model}${process.env.IRIS_L1_MODEL ? ` (env: ${process.env.IRIS_L1_MODEL})` : ''}`);
        ...
        console.log(`    Reasoning model  : ${process.env.IRIS_REASONING_MODEL || chalk.gray('(unset — uses L1)')}`);
        console.log(`    L2A (decomposer) : ${process.env.IRIS_L2A_MODEL || chalk.gray('(unset)')}`);
        console.log(`    L2B (validator)  : ${process.env.IRIS_L2B_MODEL || chalk.gray('(unset)')}`);
        console.log(`    Router model     : ${process.env.IRIS_ROUTER_MODEL || chalk.gray('(unset)')}`);

        console.log(chalk.cyan('\n  L3 (workers):'));
        console.log(`    Gateway          : ${replState.useGateway ? 'enabled' : 'disabled'}`);
        console.log(`    Worker model     : ${process.env.IRIS_L3_MODEL || chalk.gray('(unset — uses L1)')}`);
        console.log(`    QA model         : ${process.env.IRIS_QA_MODEL || chalk.gray('(unset)')}`);
        console.log(`    Fixer model      : ${process.env.IRIS_L3_FIXER_MODEL || chalk.gray('(unset)')}`);
        console.log(`    Review model     : ${process.env.IRIS_L3_REVIEW_MODEL || chalk.gray('(unset)')}`);
        console.log(`    Max parallel     : ${process.env.IRIS_MAX_PARALLEL_WORKERS || chalk.gray('(unset)')}`);
      ...
      if (subcommand === 'router' || subcommand === 'executor') {
        ...
        process.env.IRIS_ROUTING_ORDER = `${replState.routerProvider},${replState.executorProvider}`;
      ...
      if (stackFieldMap[subcommand]) {
        ...
        if (envKey === 'IRIS_MAX_PARALLEL_WORKERS') {
    ...
    if (command === '/tools') {
      ...
      console.log('  PTY tooling: available via `iris exec`');
    ...
    if (command === '/skills') {
      ...
      if (skills.length === 0) {
        console.log(chalk.yellow('\n  No installed skills found in ~/.iris/skills\n'));
        return true;
      }
      if (requested) {
        ...
        console.log(chalk.gray('\n  API skills run via `iris skill <name>`. Knowledge skills are activated by the runtime when needed.\n'));
        return true;
      }
      console.log(chalk.blue('\n--- Installed Skills ---\n'));
    ...
    if (command === '/summon') {
      ...
      if (!persona || !task) {
        try {
          ...
          console.log(chalk.gray('\n  Usage: /summon <persona> <task>\n  Example: /summon iris-qa write tests for src/auth.ts\n'));
        } catch {}
        return true;
      }
      try {
    ...
    if (command === '/validate') {
      ...
      try {
        ...
        const validateTask = `You are iris-judge, a blind code validator. Review these recent changes and provide a structured assessment.
...
## Changed files\n${diffStat || 'No recent changes'}\n\n## Code\n${codeSnippets || 'No code to review'}`;
  ...
  rl.on('line', async (input: string) => {
    ...
    try {
      ...
      if (/\b(switch|set|use).*(solo|standalone)\b/.test(lower)) {
        ...
        process.env.IRIS_INTERFACE_MODE = 'standalone';
        console.log(chalk.cyan('\n  ┌─ Response'));
      ...
      if (/\b(switch|set|use).*(connected|gateway)\b/.test(lower)) {
        ...
        process.env.IRIS_INTERFACE_MODE = 'connected';
        console.log(chalk.cyan('\n  ┌─ Response'));
        console.log(chalk.white('  Switched to connected mode. Gateway/iris-lead orchestration is now preferred.'));
        console.log('  └─\n');
        isProcessing = false;
        rl.prompt();
        return;
      ...
      const agent = route.agent || 'iris-main';
      ...
      if (route.decision === 'CHAT') {
        ...
        const responseText = route.response || 
          "I'm iris-cli, a multi-agent coding orchestrator. Ask me to build something, review code, or dispatch to specialists!";
        console.log(chalk.cyan('\n  ┌─ Response'));
        logger.printWithHighlight(`  ${responseText}`);
        console.log('  └─\n');
      ...
      const useLegacyStandalone = String(process.env.IRIS_LEGACY_ROUTER || '').toLowerCase() === 'true';
      ...
      const useDirectCliEngine = standaloneMode && selectedStandaloneEngine !== 'auto' && selectedStandaloneEngine !== 'iris-cli';
      ...
      const engineUsed = typedResult.engine || (useDirectCliEngine ? selectedStandaloneEngine : (standaloneMode ? 'iris-cli' : (dispatchOpts.engine || replState.engine || 'gateway')));
        ...
        if (shouldAutoApply) {
          try {
            ...
            if (isRiskBlocked(report.risk, policy.riskThreshold, policy.forceAutoApply)) {
              ...
              console.log(chalk.gray('  Use /preview and /apply, or set IRIS_FORCE_AUTO_APPLY=true to override.'));
              console.log();
              isProcessing = false;
              rl.prompt();
              return;
  ...
  rl.on('close', async () => {
    ...
    console.log(chalk.cyan('\n  Session saved to .iris/ — run "iris repl" to continue.\n'));
