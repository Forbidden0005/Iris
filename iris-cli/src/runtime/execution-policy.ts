...
export function getExecutionPolicy(input: Partial<ExecutionPolicy> = {}): ExecutionPolicy {
  const retryAttempts = Number(input.retryAttempts ?? process.env.IRIS_RETRY_ATTEMPTS ?? 2);
  const retryBackoffMs = Number(input.retryBackoffMs ?? process.env.IRIS_RETRY_BACKOFF_MS ?? 600);
  const rawThreshold = String(input.riskThreshold ?? process.env.IRIS_RISK_THRESHOLD ?? 'high').toLowerCase();
  ...
  const strictPreflight =
    ...
    String(process.env.IRIS_STRICT_PREFLIGHT || '').toLowerCase() === 'true';
  const forceAutoApply =
    Boolean(input.forceAutoApply) ||
    String(process.env.IRIS_FORCE_AUTO_APPLY || '').toLowerCase() === 'true';
  const diffFirst = String(process.env.IRIS_DIFF_FIRST || 'true').toLowerCase() !== 'false';
