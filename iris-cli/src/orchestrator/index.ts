...
const ROUTING_SYSTEM_PROMPT = `You are the intelligent routing system for iris CLI (always lowercase "iris"), a standalone agentic coding engine.
...
export class Orchestrator {
  ...
  async route(input: string): Promise<RouteResult> {
    ...
    if (llmDecision) {
      if (llmDecision.decision === RouteDecision.CHAT && this.isExecutionIntent(input)) {
        ...
        llmDecision.agent = llmDecision.agent || 'iris-coder';
    ...
    const isStandalone = String(process.env.IRIS_INTERFACE_MODE || 'standalone').toLowerCase() !== 'connected';
    ...
    // Planning/PM tasks - roadmap, planning, architecture, research
      ...
      result = { decision: isStandalone ? RouteDecision.CODE : RouteDecision.DISPATCH, agent: isStandalone ? 'iris-coder' : 'iris-pm', task: input };
    }
    // Specialist dispatch detection
    else if (lower.includes('ask') || lower.includes('tell')) {
      if (isStandalone) {
        result = { decision: RouteDecision.CODE, agent: 'iris-coder', task: input };
      } else if (lower.includes('fixer') || lower.includes('fix')) {
        result = { decision: RouteDecision.DISPATCH, agent: 'iris-fixer', task: input };
      } else if (lower.includes('qa') || lower.includes('test')) {
        result = { decision: RouteDecision.DISPATCH, agent: 'iris-qa', task: input };
      } else if (lower.includes('frontend') || lower.includes('ui')) {
        result = { decision: RouteDecision.DISPATCH, agent: 'iris-frontend', task: input };
      } else if (lower.includes('security') || lower.includes('audit')) {
        result = { decision: RouteDecision.DISPATCH, agent: 'iris-security', task: input };
      } else {
        result = { decision: RouteDecision.DISPATCH, agent: 'iris-main', task: input };
      }
    }
    ...
    // Simple questions about system capabilities
      ...
      result = { 
        ...
        response: "I'm iris-cli, a multi-agent orchestration system. I can build code, plan projects, fix bugs, review security, and coordinate specialists. Try asking me to build something or create a roadmap!",
      ...
      result = { decision: isStandalone ? RouteDecision.CODE : RouteDecision.DISPATCH, agent: isStandalone ? 'iris-coder' : 'iris-main', task: input };
    }
  ...
  private shouldUseUnifiedRouter(): boolean {
    const explicitLegacy = String(process.env.IRIS_LEGACY_ROUTER || '').trim().toLowerCase();
    if (explicitLegacy === '1' || explicitLegacy === 'true' || explicitLegacy === 'yes') {
      return false;
    }
    const explicit = String(process.env.IRIS_USE_UNIFIED_ROUTER || '').trim().toLowerCase();
  ...
  private normalizeAgentName(raw?: string): string | undefined {
    ...
    const aliases: Record<string, string> = {
      fixer: 'iris-fixer', thefixer: 'iris-fixer', crewfixer: 'iris-fixer',
      coder: 'iris-coder', thecoder: 'iris-coder', crewcoder: 'iris-coder',
      qa: 'iris-qa', theqa: 'iris-qa', crewqa: 'iris-qa',
      frontend: 'iris-frontend', thefrontend: 'iris-frontend', crewfrontend: 'iris-frontend',
      main: 'iris-main', crewmain: 'iris-main',
      security: 'iris-security', crewsecurity: 'iris-security',
      pm: 'iris-pm', crewpm: 'iris-pm',
      copywriter: 'iris-copywriter', crewcopywriter: 'iris-copywriter',
    };
    return aliases[lower] || (raw.startsWith('iris-') ? raw : undefined);
  }
  ...
  private async parseRouteJson(raw: string, fallbackTask: string): Promise<RouteResult | null> {
    try {
      ...
      return {
        decision,
        agent: parsed.agent || (decision === RouteDecision.CODE ? 'iris-coder' : undefined),
  ...
  private async routeWithLLM(input: string): Promise<RouteResult | null> {
    ...
    const routingOrder = (process.env.IRIS_ROUTING_ORDER || 'grok,gemini,deepseek')
      .toLowerCase()
      .split(',')
      .map(s => s.trim());
  ...
  async executeAgentic(
    ...
    try {
      ...
      const result = await this.pipeline.execute({
        userInput: fullTask,
        sessionId: options.sessionId || 'iris-cli',
      ...
      return {
        ...
        model: options.model || process.env.IRIS_EXECUTION_MODEL || 'gemini-2.5-flash',
  ...
  async parseAndApplyToSandbox(agentOutput: string): Promise<string[]> {
    ...
    while (i < lines.length) {
      ...
      // Support @@WRITE_FILE format (iris tool syntax)
