...
async function loadPipelineRunEvents(traceId: string, baseDir = process.cwd()): Promise<PipelineRunEvent[]> {
  const path = join(baseDir, '.iris', 'pipeline-runs', `${traceId}.jsonl`);
...
export async function main(args = []) {
  ...
  if (['doctor', 'update', 'version'].includes(firstArg)) {
    ...
    lightweight
      .command('update')
      .description('Check for updates and install latest iris-cli globally')
      ...
      .action(async options => {
        ...
        if (cmp >= 0) {
          ...
          console.log(chalk.gray('Run: npm i -g iris-cli@latest'));
        }
      });

    lightweight
      .command('version')
      .description('Show iris-cli version')
      .action(async () => {
  ...
  if (normalizedArgs.includes('--legacy-router')) {
    process.env.IRIS_LEGACY_ROUTER = 'true';
    process.env.IRIS_USE_UNIFIED_ROUTER = 'false';
    const idx = normalizedArgs.indexOf('--legacy-router');
    normalizedArgs.splice(idx, 1);
    args = normalizedArgs;
  }

  // Show banner on first launch (or always if IRIS_SHOW_BANNER=1)
  const bannerFile = join(process.env.HOME || homedir(), '.iris', 'cli-banner-seen');
  const showAlways = process.env.IRIS_SHOW_BANNER === '1';
...
`;
  ...
  // Load env vars from ~/.iris/iris.json (set via Dashboard → Settings)
  // These override process.env only if not already set (env vars take precedence)
  try {
    const swarmCfgPath = join(homedir(), '.iris', 'iris.json');
  ...
  if (String(process.env.IRIS_RAG_MODE || 'auto').toLowerCase() !== 'off') {
  ...
  program
    .name('iris')
    .description('iris CLI - Agent orchestration made simple')
    .version(cliVersion)
    .addHelpText('after', `
Examples:
  $ iris doctor                          Check Node/git/Ollama/config health
  $ iris chat "explain this repo"        One-shot chat, routed to the best agent
  $ iris auto "fix the failing test"     Autonomous mode, iterates until done
  $ iris auto "add input validation" --auto-apply
                                          Autonomous mode, applies edits automatically

First time here? See docs/RUNNING-LOCALLY.md for local (Ollama) setup,
or run "iris doctor" to check your environment before running a task.
`);
  ...
  program
    .command('chat')
    .description('Chat with iris (automatically routed to best agent)')
    ...
    .option('--iris', 'Use full multi-agent iris via gateway (like OpenCode PM loop)', false)
    ...
    .addHelpText('after', `
Examples:
  $ iris chat "what does this function do?"
  $ iris chat "summarize recent commits" --stdin < git-log.txt
  $ iris chat "refactor auth.js" --apply           Apply resulting edits to disk
  $ iris chat "describe the API" --docs            Retrieve matching docs as context
...
`)
    .action(async (inputArray, options) => {
      ...
      try {
        ...
        // iris-CLI: use connected mode (gateway dispatch) when --gateway or --iris is specified
        // Otherwise use standalone agentic executor with built-in tools
        const useConnected = Boolean(options.gateway || options.iris);
        ...
        // iris-CLI uses agentic executor (standalone) unless explicitly connected
        const selectedStandaloneEngine = normalizeStandaloneEngine(options.engine || '');
        const useDirectCliEngine = !useConnected && !useLegacyStandalone && selectedStandaloneEngine !== 'auto' && selectedStandaloneEngine !== 'iris-cli';
        ...
        if (route.decision === 'CHAT' || route.decision === 'CODE' || route.decision === 'DISPATCH') {
          ...
          let appliedPaths: string[] = [];
          if (edits.length > 0 && (options.apply || options.iris)) {
          ...
          if (edits.length > 0) {
            if (appliedPaths.length > 0) {
              ...
              logger.success(`Added changes to ${edits.length} files in sandbox. Run "iris preview" to review.`);
            }
          }
        } else if (route.decision === 'SKILL') {
          logger.info('Detected skill request. Please use "iris skill <name>" for now.');
        }
      } catch (error) {
...
Examples:
  ...
  $ iris auto "add tests for the parser" --auto-apply
  $ iris auto "migrate to the new API" --max-iterations 20
...
`)
        ...
        if (options.autoApply) {
          try {
            ...
            if (options.escalateRisk && isSeverityAtLeast(patchRisk.riskLevel, String(options.riskThreshold || 'high').toLowerCase() as RiskLevel)) {
              ...
              const qa = await dispatchWithFallback(agentRouter, 'iris-qa', escalationTask, {
                project: projectDir,
                sessionId: await sessionManager.getSessionId(),
                gateway: options.gateway
              }, fallbackModels, checkpoints, runId);
              const sec = await dispatchWithFallback(agentRouter, 'iris-security', escalationTask, {
                project: projectDir,
                sessionId: await sessionManager.getSessionId(),
                gateway: options.gateway
              }, fallbackModels, checkpoints, runId);
            ...
            logger.info('Run "iris apply" manually to apply changes.');
          }
        } else {
          logger.info('\nRun "iris apply" to write changes to disk, or "iris preview" to review.');
        }
      }
  ...
  program
    .command('status')
    .description('Show iris orchestration status dashboard')
    .action(async () => {
  ...
  program
    ...
    .action(options => {
      // iris-CLI is always standalone - no connected mode
    ...
    .action(async options => {
      try {
        ...
        const capabilityHandshake = getCapabilityHandshake(
          String(process.env.IRIS_INTERFACE_MODE || 'standalone').toLowerCase() === 'connected'
            ? 'connected'
            : 'standalone'
        );
        ...
        if (edits.length > 0) {
          logger.success(`Staged ${edits.length} file change(s). Run "iris preview" then "iris apply".`);
        }
      } catch (error) {
  ...
  program
    ...
    .option('--out <path>', 'Output path for --visualize HTML', join(process.cwd(), '.iris', 'repo-graph.html'))
      ...
      try {
        if (options.graph || options.visualize) {
          ...
          if (options.visualize) {
            const htmlPath = String(options.out || join(process.cwd(), '.iris', 'repo-graph.html'));
  ...
  program
    ...
    .action(async (command, args, options) => {
      ...
      if (looksLikeNaturalLanguage) {
        logger.info('Interpreting `iris exec` input as a one-shot task. Use `iris exec <cmd> [args...]` for PTY commands.');
        try {
    ...
    .action(async (taskArray, options) => {
      ...
      await Promise.all(branches.map(async (b) => {
        try {
          ...
          const result = await agentRouter.dispatch('iris-coder', b.prompt, {
            project: projectDir,
      ...
      if (choice !== 'none') {
        ...
        logger.info(`Switched to branch: ${choice}. Use "iris preview" to review or "iris merge ${choice} main" to merge.`);
      }
    });
  ...
  program
    .command('repos-sync')
    .description('Sync and store sibling repository snapshots to .iris/multi-repo-sync.json')
    .action(async () => {
  ...
  program
    ...
    .option('--upload', 'Upload local .iris session/corrections to team store')
    .option('--download', 'Download shared team context into local .iris')
  ...
  const configCmd = program
    .command('config')
    .description('Manage repo-level configuration in .iris/config.json and .iris/config.local.json');
  ...
  program
    ...
    .action(async (requestArray, options) => {
      ...
      if (intent.kind === 'unknown') {
        ...
        logger.info('  iris github "list open issues limit 20"');
        logger.info('  iris github "create issue \\"Fix login bug\\" body: repro steps..."');
        logger.info('  iris github "update issue #42 close"');
        logger.info('  iris github "create draft pr \\"Refactor auth\\" body: summary..."');
        process.exit(1);
      }
  ...
  program
    ...
    .option('--tts-skill <id>', 'iris skill for TTS', 'elevenlabs.tts')
    .action(async options => {
  ...
  program
    ...
    .option('--agent <id>', 'Agent to run review with', 'iris-qa')
  ...
  program
    ...
    .action(async options => {
      ...
      if (options.writeSummary) {
        ...
        await writeFile(join(process.cwd(), '.iris', 'context-summary.md'), `${summary}\n`, 'utf8');
      }
  ...
  mcp
    .command('list')
    .description('List local MCP servers from .iris/mcp-servers.json')
    .action(async () => {
  ...
  program
    .command('clear')
    .description('Clear local iris-cli session state (.iris)')
    .action(async () => {
      try {
        await sessionManager.clear();
        logger.success('Cleared session state in .iris/');
      } catch (error) {
  ...
  program
    .command('skill')
    .description('Call a iris skill by name')
  ...
  program
    ...
    .option('--reflect-agent <id>', 'Agent used for reflect step', 'iris-main')
    ...
    .action(async (taskArray, options) => {
        ...
        for (const step of plan.steps) {
          ...
          try {
            ...
            const reflect = await dispatchWithFallback(
              agentRouter,
              options.reflectAgent || 'iris-main',
              reflectPrompt,
              {
                sessionId: await sessionManager.getSessionId(),
                project: process.cwd()
            ...
            logger.info(chalk.gray(`Reflect (${options.reflectAgent || 'iris-main'}): ${String(reflect.result.result || '').slice(0, 180)}`));
      ...
      logger.success('Plan execution complete. Use "iris preview" to review changes.');
      await checkpoints.finish(runId, 'completed');
      if (options.memory !== false) {
        try {
          await agentKeeper.compact();
  ...
  program
    ...
    .action(async (options) => {
      ...
      if (ready.length === 0) {
        console.log(chalk.yellow('\nNo engine is fully ready yet. Install/login first, then rerun `iris auth --link --apply`.'));
        return;
      }
      ...
      if (!shouldApply) {
        console.log(chalk.gray('Use --apply to persist this into .iris/config.local.json'));
        return;
      }
      ...
      if (implicitConnectMode) {
        console.log(chalk.gray('  mode: implicit (iris auth)'));
      }
  ...
  program
    .command('correction')
    .description('Record a user correction for local training data (.iris/training-data.jsonl)')
    ...
    .action(async options => {
      ...
      const watcher = startWatchMode(root, async event => {
        if (event.type === 'todo_detected') {
          ...
          if (confirm) {
            await agentRouter.dispatch('iris-coder', `Implement TODOs in ${event.file}:\n${todoText}`, {
              sessionId: await sessionManager.getSessionId(),
              project: process.cwd()
            });
  ...
  program
    .command('browser-fix')
    .description('Collect browser errors / failing UI tests and dispatch to iris-fixer')
    .requiredOption('--url <url>', 'Target URL')
    .option('--duration-ms <ms>', 'Capture duration in milliseconds', '5000')
    ...
    .action(async options => {
      ...
      const result = await agentRouter.dispatch('iris-fixer', task, {
        sessionId: await sessionManager.getSessionId(),
        project: process.cwd()
      });
  ...
  program
    ...
    .action(async options => {
      ...
      console.log(chalk.blue('iris doctor'));
      checks.forEach(check => {
        let marker = check.ok ? chalk.green('✓') : chalk.red('✗');
        if (check.name === 'CLI update status' && String(check.details || '').toLowerCase().includes('update available')) {
          marker = chalk.yellow('!');
      ...
      try {
        ...
        if (!options.yes) {
          const { confirm } = await (await import('inquirer')).default.prompt([{
            type: 'confirm',
            name: 'confirm',
            message: `Install iris-cli@${options.tag || 'latest'} globally now?`,
            default: true
          }]);
          if (!confirm) {
        ...
        await new Promise((resolve, reject) => {
          const child = spawn('npm', ['install', '-g', `iris-cli@${options.tag || 'latest'}`], {
            stdio: 'inherit',
            shell: false
          });
        ...
        logger.success(`Updated iris-cli to ${refreshed || options.tag || 'latest'}.`);
      } catch (error) {
    ...
    .action(async (branch, options) => {
      ...
      try {
        ...
        if (isRiskBlocked(report.risk, policy.riskThreshold, policy.forceAutoApply)) {
          ...
          logger.warn('Run "iris preview" to inspect changes, then re-run with --force if intentional.');
          process.exit(1);
        }
          ...
          while (attempt < maxRetries && !passed) {
            ...
            try {
              ...
              if (attempt < maxRetries) {
                logger.warn(`Dispatching iris-fixer with parsed diagnostics (attempt ${attempt})...`);
                try {
  ...
  program
    ...
    .action(async options => {
      try {
        ...
        console.log('Run "iris preview" to see the diff.');
      } catch (error) {
  ...
  checkpointCmd
    ...
    .action(async (runId, options) => {
      ...
      if (run.mode === 'plan') {
        logger.info(`Use: iris plan "${run.task}" --resume ${run.runId}`);
        return;
      }
      ...
      const agent =
        String(run.events.find(e => e.type === 'dispatch.completed')?.data?.agent || 'iris-main');
  ...
  program
    ...
    .option('--host <host>', 'Bind host', process.env.IRIS_API_HOST || '127.0.0.1')
    .option('--port <port>', 'Bind port', process.env.IRIS_API_PORT || '4317')
    .action(async (options) => {
      const requestedMode = String(options.mode || 'standalone').trim().toLowerCase();
      if (requestedMode !== 'standalone') {
        logger.error(`Unsupported --mode "${requestedMode}". iris serve only supports standalone mode now.`);
        logger.info('Use: iris serve --port 4097');
        process.exit(1);
      }
      const mode = 'standalone';
  ...
  // ── iris validate — blind AI code review ──
  program
    .command('validate')
