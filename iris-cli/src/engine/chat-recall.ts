 ...
 *
 * Uses the existing session history stored in .iris/session.json
...
async function loadSessionHistories(projectDir: string): Promise<IndexedSession[]> {
  ...
  // Current session
  const sessionPath = join(projectDir, '.iris', 'session.json');
  ...
  // Past session logs (.iris/sessions/ directory if exists)
  const sessionsDir = join(projectDir, '.iris', 'sessions');
  ...
  // Routing log (.iris/routing.log — JSONL format)
  const routingPath = join(projectDir, '.iris', 'routing.log');
