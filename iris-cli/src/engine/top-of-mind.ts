 ...
 *
 * Users create .iris/instructions.md (project-level) or
 * ~/.iris/instructions.md (global) with persistent context
...
export async function loadTopOfMind(projectDir: string = process.cwd()): Promise<string> {
  ...
  // Global instructions (~/.iris/instructions.md)
  const globalPath = join(homedir(), '.iris', 'instructions.md');
  ...
  // Project instructions (.iris/instructions.md)
  const projectPath = join(projectDir, '.iris', 'instructions.md');
