...
export class AgentMemory {
  ...
  constructor(agentId: string, options?: { storageDir?: string }) {
    // Use IRIS_MEMORY_DIR for shared cross-system memory, or local dir for CLI-only
    const baseDir = options?.storageDir || 
      process.env.IRIS_MEMORY_DIR || 
      process.cwd();
    this.storageDir = resolve(baseDir, '.iris', 'agent-memory');
...
/**
 * Get or create a iris-wide memory instance (shared across CLI and gateway)
 */
export function getCrewMemory(crewId: string = 'iris-lead'): AgentMemory {
