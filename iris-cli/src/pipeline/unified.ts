...
export class UnifiedPipeline {
  ...
  private normalizeDecision(raw: string): 'direct-answer' | 'execute-local' | 'execute-parallel' | 'execute-direct' {
    ...
    if (value === 'execute-local' || value === 'code') {
      return process.env.IRIS_ALLOW_EXECUTE_LOCAL === 'true'
        ? 'execute-local'
        : 'execute-parallel';
    }
  ...
  private getReasoningModel(): string | undefined {
    const model = String(process.env.IRIS_REASONING_MODEL || process.env.IRIS_CHAT_MODEL || '').trim();
  ...
  private getRequestedEffortOverride(): EffortLevel | undefined {
    const env = String(process.env.IRIS_EFFORT || '').trim().toLowerCase();
  ...
  private getModelForLayer(layer: 'l1' | 'l3' | 'l3-review' | 'l3-fixer', effort: EffortLevel = 'medium'): string | undefined {
    const envByLayer: Record<typeof layer, string[]> = {
      l1: ['IRIS_L1_MODEL', 'IRIS_ROUTER_MODEL'],
      l3: ['IRIS_L3_MODEL', 'IRIS_EXECUTION_MODEL'],
      'l3-review': ['IRIS_L3_REVIEW_MODEL', 'IRIS_QA_MODEL'],
      'l3-fixer': ['IRIS_L3_FIXER_MODEL', 'IRIS_EXECUTION_MODEL']
    };
  ...
  private getMaxTurnsForEffort(effort: EffortLevel): number {
    const explicit = Number(process.env.IRIS_MAX_TURNS || 0);
  ...
  private getRouterModel(): string | undefined {
    ...
    // If IRIS_REASONING_MODEL is a reasoning-only model (deepseek-reasoner, gemini-*-preview),
    // fall back to chat model for structured output
    const reasoningModel = String(process.env.IRIS_REASONING_MODEL || '').trim();
    if (reasoningModel && 
    ...
    return String(process.env.IRIS_CHAT_MODEL || '').trim() || undefined;
  }

  private getQaModel(): string | undefined {
    const model = String(process.env.IRIS_QA_MODEL || process.env.IRIS_L3_REVIEW_MODEL || process.env.IRIS_REASONING_MODEL || '').trim();
    return model || undefined;
  }

  private getJsonRepairModel(): string | undefined {
    const explicit = String(process.env.IRIS_JSON_REPAIR_MODEL || '').trim();
  ...
  private getJsonParseAttempts(): number {
    const n = Number(process.env.IRIS_JSON_PARSE_MAX_ATTEMPTS || 2);
    if (!Number.isFinite(n) || n < 1) return 2;
    return Math.min(4, Math.floor(n));
  }

  private qaLoopEnabled(): boolean {
    return process.env.IRIS_QA_LOOP_ENABLED === 'true';
  }

  private scaffoldGateEnabled(): boolean {
    const raw = String(process.env.IRIS_SCAFFOLD_GATE_ENABLED || 'true').trim().toLowerCase();
    return raw !== 'false' && raw !== '0' && raw !== 'off';
  }

  private definitionOfDoneEnabled(): boolean {
    const raw = String(process.env.IRIS_DOD_GATE_ENABLED || 'true').trim().toLowerCase();
    return raw !== 'false' && raw !== '0' && raw !== 'off';
  }

  private goldenBenchmarkGateEnabled(): boolean {
    const raw = String(process.env.IRIS_GOLDEN_BENCH_GATE_ENABLED || 'true').trim().toLowerCase();
    return raw !== 'false' && raw !== '0' && raw !== 'off';
  }

  private qaMaxRounds(): number {
    const value = Number(process.env.IRIS_QA_MAX_ROUNDS || 2);
    if (!Number.isFinite(value) || value < 1) return 2;
    return Math.min(5, Math.floor(value));
  }

  private getExtraL2ValidatorModels(): string[] {
    const raw = String(process.env.IRIS_L2_EXTRA_VALIDATORS || '').trim();
    if (!raw) return [];
    return raw
      .split(',')
      .map(v => v.trim())
  ...
  private getMaxParallelWorkers(): number {
    const raw = Number(process.env.IRIS_MAX_PARALLEL_WORKERS || 6);
    if (!Number.isFinite(raw) || raw < 1) return 6;
    return Math.min(32, Math.floor(raw));
  }

  private reviewerEnabled(): boolean {
    const raw = String(process.env.IRIS_L3_REVIEW_ENABLED || 'true').trim().toLowerCase();
    return raw !== 'false' && raw !== '0' && raw !== 'off';
  }

  private getReviewerMaxCycles(): number {
    const raw = Number(process.env.IRIS_L3_REVIEW_MAX_CYCLES || 2);
  ...
  private async passesDeterministicSmallTaskGate(
    ...
    const verbose = process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true';
  ...
  private async reviewAndFixWorkerResult(
    ...
    for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
      ...
      const fixResult = await this.runWorker(context ? `${context}\n\n${fixTask.goal}` : fixTask.goal, {
        ...
        verbose: process.env.IRIS_VERBOSE === 'true' || process.env.IRIS_DEBUG === 'true',
        persona: task.persona,
  ...
  private async recordPipelineMetrics(entry: {
    ...
    try {
      const dir = resolve(process.cwd(), '.iris');
  ...
  private async writeRunCheckpoint(traceId: string, payload: Record<string, unknown>): Promise<void> {
    try {
      const dir = resolve(process.cwd(), '.iris', 'pipeline-runs');
      await mkdir(dir, { recursive: true });
...
If output has blockers, set approved=false.`,
  ...
  private async fixerPatchResponse(
    ...
    const templateId = getTemplateForPersona('iris-fixer');
  ...
  private startCheckpointInterval(traceId: string): void {
    ...
    this._intervalTimer = setInterval(async () => {
      try {
        ...
        const tag = `iris-interval-${traceId.slice(0, 8)}-${this._intervalSnapshotCount}`;
  ...
  private async gitCheckpoint(traceId: string, executionResults?: L3Result): Promise<void> {
    try {
      ...
      const filesSummary = filesChanged.length > 0
        ...
        : 'pipeline changes';
      const msg = `checkpoint(iris-cli): ${filesSummary} [${traceId.slice(0, 8)}]`;
  ...
  private assertMandatoryWorkGraphGates(workGraph: WorkGraph) {
    ...
    if (process.env.IRIS_DUAL_L2_ENABLED !== 'true') return;
          ...
          content: `Return ONLY valid JSON:
{
  ...
  async execute(request: L1Request): Promise<PipelineResult> {
    ...
    try {
      ...
      else if (process.env.IRIS_FORCE_L2 === 'true' && plan.workGraph) {
  ...
  private async l2Orchestrate(
    ...
    const currentModel = this.getRouterModel() || process.env.IRIS_CHAT_MODEL || process.env.IRIS_EXECUTION_MODEL || 'unknown';
    const interfaceMode = String(process.env.IRIS_INTERFACE_MODE || 'standalone').toLowerCase();
    overlays.push({
      type: 'constraints',
      content: `You are iris's CLI assistant, running model "${currentModel}" in ${interfaceMode} mode.
...
When asked about your identity or what model you are, answer: "I'm iris CLI running ${currentModel}." The brand is always lowercase "iris" — never "iris" or "iris".
...
Return ONLY valid JSON:
{
  ...
  "estimatedCost": 0.001
    ...
    const dualL2Enabled = process.env.IRIS_DUAL_L2_ENABLED === 'true';

    // IRIS_FORCE_L2=true bypasses L1 routing and always runs L2 planner (used by enhance-prompt)
    const forceL2 = process.env.IRIS_FORCE_L2 === 'true';
    if ((normalizedDecision === 'execute-parallel' || forceL2) && dualL2Enabled) {
      ...
      if (workGraph) {
        const mode = String(process.env.IRIS_INTERFACE_MODE || 'standalone').toLowerCase() === 'connected'
          ? 'connected'
          : 'standalone';
      ...
      const allowCritical = process.env.IRIS_ALLOW_CRITICAL === 'true';
      if (validation.riskLevel === 'critical' && !allowCritical) {
        throw new Error(
          `CRITICAL RISK detected. Task blocked.\n${validation.concerns.join('\n')}\n` +
          `Use IRIS_ALLOW_CRITICAL=true to override (not recommended).`
        );
      }
    }
    ...
    try {
      ...
      if (shouldUseRag(enhancedTask)) {
        const ragContext = await autoLoadRelevantFiles(enhancedTask, process.cwd(), {
          mode: (process.env.IRIS_RAG_MODE || 'auto') as import('../context/codebase-rag.js').RagMode,
          tokenBudget: Number(process.env.IRIS_RAG_WORKER_BUDGET || 4000),
          maxFiles: Number(process.env.IRIS_RAG_MAX_FILES_LOAD || 6)
        });
...
- Escalate when: ${(task.escalationHints || []).join(' | ')}`,
    ...
    if (process.env.IRIS_DEBUG_PIPELINE) {
  ...
  private async l3ExecuteParallel(
    ...
    const worktreeIsolation = (() => {
      if (process.env.IRIS_WORKTREE_ISOLATION === 'false') return false;
      try {
      ...
      const runUnit = async (unit: typeof batch[number]) => {
        ...
        if (artifactPackId) {
          ...
          const artifactContext = this.contextPacks.retrieve(artifactPackId, {
            ...
            budgetChars: Number(process.env.IRIS_CONTEXT_BUDGET_CHARS || 7000),
            maxChunks: Number(process.env.IRIS_CONTEXT_MAX_CHUNKS || 8)
          });
...
5. After completing work, return a JSON summary: {"output":"what you did","summary":"short summary","edits":["files changed"],"validation":["verification steps"]}`,
      ...
      for (const r of batchResults) {
        ...
        if (unitDef) {
          ...
          this.delegationTuner.recordPerformance({
            persona: r.persona,
            model: String(process.env.IRIS_EXECUTION_MODEL || 'default'),
   ...
   */
  async runWorker(prompt: string, options: { model?: string; maxTurns?: number; verbose?: boolean; priorDiscoveredFiles?: string[]; persona?: string; constraintLevel?: import('../tools/gemini/iris-adapter.js').ConstraintLevel; verificationCommands?: string[] }): Promise<{ output: string; cost?: number; turns?: number; discoveredFiles?: string[] }> {
  ...
  private canUseNativeGeminiToolLoop(modelId: string): boolean {
    ...
    const mode = (process.env.IRIS_TOOL_MODE || 'auto').toLowerCase();
