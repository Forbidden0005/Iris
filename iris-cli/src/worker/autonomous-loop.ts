  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    try {
      ...
      if (isContextLengthError(err) && !reactiveCompacted) {
        ...
        console.error('[iris-cli] Context exceeded — compacted history and retrying');
    ...
    while (
      ...
      recoveryAttempts++;
      console.error(`[iris-cli] Output truncated (finish_reason=${response.finishReason}) — compacting and retrying (attempt ${recoveryAttempts}/2)`);
      clearedHistory = compactTurnHistory(clearedHistory);
      try {
        response = await executeLLM(effectiveTask, config.tools, clearedHistory, abortSignal);
      } catch (err) {
        if (isContextLengthError(err) && !reactiveCompacted) {
          reactiveCompacted = true;
          console.error('[iris-cli] Context exceeded during recovery — compacting again');
          clearedHistory = compactTurnHistory(clearedHistory);
          response = await executeLLM(effectiveTask, config.tools, clearedHistory, abortSignal);
        } else {
          throw err;
