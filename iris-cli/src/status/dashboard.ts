...
export async function getSystemStatus(): Promise<StatusInfo> {
  const status: StatusInfo = {
    ...
    runningTasks: 0,
    models: [],
    gatewayUrl: process.env.IRIS_LEAD_URL || 'http://127.0.0.1:5010',
    version: '0.1.0-alpha'
  };

  // Check which providers have API keys (from iris.json + env vars)
  const providers: string[] = [];
  try {
    const { readFileSync } = await import('node:fs');
    const cfgPath = `${homedir()}/.iris/iris.json`;
  ...
  try {
    // Read auth token from ~/.iris/config.json
    let authToken = '';
    try {
      const { readFileSync } = await import('node:fs');
      const { homedir } = await import('node:os');
      const cfg = JSON.parse(readFileSync(`${homedir()}/.iris/config.json`, 'utf8'));
...
export function renderStatusDashboard(status: StatusInfo, options: StatusRenderOptions = {}): string {
  ...
  const lines = [
    border('┌─[ IRIS-CLI :: AGENTIC CODING ENGINE ]──────────────────────────┐'),
    '',
    ...
    '',
    `   ${chalk.italic.gray('"One idea. One Build. One Iris."')}`,
    '',
    border('└──────────────────────────────────────────────────────────────────┘')
  ];
