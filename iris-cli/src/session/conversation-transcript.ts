 ...
 *
 * Each turn is appended as a single JSON line to .iris/transcript-{sessionId}.jsonl
...
export class ConversationTranscriptStore {
  ...
  async getRecentTurns(sessionId: string, maxTurns = 8): Promise<ConversationTurn[]> {
    ...
    const maxSessionTokens = Number(process.env.IRIS_MAX_SESSION_TOKENS) || 100_000;
    let totalTokens = 0;
