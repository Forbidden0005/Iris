 ...
 * Call this from iris-cli when commands run in project context
 */
...
async function ensureImports() {
  ...
  // Find iris root by looking for package.json with "name": "iris"
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (existsSync(pkgPath)) {
      try {
        ...
        if (pkg.name === 'iris' || existsSync(join(currentDir, 'lib/chat/project-messages.mjs'))) {
          irisRoot = currentDir;
          break;
        }
      } catch (e) {
  ...
  if (!irisRoot) {
    throw new Error('[CLI Bridge] Cannot find iris root from: ' + process.cwd());
  }
  
  const modulePath = join(irisRoot, 'lib/chat/project-messages.mjs');
