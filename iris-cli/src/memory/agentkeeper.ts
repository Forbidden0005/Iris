 ...
 * append-only JSONL store (`.iris/agentkeeper.jsonl`).  Supports retrieval
...
export class AgentKeeper {
  ...
  constructor(baseDir: string, options: AgentKeeperOptions = {}) {
    const storageBase = options.storageDir || process.env.IRIS_MEMORY_DIR || baseDir;
    this.storePath = join(storageBase, '.iris', 'agentkeeper.jsonl');
