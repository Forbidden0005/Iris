...
export class MemoryBroker {
  ...
  constructor(projectDir: string, options: { crewId?: string; storageDir?: string } = {}) {
    ...
    this.keeper = new AgentKeeper(this.projectDir, {
      storageDir: options.storageDir || process.env.IRIS_MEMORY_DIR
    });
    this.factMemory = new AgentMemory(options.crewId || 'iris-lead', {
      storageDir: options.storageDir || process.env.IRIS_MEMORY_DIR || this.projectDir
    });
  }
