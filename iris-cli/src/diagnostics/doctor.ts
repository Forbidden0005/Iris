...
export async function getInstalledCliVersion(): Promise<string | null> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const looksLikeCli =
        pkgName === 'iris-cli' ||
        pkgName === '@iris/iris-cli' ||
        candidate.includes(`${join('iris-cli', 'package.json')}`);
...
export async function getLatestCliVersion(tag = 'latest'): Promise<string | null> {
  try {
    const { stdout } = await execFileAsync('npm', ['view', `iris-cli@${tag}`, 'version'], {
      timeout: 8000
    });
...
export async function isGlobalInstallLinked(): Promise<boolean> {
  try {
    const { stdout } = await execFileAsync('npm', ['-g', 'ls', 'iris-cli', '--depth=0']);
...
export function checkApiKeys(): { configured: string[]; missing: string[]; details: string; hint: string } {
  ...
  if (configured.length === 0) {
    details = 'No API keys found — iris-cli cannot run';
...
export async function runDoctorChecks(options: { gateway?: string; updateTag?: string } = {}) {
  ...
  if (installedVersion && latestVersion) {
    ...
    if (cmp < 0) {
      updateDetails = `Update available: ${installedVersion} -> ${latestVersion} (run "iris update")`;
    } else {
  ...
  return [
    ...
    {
      name: 'iris config present',
      ok: config.ok,
      details: config.path
    },
    {
      name: 'iris gateway reachable',
      ok: gatewayOk,
