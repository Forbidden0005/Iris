/**
 * Adapter to use Gemini CLI tools with iris-cli's sandbox
 */
...
export function constraintLevelForPersona(persona: string): ConstraintLevel {
  ...
  // In standalone iris-cli, all personas run locally and need file access.
...
// Shell timeout: configurable via IRIS_SHELL_TIMEOUT env (seconds), default 120s, max 600s
function getShellTimeout(): number {
  const envVal = parseInt(process.env.IRIS_SHELL_TIMEOUT || '', 10);
...
export class GeminiToolAdapter {
  ...
  private async webFetchTool(params: { url: string }): Promise<ToolResult> {
    ...
    try {
      const res = await fetch(url, {
        headers: { 'User-Agent': 'iris-CLI/1.0' },
  ...
  private async saveMemoryTool(params: { fact: string }): Promise<ToolResult> {
    ...
    const memDir = resolve(this.config.getWorkspaceRoot(), '.iris');
  ...
  private crewDirPath() {
    return resolve(this.config.getWorkspaceRoot(), '.iris');
  }
  ...
  private trackerFilePath() {
    return resolve(this.config.getWorkspaceRoot(), '.iris', 'tracker.json');
  }
  ...
  private async writeTracker(tasks: TrackerTask[]): Promise<void> {
    const dir = resolve(this.config.getWorkspaceRoot(), '.iris');
    await mkdir(dir, { recursive: true });
  ...
  private async spawnAgentTool(params: { task: string; model?: string; max_turns?: number }): Promise<ToolResult> {
    ...
    const model = params.model || process.env.IRIS_WORKER_MODEL || process.env.IRIS_EXECUTION_MODEL || '';
    ...
    try {
      ...
      const result = await runAgenticWorker(task, this.sandbox, {
        ...
        maxTurns,
        stream: false,
        verbose: Boolean(process.env.IRIS_DEBUG),
        tier: 'fast',
   ...
   */
  getToolDeclarations() {
    const dynamicEnabled = process.env.IRIS_GEMINI_DYNAMIC_DECLARATIONS !== 'false';
    ...
    if (!allDecls) {
      ...
      },
      {
        ...
        description: 'Create tracker task in local .iris/tracker.json.',
        parameters: {
          type: 'object',
          properties: {
            title: { type: 'string' },
