...
async function quickTest() {
  console.log(`\n${YELLOW}🚀 IRIS-CLI QUICK TEST${RESET}\n`);
  
  const testDir = mkdtempSync(join(tmpdir(), 'iris-quick-'));
  const crewBin = join(process.cwd(), 'bin/iris.js');
  ...
  try {
    ...
    if (!existsSync(helloPath)) {
      // Check .iris/sandbox.json for staged files
