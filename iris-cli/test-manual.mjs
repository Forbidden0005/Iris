...
`;
    ...
    // Test 1: iris diff (should fail since CLI needs CumulativeDiffSandbox instance)
    info('Test: iris diff command...');
    try {
      const output = execSync('node dist/iris.mjs diff', {
        cwd: TEST_DIR,
        encoding: 'utf-8'
      });
      
      if (output.includes('file1.txt') || output.includes('No pending changes')) {
        success('iris diff works');
      } else {
        error('iris diff unexpected output');
      }
    } catch (err) {
      // Expected - CLI creates regular Sandbox, not CumulativeDiffSandbox
      info('iris diff requires CumulativeDiffSandbox instance (expected)');
    }
...
async function main() {
  ...
  if (passed) {
    ...
    console.log(chalk.cyan('1. Run: node dist/iris.mjs help'));
