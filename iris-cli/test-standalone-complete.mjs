#!/usr/bin/env node
/**
 * IRIS-CLI STANDALONE COMPLETE TEST
 * 
...
const TEST_DIR = `/tmp/iris-cli-test-${Date.now()}`;
...
async function runTestScenario(scenario, index) {
  ...
  try {
    ...
    // Build iris-cli if not already built
    if (!existsSync('dist/iris.mjs')) {
      log('\n🔨 Building iris-cli...', YELLOW);
    ...
    const env = {
      ...
      IRIS_DUAL_L2_ENABLED: scenario.requiresDualL2 ? 'true' : 'false',
      IRIS_CONTEXT_BUDGET_CHARS: '7000',
      IRIS_CONTEXT_MAX_CHUNKS: '8',
    };

    if (USE_DOCKER) {
      env.IRIS_DOCKER_SANDBOX = 'true';
    }
    ...
    const sandboxPath = join(testDir, '.iris', 'sandbox.json');
    ...
    const metricsPath = join(testDir, '.iris', 'pipeline-metrics.jsonl');
...
async function main() {
  section('🧪 Iris-CLI Standalone Complete Test Suite');
  ...
  // Check if iris-cli exists
  if (!existsSync('bin/iris.js')) {
    log('❌ iris-cli not found. Run from iris-cli directory.', RED);
    process.exit(1);
  }
  log('✅ iris-cli found', GREEN);
