#!/usr/bin/env node
/**
 * iris-lead.mjs — Conversational commander (HTTP server)
 *
...
import {
  ...
  IRIS_LEAD_PORT as PORT,
  IRIS_LEAD_PID_PATH as PID_PATH,
  IRIS_LEAD_HISTORY_DIR as HISTORY_DIR,
  IRIS_REPO_ROOT,
  PROJECTS_REGISTRY,
  MAX_HISTORY,
  LLM_TIMEOUT,
...
import { createAgent, removeDynamicAgent } from "./lib/iris-lead/agent-manager.mjs";
import { initTools, execCrewLeadTools } from "./lib/iris-lead/tools.mjs";
import { initBrain, appendToBrain, readGlobalRules, writeGlobalRules, appendGlobalRule, searchWithBrave, searchCodebase } from "./lib/iris-lead/brain.mjs";
import { initPrompts, buildSystemPrompt } from "./lib/iris-lead/prompts.mjs";
import { initClassifier, classifyTask } from "./lib/iris-lead/classifier.mjs";
import { initIntent, writeTaskBrief, parseServiceIntent, messageNeedsSearch, isDispatchIntended } from "./lib/iris-lead/intent.mjs";
import { initBackground, startBackgroundLoop, recordAgentTimeout, getRateLimitFallback, _agentTimeoutCounts, RATE_LIMIT_PATTERN } from "./lib/iris-lead/background.mjs"; // RATE_LIMIT_PATTERN used in RT handler
...
import { initChatHandler, handleChat } from "./lib/iris-lead/chat-handler.mjs";
import { initHttpServer, createAndStartServer } from "./lib/iris-lead/http-server.mjs";
import { initWsRouter } from "./lib/iris-lead/ws-router.mjs";
import {
  initWaveDispatcher,
  pendingDispatches,
  pendingPipelines,
...
import { initIntervalManagers } from "./lib/iris-lead/interval-manager.mjs";

// ── Single instance + canonical PID (dashboard / restart-iris-lead.sh use this path) ──
const _crewLeadLock = acquireStartupLock("iris-lead", {
  port: PORT,
  killStale: false,
  pidFile: PID_PATH,
});
if (!_crewLeadLock.ok) {
  console.error(`[iris-lead] Refusing to start: ${_crewLeadLock.message}`);
  console.error(
    `[iris-lead] Stop the other instance (Services tab, or: bash scripts/restart-iris-lead.sh)`,
  );
  console.error(
    `[iris-lead] If it crashed and left a stale PID file: rm -f ${PID_PATH}`,
  );
  process.exit(1);
}
...
function broadcastSSE(payload) {
  ...
  if (clientCount === 0 && payload?.type === "chat_message") {
    console.log(`[iris-lead] ⚠️ broadcastSSE called but no SSE clients connected (type=${payload.type})`);
  }
  for (const client of sseClients) {
    try { client.write(`data: ${event}\n\n`); } catch { }
  }
  if (clientCount > 0 && payload?.type === "chat_message") {
    console.log(`[iris-lead] SSE broadcast to ${clientCount} client(s): ${payload.role} message sessionId=${payload.sessionId} (${event.length} bytes)`);
  }
}
...
const BG_CONSCIOUSNESS_INTERVAL_MS = Number(process.env.IRIS_BG_CONSCIOUSNESS_INTERVAL_MS) || 15 * 60 * 1000;
let BG_CONSCIOUSNESS_MODEL = (() => {
  if (process.env.IRIS_BG_CONSCIOUSNESS_MODEL) return process.env.IRIS_BG_CONSCIOUSNESS_MODEL;
...
// Reads from env first, then from ~/.iris/iris.json bgConsciousness field.
function loadBgConsciousnessEnabled() {
  if (process.env.IRIS_BG_CONSCIOUSNESS) return /^1|true|yes$/i.test(String(process.env.IRIS_BG_CONSCIOUSNESS));
  const cfg = loadSystemConfig();
...
function loadConfig() {
  ...
  const displayName = agentCfg?.identity?.name || "iris-lead";
  ...
  // Canonical string from config (same as iris.json agents[].model for iris-lead)
...
const BRAIN_PATH = path.join(IRIS_REPO_ROOT, "memory", "brain.md");
const GLOBAL_RULES_PATH = path.join(os.homedir(), ".iris", "global-rules.md");
const IRIS_CFG_FILE = path.join(os.homedir(), ".iris", "iris.json");
...
// Rolling log of RT bus traffic so iris-lead has eyes on the system
...
async function isAgentOnRtBus(agentId) {
  try {
    const rtHost = process.env.IRIS_RT_HOST || "127.0.0.1";
    const rtPort = Number(process.env.IRIS_RT_PORT || 18889);
...
initTools({ historyDir: HISTORY_DIR, irisCfgFile: IRIS_CFG_FILE });
initBrain({ brainPath: BRAIN_PATH, globalRulesPath: GLOBAL_RULES_PATH });
initPrompts({ irisCfgFile: IRIS_CFG_FILE, historyDir: HISTORY_DIR, getAgentPrompts, tryRead, maxDynamicAgents: Number(process.env.IRIS_MAX_DYNAMIC_AGENTS || "5") });
...
/** Execute a skill from iris-lead (used when iris-lead emits @@SKILL in its reply). */
async function executeSkillFromCrewLead(skillName, params) {
  ...
  const swarmCfg = tryRead(path.join(os.homedir(), ".iris", "iris.json")) || {};
  ...
  if (skillDef.type === "cmd") {
    ...
    console.log(`[iris-lead] @@SKILL ${skillName} → cmd: ${cmd}`);
  ...
  console.log(`[iris-lead] @@SKILL ${skillName} → ${method} ${urlStr}`);
  const r = await fetch(urlStr, reqOpts);
  const text = await r.text();
  console.log(`[iris-lead] @@SKILL ${skillName} ← ${r.status} ${text.slice(0, 120).replace(/\n/g, " ")}`);
...
// handleChat → lib/iris-lead/chat-handler.mjs
...
// HTTP server → lib/iris-lead/http-server.mjs
...
const bgConsciousnessRef = {
  ...
  set model(v) {
    ...
    try {
      ...
      console.error(`[iris-lead] Failed to persist bgConsciousnessModel: ${e.message}`);
    }
  },
...
const tmuxBridgeRef = {
  ...
  set enabled(v) {
    ...
    process.env.IRIS_TMUX_BRIDGE = v ? "1" : "0";
    resetTmuxBridge();
  },
};
...
process.on("unhandledRejection", (reason) => {
  ...
  console.error("[iris-lead] unhandled rejection:", msg);
  ...
  if (/EADDRINUSE|EACCES|out of memory|cannot allocate/i.test(msg)) {
    console.error("[iris-lead] FATAL — exiting");
    process.exit(1);
  }
...
process.on("uncaughtException", (err) => {
  ...
  console.error("[iris-lead] uncaught exception:", err?.stack || msg);
  ...
  if (
    ...
    console.error("[iris-lead] Non-fatal uncaughtException — keeping alive");
    return;
  }

  // Fatal errors: port conflicts, permissions, OOM — must exit
  if (/EADDRINUSE|EACCES|out of memory|cannot allocate/i.test(msg)) {
    console.error("[iris-lead] FATAL — exiting due to uncaught exception");
    process.exit(1);
  }

  // Default: log but keep alive
  console.error("[iris-lead] Unexpected uncaughtException — keeping alive (not fatal)");
});

// ── RT Bus listener — receives replies from agents ────────────────────────────

const RT_URL = process.env.IRIS_RT_URL || "ws://127.0.0.1:18889";
const RT_TOKEN = process.env.IRIS_RT_AUTH_TOKEN || (() => {
  const cs = loadSystemConfig();
...
// The iris-feed OpenCode plugin POSTs events here; we forward to the
