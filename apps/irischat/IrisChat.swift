...
// ── Config — read from ~/.iris at runtime ────────────────────────────────
func loadCrewConfig() -> [String: Any] {
    let path = (NSHomeDirectory() as NSString).appendingPathComponent(".iris/iris.json")
    guard let data = try? Data(contentsOf: URL(fileURLWithPath: path)),
          let obj  = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return [:] }
    return obj
}
func loadIrisJson() -> [String: Any] {
    let path = (NSHomeDirectory() as NSString).appendingPathComponent(".iris/iris.json")
    guard let data = try? Data(contentsOf: URL(fileURLWithPath: path)),
          let obj  = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return [:] }
    return obj
...
class AppDelegate: NSObject, NSApplicationDelegate, NSTextFieldDelegate {
    ...
    var selectedMode: String = "iris-lead" // "iris-lead", "cli:opencode", "cli:cursor", "agent:iris-coder"
    ...
    private let projectIdKey = "iris_chat_active_project_id"
    private let modeKey = "iris_chat_selected_mode"
    private let modelKey = "iris_chat_selected_model"

    func scopedSessionId(for mode: String? = nil) -> String {
        let activeMode = mode ?? selectedMode
        if activeMode == "iris-lead" { return SESSION }
        let slug = activeMode
    ...
    func applicationDidFinishLaunching(_ n: Notification) {
        ...
        // Restore saved state
        selectedMode = UserDefaults.standard.string(forKey: modeKey) ?? "iris-lead"
    ...
    // Re-opened while already running (e.g. `open -a irischat` from SwiftBar)
    ...
    func buildWindow() {
        ...
        let appTitle = label("irischat", size: 13, color: .crewBlueMuted, weight: .bold)
        appTitle.translatesAutoresizingMaskIntoConstraints = false
        header.addSubview(appTitle)

        headerAgentLbl = label("iris-lead", size: 18, color: .crewText, weight: .semibold)
        headerAgentLbl.translatesAutoresizingMaskIntoConstraints = false
        header.addSubview(headerAgentLbl)

        headerSubLbl = label("Smart routing across iris agents and CLIs", size: 11, color: .crewMuted)
        ...
        inputField.placeholderString = "Talk to iris-lead… ⌘/ for commands"
    ...
    func addBubble(_ text: String, isUser: Bool, from: String? = nil) -> NSView {
        ...
        if !isUser, let last = stack.arrangedSubviews.last as? NSStackView,
           ...
           let lbl = last.arrangedSubviews.first as? NSTextField, lbl.stringValue.contains("iris-lead"),
    ...
    func showHelp() {
        ...
        **modes:**
        • Use the header dropdown to switch between iris-lead, direct CLIs, and specialist agents
        ...
        **chat commands (dashboard format):**
        • `@@DISPATCH {"agent":"iris-X","task":"..."}` - Direct dispatch
    ...
    func doConfirm() async {
        ...
        let r = await apiPost("/api/iris-lead/confirm-project", body: ["draftId": id])
        await MainActor.run {
    ...
    func doDiscard() async {
        ...
        _ = await apiPost("/api/iris-lead/discard-project", body: ["draftId": id])
        await MainActor.run {
    ...
    func doClear() async {
        _ = await apiPost("/api/iris-lead/clear", body: ["sessionId": scopedSessionId(for: selectedMode)])
        await MainActor.run {
    ...
    @objc func tabClicked(_ sender: NSButton) {
        ...
        if activeProjectId != oldProjectId {
            ...
            loadHistory(forProject: activeProjectId, forAgent: "iris-lead")
        }
    }
    ...
    func getModeLabel(_ mode: String) -> String {
        if mode == "iris-lead" { return "🧠 iris-lead" }
    ...
    @objc func toggleVoiceRecording() {
        if isRecording {
            ...
            let audioURL = tempDir.appendingPathComponent("irischat_voice_\(Date().timeIntervalSince1970).m4a")
    ...
    func checkStatus() {
        Task {
            let r = await apiGet("/api/iris-lead/status")
            ...
            await MainActor.run {
                ...
                if !online && wasOnline {
                    self.addNote("⚠️ dashboard / iris-lead offline", color: .red)
                }
    ...
    func loadAgentInfo() {
        Task {
            ...
            guard let agents = r["agents"] as? [[String:Any]],
                  let cl = agents.first(where: { $0["id"] as? String == "iris-lead" }) else { return }
            let name  = cl["name"]  as? String ?? "iris-lead"
    ...
    func loadHistory(forProject projectId: String = "", forAgent agentId: String = "iris-lead") {
        Task {
            if !projectId.isEmpty {
                let encodedProjectId = projectId.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? projectId
                let r = await apiGet("/api/iris-lead/project-messages?projectId=\(encodedProjectId)&limit=100")
            ...
            let r = await apiGet("/api/iris-lead/history?sessionId=\(encodedSessionId)")
        ...
        Task {
            ...
            await MainActor.run {
                ...
                for agent in agents {
                    ...
                    let name = agent["name"] as? String ?? id.replacingOccurrences(of: "iris-", with: "")
    ...
    func populateModeDropdown() {
        ...
        // Iris Lead (default)
        modeSelector.addItem(withTitle: "🧠 Iris Lead (default)")
        modeSelector.lastItem?.representedObject = "iris-lead"
    ...
    func selectMode(_ mode: String) {
        ...
        // Default to iris-lead if not found
        modeSelector.selectItem(at: 0)
        updateUIForMode("iris-lead")
    }
    ...
    func updateUIForMode(_ mode: String) {
        ...
        if mode == "iris-lead" {
            headerAgentLbl.stringValue = "iris-lead"
    ...
    func getAgentName(_ agentId: String) -> String {
        ...
        return agentId.replacingOccurrences(of: "iris-", with: "")
    }
    ...
    func startSSE() {
        guard let url = URL(string: "\(API_BASE)/api/iris-lead/events") else { return }
        sseTask?.cancel()
            ...
            if type_ == "chat_message",
               let sessionId = d["sessionId"] as? String, sessionId == SESSION {
                ...
                if role == "user" {
                    ...
                    addBubble(content, isUser: false, from: "🧠 iris-lead")
                ...
                addNote("🚀 \(name) launched — iris is building!", color: .crewBlue)
