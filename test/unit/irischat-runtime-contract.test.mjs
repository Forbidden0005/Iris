...
const source = fs.readFileSync(
  path.resolve("apps/irischat/IrisChat.swift"),
  "utf8",
);

test("irischat mode dropdown includes iris-lead, direct CLIs, and agent section", () => {
  assert.match(source, /modeSelector\.addItem\(withTitle:\s*"🧠 Iris Lead \(default\)"\)/);
  assert.match(source, /\("⚡ OpenCode", "cli:opencode"\)/);
  assert.match(source, /\("🖱 Cursor CLI", "cli:cursor"\)/);
  assert.match(source, /\("🔧 Iris CLI", "cli:iris-cli"\)/);
...
test("irischat model filtering respects engine families", () => {
  ...
  assert.match(source, /case "opencode", "cursor", "iris-cli":[\s\S]*featuredModels/);
});

test("irischat fallback models cover Claude, Gemini, Codex, and a general fallback lane", () => {
...
test("irischat engine labels expose direct API and CLI runtime identity", () => {
...
test("irischat message source labels preserve source and agent identity", () => {
