/**
 * Unit tests for lib/iris-lead/interval-manager.mjs
...
describe("interval-manager — initIntervalManagers does not throw", () => {
  ...
  it("accepts Maps with pre-populated data without throwing", () => {
    ...
    const agents = new Map([["iris-coder", { since: Date.now() }]]);
...
describe("interval-manager — activeOpenCodeAgents cleanup callback (interval 1)", () => {
  it("invokes captured callback and removes stale agent, broadcasts agent_idle", () => {
    ...
    activeOpenCodeAgents.set("iris-coder", { since: STALE });
    activeOpenCodeAgents.set("iris-qa", { since: FRESH });

    capturedIntervals[1].fn();

    assert.ok(!activeOpenCodeAgents.has("iris-coder"), "stale agent not removed");
    assert.ok(activeOpenCodeAgents.has("iris-qa"), "fresh agent incorrectly removed");
    assert.equal(broadcastCalls.length, 1);
    assert.equal(broadcastCalls[0].type, "agent_idle");
    assert.equal(broadcastCalls[0].agent, "iris-coder");
  ...
  it("deletes stale agent sessions and broadcasts agent_idle (logic test)", () => {
    ...
    agents.set("iris-coder", { since: STALE });
    agents.set("iris-qa", { since: FRESH });
    ...
    assert.ok(!agents.has("iris-coder"), "stale agent not removed");
    assert.ok(agents.has("iris-qa"), "fresh agent incorrectly removed");
  ...
  it("broadcasts nothing when all agents are fresh", () => {
    const agents = new Map([["iris-coder", { since: Date.now() - 60_000 }]]);
    const calls = [];
    const staleThresholdMs = 15 * 60 * 1000;
    const now = Date.now();
    for (const [agentId, { since }] of agents.entries()) {
    ...
    assert.ok(agents.has("iris-coder"));
  });
  ...
  it("removes multiple stale agents in one pass", () => {
    ...
    agents.set("iris-coder", { since: STALE });
    agents.set("iris-qa", { since: STALE });
    agents.set("iris-pm", { since: Date.now() - 60_000 });
