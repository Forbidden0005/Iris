/**
 * Unit tests for lib/iris-lead/tools.mjs
...
import {
  ...
  execCrewLeadTools,
} from "../../lib/iris-lead/tools.mjs";

// ── Temp dir shared across tests ───────────────────────────────────────────
let tmpDir;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "iris-tools-test-"));
  initTools({ historyDir: tmpDir, irisCfgFile: path.join(tmpDir, "iris.json") });
});
...
describe("CREWLEAD_BLOCKED_CMDS", () => {
  ...
  it("blocks pkill -9 iris-lead", () => {
    assert.ok(CREWLEAD_BLOCKED_CMDS.test("pkill -9 iris-lead"));
  });
...
describe("execCrewLeadTools — @@RUN_CMD", () => {
  it("runs a safe command and captures stdout", async () => {
    const results = await execCrewLeadTools("@@RUN_CMD echo hello_iris");
    assert.ok(results[0].includes("✅"));
    assert.ok(results[0].includes("hello_iris"));
  });
...
describe("execCrewLeadTools — @@SEARCH_HISTORY", () => {
  ...
  it("returns no-history message when historyDir does not exist", async () => {
    initTools({
      ...
      irisCfgFile: path.join(tmpDir, "iris.json"),
    });
...
describe("execCrewLeadTools — @@WEB_SEARCH", () => {
  ...
  it("returns error when iris.json has no brave key", async () => {
    // Point to a temp dir with no iris.json so no Brave key is found
    initTools({ historyDir: tmpDir, irisCfgFile: path.join(tmpDir, "iris.json") });
    ...
    const results = await execCrewLeadTools("@@WEB_SEARCH iris lead test query");
    assert.equal(results.length, 1);
...
describe("execCrewLeadTools — @@WEB_FETCH", () => {
  it("returns error for unreachable URL (offline/nonexistent host)", async () => {
    ...
    const results = await execCrewLeadTools(
      "@@WEB_FETCH https://this-host-does-not-exist.iris-test.invalid/page",
    );
    assert.ok(results[0].includes("❌"));
  });
...
describe("execCrewLeadTools — @@TELEGRAM", () => {
  it("returns a result for @@TELEGRAM (either ✅ if configured or ❌ if not)", async () => {
    const results = await execCrewLeadTools("@@TELEGRAM __iris_test_ping__");
...
describe("execCrewLeadTools — @@WHATSAPP", () => {
  it("returns a result for @@WHATSAPP (error if no phone or bridge offline)", async () => {
    const results = await execCrewLeadTools("@@WHATSAPP __iris_test_ping__");
    assert.equal(results.length, 1, "expected exactly one result");
...
describe("execCrewLeadTools — tilde expansion in paths", () => {
  it("expands ~ to os.homedir() in @@READ_FILE", async () => {
    ...
    const subDir = fs.mkdtempSync(path.join(os.tmpdir(), "iris-tilde-test-"));
