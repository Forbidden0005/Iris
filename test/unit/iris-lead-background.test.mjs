/**
 * Unit tests for lib/iris-lead/background.mjs
...
import {
  ...
  RATE_LIMIT_PATTERN,
  _agentTimeoutCounts,
} from "../../lib/iris-lead/background.mjs";
...
function uniqueTestAgent() {
  return `iris-test-timeout-${++_testAgentSeq}`;
}
...
describe("background — recordAgentTimeout", () => {
  ...
  it("does not throw for unusual agent id characters", () => {
    assert.doesNotThrow(() => recordAgentTimeout("iris-custom-123_alpha"));
  });
...
describe("background — getRateLimitFallback", () => {
  it("returns iris-coder for iris-coder-back (static map)", () => {
    assert.equal(getRateLimitFallback("iris-coder-back"), "iris-coder");
  });

  it("returns iris-coder for iris-coder-front (static map)", () => {
    assert.equal(getRateLimitFallback("iris-coder-front"), "iris-coder");
  });

  it("returns iris-main for iris-coder (static map)", () => {
    assert.equal(getRateLimitFallback("iris-coder"), "iris-main");
  });

  it("returns iris-coder for iris-frontend (static map)", () => {
    assert.equal(getRateLimitFallback("iris-frontend"), "iris-coder");
  });

  it("returns iris-main for iris-pm (static map)", () => {
    assert.equal(getRateLimitFallback("iris-pm"), "iris-main");
  });

  it("returns iris-main for iris-qa (static map)", () => {
    assert.equal(getRateLimitFallback("iris-qa"), "iris-main");
  });

  it("returns iris-main for iris-copywriter (static map)", () => {
    assert.equal(getRateLimitFallback("iris-copywriter"), "iris-main");
  });

  it("returns iris-main for iris-security (static map)", () => {
    assert.equal(getRateLimitFallback("iris-security"), "iris-main");
  });

  it("returns iris-main for completely unknown agent", () => {
    // tryRead returns null → agent not found → default iris-main
    assert.equal(getRateLimitFallback("iris-totally-unknown-xyz"), "iris-main");
  });

  it("returns iris-main for empty string agent", () => {
    assert.equal(getRateLimitFallback(""), "iris-main");
  });

  it("uses _role fallback for dynamic agent with coder role", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return { agents: [{ id: "iris-ml", _role: "coder" }] };
        }
        return null;
      },
    });
    assert.equal(getRateLimitFallback("iris-ml"), "iris-coder");
  });
  ...
  it("uses _role fallback for dynamic agent with writer role", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return { agents: [{ id: "iris-blogger", _role: "writer" }] };
        }
        return null;
      },
    });
    assert.equal(getRateLimitFallback("iris-blogger"), "iris-copywriter");
  });
  ...
  it("uses _role fallback for dynamic agent with researcher role", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return { agents: [{ id: "iris-analyst", _role: "researcher" }] };
        }
        return null;
      },
    });
    assert.equal(getRateLimitFallback("iris-analyst"), "iris-main");
  });
  ...
  it("uses _role fallback for dynamic agent with auditor role", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return { agents: [{ id: "iris-auditor", _role: "auditor" }] };
        }
        return null;
      },
    });
    assert.equal(getRateLimitFallback("iris-auditor"), "iris-qa");
  });
  ...
  it("uses _role fallback for dynamic agent with ops role", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return { agents: [{ id: "iris-devops", _role: "ops" }] };
        }
        return null;
      },
    });
    assert.equal(getRateLimitFallback("iris-devops"), "iris-main");
  });
  ...
  it("returns agentId (self) when agent has fallbackModel configured", () => {
    initBackground({
      tryRead: (p) => {
        if (p.endsWith("iris.json")) {
          return {
            agents: [
              { id: "iris-ml", fallbackModel: "groq/llama-3.1-70b-versatile" },
            ],
          };
        }
        return null;
    ...
    assert.equal(getRateLimitFallback("iris-ml"), "iris-ml");
  });
});
