/**
 * Unit tests for lib/iris-judge/judge.mjs
 *
 * Covers:
...
const { judgeNextCycle, heuristicJudge } = await import("../../lib/iris-judge/judge.mjs");

// ── heuristicJudge — RESET branch ────────────────────────────────────────────

describe("iris-judge — heuristicJudge: RESET branch", () => {
...
describe("iris-judge — heuristicJudge: SHIP branch", () => {
...
describe("iris-judge — heuristicJudge: CONTINUE branch", () => {
...
describe("iris-judge — heuristicJudge: return shape", () => {
  const cases = [
...
describe("iris-judge — judgeNextCycle", () => {
  let tmpDir;
  let roadmapPath;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "iris-judge-test-"));
  ...
  it("handles LLM failure gracefully (confidence >= 0.1 in fail-open mode)", async () => {
    ...
    const orig = process.env.IRIS_JUDGE_MODEL;
    process.env.IRIS_JUDGE_MODEL = "invalid-provider/nonexistent-model";
    try {
      const result = await judgeNextCycle({
        roadmapPath,
        completedItems: ["a"],
      ...
      if (orig === undefined) {
        delete process.env.IRIS_JUDGE_MODEL;
      } else {
        process.env.IRIS_JUDGE_MODEL = orig;
      }
    }
  });
});
