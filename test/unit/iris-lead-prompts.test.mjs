/**
 * Unit tests for lib/iris-lead/prompts.mjs
...
import {
  initPrompts,
  buildSystemPrompt,
} from "../../lib/iris-lead/prompts.mjs";
...
function resetPromptModule() {
  ...
  initPrompts({
    irisCfgFile: "/nonexistent/iris.json",
...
describe("prompts — agentRoster rendering", () => {
  ...
  it("uses knownAgents when agentRoster is empty", () => {
    ...
    assert.ok(prompt.includes("iris-coder"));
    assert.ok(prompt.includes("iris-qa"));
  });
  ...
  it("prefers agentRoster over knownAgents — agentRoster member is listed", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [
          { id: "iris-coder", name: "Coder", emoji: "💻", model: "groq/llama" },
        ],
        knownAgents: ["iris-totally-unique-not-in-roster-xyz"],
      }),
    );
    // agentRoster member appears (in the agent list section)
    assert.ok(prompt.includes("iris-coder"), "agentRoster member not found");
    // The unique knownAgent string should NOT appear (since agentRoster is used instead)
    assert.ok(
      !prompt.includes("iris-totally-unique-not-in-roster-xyz"),
  ...
  it("includes agent model in brackets when provided", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [
          { id: "iris-coder", name: "Coder", emoji: "💻", model: "openai/gpt-5.4" },
        ],
      }),
    );
  ...
  it("renders role from static _FUNCTIONAL_ROLES_STATIC for iris-coder", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [
          { id: "iris-coder", name: "Coder" },
        ],
      }),
    );
    assert.ok(
      prompt.includes("general coding"),
      "Expected static role for iris-coder",
    );
  });

  it("renders role from static map for iris-qa", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [{ id: "iris-qa", name: "QA" }],
      }),
    );
  ...
  it("falls back to 'general agent' for unknown id with no config", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [{ id: "iris-unknown-xyz", name: "Unknown" }],
      }),
    );
  ...
  it("uses identity.theme as role for dynamic agent when available", () => {
    ...
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [{ id: "iris-custom", name: "Custom" }],
      }),
    );
  ...
  it("uses _role description for dynamic agent with recognized _role", () => {
    ...
    const prompt = buildSystemPrompt(
      baseCfg({
        agentRoster: [{ id: "iris-ml", name: "MLBot" }],
      }),
    );
  ...
  it("includes model list header when agentModels is populated", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        ...
        modelId: "gpt-5.4",
        agentModels: {
          "iris-lead": "openai/gpt-5.4",
          "iris-coder": "groq/llama-3.1-70b",
        },
      }),
    );
    assert.ok(prompt.includes("YOUR model (iris-lead)"));
    assert.ok(prompt.includes("iris-coder: groq/llama-3.1-70b"));
  });

  it("excludes iris-lead from the 'other agents' list", () => {
    const prompt = buildSystemPrompt(
      baseCfg({
        agentModels: {
          "iris-lead": "openai/gpt-5.4",
    ...
    // iris-lead should appear once in the YOUR model line only
    const idx = prompt.indexOf("iris-lead: openai/gpt-5.4");
    // The "other agents" block should not list iris-lead → -1
    assert.equal(idx, -1, "iris-lead must not appear in other-agents list");
  });
...
describe("prompts — custom prompt injection", () => {
  ...
  it("injects custom prompt when getAgentPrompts returns iris-lead entry", () => {
    initPrompts({
      getAgentPrompts: () => ({ "iris-lead": "CUSTOM RULE: always be helpful" }),
      tryRead: () => null,
    });
    const prompt = buildSystemPrompt(baseCfg());
  ...
  it("still includes identity line with custom prompt", () => {
    initPrompts({
      getAgentPrompts: () => ({ "iris-lead": "custom instructions" }),
      tryRead: () => null,
    });
    const prompt = buildSystemPrompt(
      baseCfg({ providerKey: "groq", modelId: "llama-3.1-8b-instant", displayName: "Lead" }),
    );
    assert.ok(prompt.includes("iris-lead"), "identity line missing");
  ...
  it("trims whitespace from custom prompt", () => {
    initPrompts({
      getAgentPrompts: () => ({ "iris-lead": "   trimmed rule   " }),
      tryRead: () => null,
    });
    const prompt = buildSystemPrompt(baseCfg());
