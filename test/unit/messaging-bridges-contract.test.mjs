...
test("telegram bridge uses startup lock, RT bus, and iris-lead chat/events paths", () => {
  ...
  assert.match(telegramSource, /const AGENT_NAME\s+=\s+"iris-telegram"/);
  assert.match(telegramSource, /const IRIS_LEAD_URL = .*"http:\/\/127\.0\.0\.1:5010"/);
  assert.match(telegramSource, /fetch\(`\$\{IRIS_LEAD_URL\}\/chat`, \{/);
  assert.match(telegramSource, /const IRIS_LEAD_EVENTS = `\$\{IRIS_LEAD_URL\}\/events`/);
  assert.match(telegramSource, /saveBridgeMessage\(/);
  assert.match(telegramSource, /saveBridgeMessage, detectProjectFromMessage/);
});

test("whatsapp bridge exposes local send and health endpoints plus RT and iris-lead wiring", () => {
  assert.match(whatsappSource, /const HTTP_PORT\s+=.*"5015"/);
  assert.match(whatsappSource, /const AGENT_NAME\s+=\s+"iris-whatsapp"/);
  assert.match(whatsappSource, /const RT_URL\s+=.*"ws:\/\/127\.0\.0\.1:18889"/);
  assert.match(whatsappSource, /const IRIS_LEAD_URL = .*"http:\/\/127\.0\.0\.1:5010"/);
  ...
  assert.match(whatsappSource, /fetch\(`\$\{IRIS_LEAD_URL\}\/chat`/);
});
