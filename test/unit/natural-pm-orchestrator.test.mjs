...
describe("natural-pm-orchestrator — normalizeAgentName: coder aliases", () => {
  it("maps 'codex' to iris-coder", () => {
    assert.equal(normalizeAgentName("codex"), "iris-coder");
  });

  it("maps 'coder' to iris-coder", () => {
    assert.equal(normalizeAgentName("coder"), "iris-coder");
  });

  it("maps 'developer' to iris-coder", () => {
    assert.equal(normalizeAgentName("developer"), "iris-coder");
  });
});

describe("natural-pm-orchestrator — normalizeAgentName: qa aliases", () => {
  it("maps 'qa' to iris-qa", () => {
    assert.equal(normalizeAgentName("qa"), "iris-qa");
  });

  it("maps 'tester' to iris-qa", () => {
    assert.equal(normalizeAgentName("tester"), "iris-qa");
  });

  it("maps 'test' to iris-qa", () => {
    assert.equal(normalizeAgentName("test"), "iris-qa");
  });
});

describe("natural-pm-orchestrator — normalizeAgentName: other aliases", () => {
  it("maps 'fixer' to iris-fixer", () => {
    assert.equal(normalizeAgentName("fixer"), "iris-fixer");
  });

  it("maps 'debugger' to iris-fixer", () => {
    assert.equal(normalizeAgentName("debugger"), "iris-fixer");
  });

  it("maps 'security' to iris-security", () => {
    assert.equal(normalizeAgentName("security"), "iris-security");
  });

  it("maps 'guardian' to iris-security", () => {
    assert.equal(normalizeAgentName("guardian"), "iris-security");
  });

  it("maps 'audit' to iris-security", () => {
    assert.equal(normalizeAgentName("audit"), "iris-security");
  });

  it("maps 'pm' to iris-pm", () => {
    assert.equal(normalizeAgentName("pm"), "iris-pm");
  });

  it("maps 'planner' to iris-pm", () => {
    assert.equal(normalizeAgentName("planner"), "iris-pm");
  });

  it("maps 'quill' to iris-main", () => {
    assert.equal(normalizeAgentName("quill"), "iris-main");
  });

  it("maps 'main' to iris-main", () => {
    assert.equal(normalizeAgentName("main"), "iris-main");
  });
...
describe("natural-pm-orchestrator — parseNaturalLanguagePlan: pattern 1 (I'll have)", () => {
  it("parses 'I'll have Codex create the file'", () => {
    ...
    assert.equal(result[0].agent, "iris-coder");
  ...
  it("parses both tasks from a two-step plan", () => {
    ...
    assert.ok(agents.includes("iris-coder"), `Expected iris-coder in: ${JSON.stringify(agents)}`);
  });
  ...
  it("normalizes agent name in pattern 1", () => {
    ...
    assert.ok(result.some(r => r.agent === "iris-qa"));
  });
});
...
describe("natural-pm-orchestrator — parseNaturalLanguagePlan: pattern 2 (will/should/can)", () => {
  ...
  it("parses 'Security can audit the routes'", () => {
    ...
    assert.ok(result.some(r => r.agent === "iris-security"));
  });

  it("capitalizes action verb in task field", () => {
    const result = parseNaturalLanguagePlan("Fixer should fix the bug.");
    const entry = result.find(r => r.agent === "iris-fixer");
    assert.ok(entry, "Expected iris-fixer entry");
  ...
  it("builds acceptance from target and action", () => {
    ...
    const entry = result.find(r => r.agent === "iris-coder");
    assert.ok(entry, "Expected iris-coder entry");
...
describe("natural-pm-orchestrator — parseNaturalLanguagePlan: pattern 3 (Task for)", () => {
  it("parses 'Task for coder: Create package.json'", () => {
    ...
    assert.ok(result.some(r => r.agent === "iris-coder" && r.task.includes("Create package.json")));
  });
...
describe("natural-pm-orchestrator — parseNaturalLanguagePlan: edge cases", () => {
  ...
  it("handles mixed patterns in one text", () => {
    ...
    assert.ok(agents.some(a => a === "iris-coder" || a === "iris-qa" || a === "iris-fixer"));
  });
});
