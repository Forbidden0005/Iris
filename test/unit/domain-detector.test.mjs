...
describe("DOMAINS constant", () => {
  it("defines all expected domain IDs", () => {
    const expected = ["iris-cli", "frontend", "core", "integrations", "docs"];
...
describe("detectDomain", () => {
  it("detects iris-cli domain for CLI-related text", () => {
    const result = detectDomain("Add a new CLI command for iris exec in iris-cli/src/");
  ...
  it("detects core domain for orchestration text", () => {
    const result = detectDomain("Fix the gateway-bridge WebSocket dispatch and iris-lead coordination");
  ...
  it("confidence is between 0 and 1", () => {
    const result = detectDomain("Add a CLI command for iris exec in iris-cli/src/ with TypeScript pipeline");
    assert.ok(result.confidence >= 0);
...
describe("buildDomainContext", () => {
  ...
  it("includes domain description and subdirs for iris-cli", () => {
    const ctx = buildDomainContext("iris-cli", "Add new command");
    assert.ok(ctx.includes("iris-cli"));
    assert.ok(ctx.includes("Domain:"));
    assert.ok(ctx.includes("iris-cli/src"));
  });
...
describe("logDomainRouting", () => {
  it("does not throw for matched domain", () => {
    assert.doesNotThrow(() => {
      logDomainRouting("CLI task", { domain: "iris-cli", pmAgent: "iris-pm-cli", confidence: 0.8 });
    });
  });

  it("does not throw for unmatched domain", () => {
    assert.doesNotThrow(() => {
      logDomainRouting("vague task", { domain: null, pmAgent: "iris-pm", confidence: 0 });
    });
  });
});
