#!/usr/bin/env node
/**
 * IRIS CLI - Unified interface for iris multi-agent system
 * 
 * Usage:
 *   iris "Build user auth system"
 *   iris code "Create login endpoint"
 *   iris test "Test auth flow"
 *   iris fix "Debug login timeout"
 *   iris audit "Security review of auth"
 *   iris --watch "Build dashboard"
 *   iris --github "Build payment system"
 */
...
const ORCHESTRATOR = join(IRIS_DIR, 'natural-pm-orchestrator.mjs');
...
function showUsage() {
  console.log(`
${COLORS.bright}IRIS CLI${COLORS.reset} - iris Multi-Agent System

${COLORS.cyan}Basic Usage:${COLORS.reset}
  iris "Build user authentication system"
  iris "Fix all linter errors in src/"
  iris "Add JSDoc comments to api.js"

${COLORS.cyan}Specialized Agents:${COLORS.reset}
  iris code  "Create login endpoint"       ${COLORS.dim}→ Routes to iris-coder${COLORS.reset}
  iris test  "Test auth flow"              ${COLORS.dim}→ Routes to iris-qa${COLORS.reset}
  iris fix   "Debug login timeout"         ${COLORS.dim}→ Routes to iris-fixer${COLORS.reset}
  iris audit "Security review"             ${COLORS.dim}→ Routes to iris-security${COLORS.reset}

${COLORS.cyan}Advanced Options:${COLORS.reset}
  iris --watch "Build dashboard"           ${COLORS.dim}→ Watch for file changes${COLORS.reset}
  iris --github "Build payment system"     ${COLORS.dim}→ Create PR when done${COLORS.reset}
  iris --cursor "Refactor auth.js"         ${COLORS.dim}→ Open results in Cursor${COLORS.reset}
  iris --status                            ${COLORS.dim}→ Show iris status${COLORS.reset}

${COLORS.cyan}Examples:${COLORS.reset}
  ${COLORS.dim}# Complex multi-agent task${COLORS.reset}
  iris "Build TODO API with CRUD, tests, and security audit"

  ${COLORS.dim}# Watch mode (see changes in real-time)${COLORS.reset}
  iris --watch "Add dark mode to dashboard"

  ${COLORS.dim}# Auto-create GitHub PR${COLORS.reset}
  iris --github "Implement stripe payment integration"

  ${COLORS.dim}# Quick fix and review in Cursor${COLORS.reset}
  iris --cursor fix "TypeError in login handler"
`);
}
...
async function createGitHubPR(result, task) {
  ...
  const branchName = `iris/${result.opId}`;
...
${result.changedFiles.map(f => `- ${f}`).join('\n')}"`);
    ...
    try {
      await execCommand(`gh pr create --title "feat: ${commitMessage}" --body "**Auto-generated by iris**
...
---
*This PR was created automatically by iris.*" --web`);
...
async function showStatus() {
  log('cyan', '📊', 'Iris Status');
  console.log('');
  
  try {
    // Check iris-lead
    await execCommand('curl -sf http://127.0.0.1:5010/health');
    log('green', '✓', 'iris-lead: Running (port 5010)');
  } catch {
    log('red', '✗', 'iris-lead: Stopped (run: node iris-lead.mjs)');
  }
